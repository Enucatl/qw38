...
| --- | --- | --- | --- | --- | --- | --- |
| A-01 | CUDA/Blackwell-oriented `.qw38` physical representation, one packed view; portable semantics remain the backend boundary | Runtime-format and traffic studies | DERIVED + HYPOTHESIS + POLICY | Removes runtime repacking and duplicate-view capacity while keeping model meaning independent of physical lowering | CUDA/Blackwell specialization harms prefill, workflow, or later backend work enough to outweigh it | EXP-G; a funded non-CUDA backend or portable-artifact requirement |
