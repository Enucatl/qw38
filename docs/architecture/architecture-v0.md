...
These are implementation decisions. Their quality and speed are **HYPOTHESIS**, not benchmark results. **OBSERVED** denotes checkpoint/config facts, **MEASURED** existing tensor statistics, **DERIVED** equations or arithmetic, **POLICY** an explicit V0 engineering constraint, and **UNKNOWN** a fact the dossier does not establish. Uncertainty changes a decision only through the experiments below; it does not leave the initial implementation unspecified.
...
| --- | --- | --- | --- | --- | --- | --- |
| A-01 | Historical V0 control: CUDA/Blackwell-oriented `.qw38` physical representation, one packed view; portable semantics remain the backend boundary. Candidate physical representation is reopened by OVERALL-01. | Runtime-format and traffic studies | DERIVED + HYPOTHESIS + POLICY | Removes runtime repacking and duplicate-view capacity while keeping model meaning independent of physical lowering | CUDA/Blackwell specialization harms prefill, workflow, or later backend work enough to outweigh it | TASK-019–021 feasibility, layout, and artifact evidence |
...
| I-01 | NVIDIA CUDA is the V0 reference backend | Existing CUDA lowering and kernel design | POLICY | Makes the runtime, artifact consumers, and profiling target concrete | A different backend becomes necessary for the first deliverable | A changed product target requiring a non-CUDA backend |
| I-02 | NVIDIA GeForce RTX 5090 / Blackwell compute capability 12.0, native `sm_120`, is the reference hardware | Need for a concrete resource and code-generation target | POLICY + HYPOTHESIS | Allows kernels and layouts to be evaluated against one modern device | Results on one GPU adequately represent the desired deployment | Reference deployment hardware changes or measurements invalidate the specialization |
| I-03 | V0 may optimize specifically for Blackwell | Selected reference hardware and specialized physical lowering | POLICY + HYPOTHESIS | Avoids weakening the first implementation for unvalidated compatibility | Blackwell-specific choices provide useful simplicity or performance | Specialization prevents meeting the first deployment or quality/performance goals |
| I-04 | Reference deployment is Linux in Docker with NVIDIA Container Toolkit integration | Reproducible CUDA userspace and deployment isolation | POLICY | Separates host driver/GPU integration from a pinned userspace environment | Containerized deployment satisfies development and runtime needs | A required deployment environment cannot support this container model |
| I-05 | Implementation languages are C++23 and CUDA C++23 | Runtime scope and CUDA backend choice | POLICY | Provides modern typed host facilities and native CUDA kernel implementation | The pinned CUDA/host compiler pair supports the required language subsets | Toolchain validation shows a required facility is unsupported or unreliable |
| I-06 | `.qw38` physical packing and CUDA lowering may be CUDA/Blackwell-specific; model semantics and the semantic graph remain portable | Artifact boundary and backend separation | DERIVED + POLICY | Permits consumer-oriented bytes without coupling model meaning to one backend | Backend-specific bytes remain regenerable from portable logical inputs | A requirement emerges for one physical artifact to serve heterogeneous backends |
| I-07 | Pre-Blackwell GPU and old CUDA/host-toolchain portability are not V0 constraints | Clean-sheet scope and reference-platform policy | POLICY | Keeps compatibility work from constraining the first implementation | Deferring compatibility does not block the intended deployment | Required V0 users or environments depend on older hardware/toolchains |
...
| G-02 | Decode and prefill retain separate logical execution phases; their physical schedules are candidate choices under OVERALL-01 | Decode/prefill work and reuse analysis | DERIVED + HYPOTHESIS | GEMV/state work differs from GEMM/token reuse | Common schedule is competitive | TASK-019–027 whole-path comparison |
| L-01 | Historical V0 control layouts `cuda_q4g64_v0` / `cuda_q8g32_v0`; candidate layouts are reopened by OVERALL-01 | Q4G64/Q8G32 consumer design | HYPOTHESIS | Matches initial warp-row MMV and local prefill decode | Another ownership/packing is materially better | TASK-019–021 feasibility, layout, artifact and consumer evidence |
| M-01 | Residual/state/output obligations remain semantic controls; scratch cuts and materialization are candidate choices under OVERALL-01 | Lifetime and materialization analysis | DERIVED + HYPOTHESIS | Separates mandatory state from debuggable V0 stores | A chosen store is too costly | TASK-019–027 measurements |
| T-01 | Historical decode projection tuning control: 8 warps/block, 256-input K tile | Consumer dimensions | HYPOTHESIS — tuning | Simple aligned first kernel | Resource/throughput profile is poor | TASK-019+ profiling |
| T-02 | Historical prefill tuning control: 256-token input chunk, 32×64×64 projection tile, 64-token recurrence interval | Working-set reasoning | HYPOTHESIS — tuning | Bounds scratch and reuses operands/state | Another geometry is materially better | TASK-019–026 measurement; do not freeze as production defaults |
| T-03 | Historical attention tuning control: 128-thread blocks, 256-key decode segment / 32-key subtile, 32-query × 64-key prefill tile | Head dimensions and working-set reasoning | HYPOTHESIS — tuning | Supplies parallel segments with bounded staging | Register/shared-memory profile is poor | TASK-019–026 profiling |
...
The header identifies the container version and manifest location. The directory records each tensor's logical identity and shape, storage class, **quantizer version**, physical layout version (`cuda_q4g64_v0`, `cuda_q8g32_v0`, or a BF16 layout), logical-to-physical mapping, payload/scale offsets and lengths, and shared bindings. A shared binding is a direct edge from one canonical non-alias owner to one alias; each alias has exactly one owner, and reverse pairs, alias-owned chains, and cycles are invalid. It also records the source metadata hash, config hash, compiler revision, precision policy, supported semantic scope, and state allocation schema. The source metadata hash identifies checkpoint metadata only; it never hashes or verifies the downloaded BF16 tensor payloads or shard files. Use 64-bit byte offsets and lengths and 256-byte-aligned payload and scale spans. SHA-256 is permitted only for the small `.qw38` manifest; never hash tensor payload or scale spans, including quantized output. Reader validation resolves alias spans to their canonical owners and rejects unsupported layout versions and malformed spans before allocating device memory.
...
The BF16 embedding accounts for about 2.37 GiB of capacity but is not streamed per token. Language MLP weights become 9,091,153,920 bytes. A second complete low-bit decode/prefill view would add about **13.50 GiB** for Q4 plus Q8 alone, before state and workspace. The original V0 control therefore accepts local shared-memory rearrangement during prefill and stores **one view**. Under OVERALL-01, selected duplicate views or bounded conversions are candidate choices; TASK-019–021 must justify their full capacity and end-to-end cost before use.
...
methodology's unselected suite and scoring policy. For TASK-018 onward, the
QW38 evaluation arm is the selected candidate under OVERALL-01; historical V0
runs remain development controls.
...
Retain TASK-017's BF16 identity compiler/source semantic evidence. Use focused semantic checks for norm roles, per-head q/g split, RoPE coordinates, convolution tap order, GDN update/readout, causal masks, and state continuation. Compare full prefill, chunked prefill, and repeated decode on the same token IDs; these checks detect semantic and chunk-boundary errors before evaluating compression. Tensor reconstruction and intermediate differences diagnose failures but are not the final quality gate. Under EVAL-01, BF16 is limited to an optional elementary tensor/load sanity check and is not a generation or scoring arm.

Evaluate the selected candidate under EVAL-01 using stable FP32 log-softmax over its full 248320-token vocabulary to score the frozen Q4_K_M teacher tokens. Compare target-token NLL against the teacher's captured target log probabilities on aligned P100/C92 targets. llama.cpp's top-20 output cannot supply full-vocabulary teacher KL; report that metric as unavailable (`null`) and retain the policy's top-20 overlap diagnostics. Run deterministic greedy generation and the required language, code, arithmetic/reasoning, and long-context retrieval cases. Hash the tokenized evaluation inputs and loss masks; reset at case boundaries and count each scored target once. Calibration uses disjoint data. MTP metrics remain unavailable until its semantics are verified and must never be folded into an apparently comparable language-only average.
