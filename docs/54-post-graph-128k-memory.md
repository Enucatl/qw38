...
| Atomic workspace | 172,963,328 | Candidates, scores, projections, logits, taps |
| 64 uploaded graphs | 6,291,456 | Measured graph executable allocation |
| Explicit Quartz total | 27,901,919,072 | Model + session + workspace + graphs |
| Allocator delta | 2,785,440 | CUDA delta not assigned to explicit owners |
...
Before graph creation, 5,241,044,992 bytes were free. After uploading the graph
executables, 5,234,753,536 bytes were free; their difference is the measured
6,291,456-byte graph allocation.

## Reserve result

The device exposes 33,671,348,224 total bytes. With every owner and graph alive,
5,234,753,536 bytes remained free. The required reserve is 1,610,612,736 bytes
(1.5 GiB), leaving a margin of **3,624,140,800 bytes**.

Process resident host memory measured 19,113,730,048 bytes. Host RSS is not VRAM
...
**Measured:** MEM-001 is now admitted on the pinned artifact, CUDA 13.0.2, and
RTX 5090: 131,072-token capacity plus all current graph/workspace/model/session
