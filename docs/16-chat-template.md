...
TOK-002 covers deterministic rendering only. SRV-001 supplies the bounded HTTP
listener and queue; SRV-002 now supplies Chat Completions parsing, streaming,
tools, and active CUDA cancellation. Responses objects and
`previous_response_id` remain SRV-003 work.
