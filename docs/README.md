...
44. [Memory-bounded CUDA attention prefill](44-cuda-attention-prefill.md): token-major chunks, candidate continuity, linear score workspaces, fattn-mma quality for `token_count >= 16`, Ada+ stream-K occupancy, persistent stream-K reject, register-resident value sums, tiled numeric reference, whole-chunk commit, exact token-wise equivalence, and the 131,072 boundary.
