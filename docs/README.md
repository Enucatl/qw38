...
4. [Scalar oracle](04-numerics.md): reference implementation, differential tests, and the OPT-044 strict-versus-optimized production arithmetic policy.
5. [Weights and the 32 GiB ledger](05-gpu-implementation.md): conversion and quantization.
6. [CUDA decode and prefill](06-system-optimization.md): MMV, MMQ, GDN, attention, mixer/FFN MMA, GDN/attention core quality, mixer Q8 quality / shared residual Y, skinny-M mixer dispatch, large-mixer D2R reject, dense FFN shared-Y / SwiGLU-into-down Q8, fattn Ada+ stream-K occupancy, persistent Ada+ fattn stream-K reject, Q4_K/Q6_K MMQ stream-K reject, GDN conv/gate fuse reject, Hopper/Blackwell PDL prompt-launch reject, the 4K keep/reject oracle, frozen P/D128/D2048 decode oracles with a recorded next-task order (no performance-improvement claim), the post-ladder P/D128/D2048 refresh with independent raw-wall accounting and matched-component specs (no performance-improvement claim; keep denominators unchanged), the decode one-token KV-partition keep, the prefill register-resident value-sum keep, the dual-F16 probability×V MMA keep, the warp-owned prompt QK microtile keep, the packed blockwise Q4_K/Q6_K MMV-load keep, the 4K FFN per-projection tile reject, the shared prompt GDN Q/K inverse hoist keep, the hoisted scaled Q/K and decay / conversion-inclusive GDN transpose keep, the MMQ FMA scale / packed-Y cp.async keep, and the OPT-044 production arithmetic freeze (strict reference versus optimized production; no unvalidated fast kernel; no performance-improvement claim).
...
39. [The first CUDA matrix-vector kernel](39-cuda-quant-mmv.md): BF16-to-Q8 staging, packed Q4_K/Q6_K rows, warp ownership, FP32 reduction, packed blockwise Q4_K/Q6_K field/scale loads, integer-dot decode-MMV admissibility diagnostic, device gates, and measured proof limits.
40. [Tiled CUDA multiplication for prompt rows](40-cuda-prompt-mmq.md): prompt matrices, token-major output, two-dimensional tiles, packed-weight reuse, tails, scalar/device gates, the 2K FFN/MMQ sink, mixer Q8_0 MMA MMQ, Q4_K/Q6_K quality MMA MMQ, mixer Q8_0 quality MMQ with shared residual Y, skinny-M mixer dispatch for small output rows, large-mixer aligned-SoA D2R reject, dense FFN shared-Y and SwiGLU-into-down Q8, Q4_K/Q6_K MMQ stream-K reject, and the unowned 2K parity boundary.
41. [One CUDA GDN step and atomic state staging](41-cuda-gdn-step.md): production recurrence, causal convolution, hoisted prompt Q/K inverses, candidate state, cancellation, commit, and frontier publication.
42. [Chunked CUDA GDN prefill](42-cuda-gdn-chunks.md): token-major chunks, strict recurrence order, 64-token windows, associative overlay scan, warp-column fused quality production, shared prompt Q/K inverse hoist, hoisted scaled Q/K and decay / conversion-inclusive transpose keep, conv/gate fuse reject, sequential reference, candidate continuity, cancellation, and token-wise equivalence.
43. [CUDA grouped-query attention decode](43-cuda-attention-decode.md): heads and KV sharing, normalization, partial RoPE, BF16 cache rows, causal stable softmax, candidate commit, one-token KV partitions with FP32 partial merge, warp-owned query-head decode at 16/16, independent D128/D2048 selection, and frozen numeric gates.
44. [Memory-bounded CUDA attention prefill](44-cuda-attention-prefill.md): token-major chunks, candidate continuity, linear score workspaces, fattn-mma quality for `token_count >= 16`, Ada+ stream-K occupancy, persistent stream-K reject, register-resident value sums, dual-F16 probability×V MMA, warp-owned prompt QK microtiles, tiled numeric reference, whole-chunk commit, exact token-wise equivalence, and the 131,072 boundary.
...
51. [Synchronized runtime timing and NVTX attribution](51-runtime-timing-and-nvtx.md): asynchronous launches, CUDA events, CPU clocks, named ranges, decode and 2K prefill attribution, mixer-projection MMQ versus gdn_core/attention_core, exclusive decode mixer_mmv/gdn_core/attention_core/ffn_mmv categories, independent raw host wall versus adjusted reconstruction, remainder other/idle, unavailable boundaries, overhead, and profiler limits.
...
53. [Stable-address CUDA graphs](53-stable-address-cuda-graphs.md): capture, instantiate, upload, replay, pointer lifetime, decode and 4,096-row prompt FFN graphs, PDL forbidden during FFN capture, OPT-055 remaining launch-gap no-change, dynamic exclusions, equality, launch attribution, and graph memory.
...
61. [Benchmark harness](61-benchmark-harness.md): workloads, warm-ups, samples, percentiles, telemetry, cache policy, atomic results, the frozen 4K keep/reject oracle, D128/D2048 decode oracles, the post-ladder P/D128/D2048 refresh versus this harness, the OPT-056 same-sitting outcome gate (unpassed), and proof limits.
62. [Chunked full-model CUDA prefill](62-cuda-full-prefill.md): token-major rows, layer-major work, exact MMQ/GDN/attention chunks, atomic commit, scratch, measured 2K attribution, mixer versus core split, ranked recovery, GDN/attention core remasurement, 4K keep/reject oracle, mixer Q8 quality 4K keep, skinny-M mixer 4K keep, large-mixer D2R 4K reject, dense FFN shared-Y 4K keep, fattn stream-K 4K keep, persistent fattn stream-K 4K reject, Q4_K/Q6_K MMQ stream-K 4K reject, GDN conv/gate fuse 4K reject, Hopper/Blackwell PDL 4K reject, 4K prefill attribution refresh with graphs created, post-ladder 4K attribution refresh with independent raw host wall, frozen P/D128/D2048 oracles, register-resident value-sum 4K keep, dual-F16 probability×V MMA 4K keep, warp-owned prompt QK microtile 4K keep, shared prompt GDN Q/K inverse hoist 4K keep, hoisted scaled Q/K and decay / conversion-inclusive GDN transpose 4K keep, MMQ FMA scale / packed-Y cp.async 4K keep, internal 512/1024/2048/4096 prefill microbatches inside an atomic 4K transaction, remaining decode/prompt launch-gap measurement with FFN-only graphs kept, the OPT-056 same-sitting P/D128/D2048 outcome gate (unpassed), and proof boundaries.
