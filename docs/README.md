...
6. [CUDA decode and prefill](06-system-optimization.md): MMV, MMQ, GDN, attention, mixer/FFN MMA, GDN/attention core quality, mixer Q8 quality / shared residual Y, skinny-M mixer dispatch, large-mixer D2R reject, dense FFN shared-Y / SwiGLU-into-down Q8, fattn Ada+ stream-K occupancy, and the 4K keep/reject oracle.
...
44. [Memory-bounded CUDA attention prefill](44-cuda-attention-prefill.md): token-major chunks, candidate continuity, linear score workspaces, fattn-mma quality for `token_count >= 16`, Ada+ stream-K occupancy, tiled numeric reference, whole-chunk commit, exact token-wise equivalence, and the 131,072 boundary.
