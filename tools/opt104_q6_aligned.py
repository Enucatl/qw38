"""OPT-104 lossless 64-byte-aligned Q6_K SoA admission. Production stays raw until keep."""
...
CONTRACT = ROOT / "pins/opt104_q6_aligned_contract.json"
ITERATION = ROOT / "pins/opt104_iteration_contract.json"
FIXTURE = ROOT / "fixtures/opt104_q6_aligned.json"
REPORT = ROOT / "evidence/optimization/opt104-q6-aligned-layout/REPORT.md"
EVIDENCE = REPORT.parent
OPT091_FIXTURE = ROOT / "fixtures/opt091_quality_tradeoff.json"
Q6_PIN_FILE = ROOT / "cuda/q6k_decode_path.cuh"
REPLAY = "build/qw38-cuda-component-replay"
PROBE = "build/qw38-cuda-optimization-engine-probe"
NATIVE_PARITY = "build/qw38-cuda-opt104-q6-aligned-test"
MODEL = "models/Qwen3.8-27B-Q4_K_M.gguf"
PHASES = ("parity", "q6", "quality", "d128", "d2048", "prefill-guard")
CONTROL_ID = "raw_gguf"
CANDIDATE_ID = "aligned_soa"
VERDICT_KEYS = (
    "kernel_parity_pass",
    "model_quality_pass",
    "performance_pass",
...
CONFIGS: tuple[dict[str, Any], ...] = (
    {
        ...
        "q6_warps": 2,
        "role": "control",
        "expected_q6_launches": Q6_LAUNCHES,
    },
    {
        "id": CANDIDATE_ID,
        "q6_device_layout": "aligned_soa",
        "q6_decode": "integer_q8_1",
        "q6_warps": 2,
        "role": "candidate",
        "source_task": "OPT-104",
...
def current_layout_pin() -> str:
    match = re.search(
        r'kSelectedQ6DeviceLayout\[\] = "([^"]+)"',
        Q6_PIN_FILE.read_text(encoding="utf-8"),
    )
    if match is None:
        raise AdmissionError("missing kSelectedQ6DeviceLayout")
    return match.group(1)


def apply_layout_pin(layout_id: str) -> None:
    text = Q6_PIN_FILE.read_text(encoding="utf-8")
    updated = re.sub(
        r'kSelectedQ6DeviceLayout\[\] = "[^"]+"',
        f'kSelectedQ6DeviceLayout[] = "{layout_id}"',
        text,
        count=1,
    )
    if updated == text:
        raise AdmissionError("failed to rewrite the Q6 device layout pin")
    Q6_PIN_FILE.write_text(updated, encoding="utf-8")


def apply_production_pins(results: Mapping[str, Any]) -> dict[str, Any]:
    applied = {"layout": False}
    if results.get("production_kept") and not results.get("shipping_unchanged"):
        want = str(results.get("shipping_layout") or "")
        if not want:
            raise AdmissionError("keep is missing shipping layout id")
        if want != current_layout_pin():
            apply_layout_pin(want)
            applied["layout"] = True
    elif results.get("mode") == "acceptance" and not results.get("production_kept"):
        if current_layout_pin() != CONTROL_ID:
            apply_layout_pin(CONTROL_ID)
            applied["layout"] = True
    applied["layout_pin"] = current_layout_pin()
    return applied
...
def candidate_selectors(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    selectors["q6_device_layout"] = config["q6_device_layout"]
    selectors["q6_decode"] = config["q6_decode"]
    return selectors


def aligned_layout_desc(rows: int, columns: int) -> dict[str, int]:
    empty = {
    ...
    if rows <= 0 or columns <= 0 or columns % Q6_BLOCK_VALUES != 0:
        return empty
    n_blocks_per_row = columns // Q6_BLOCK_VALUES
    n_blocks = rows * n_blocks_per_row
    d_bytes = n_blocks * 2
    d_pad = (ALIGNMENT_BYTES - (d_bytes % ALIGNMENT_BYTES)) % ALIGNMENT_BYTES
    scale_offset = d_bytes + d_pad
    scale_bytes = n_blocks * 16
    scale_pad = (ALIGNMENT_BYTES - (scale_bytes % ALIGNMENT_BYTES)) % ALIGNMENT_BYTES
    ql_offset = scale_offset + scale_bytes + scale_pad
    ql_bytes = n_blocks * 128
    qh_offset = ql_offset + ql_bytes
    qh_bytes = n_blocks * 64
    total_bytes = qh_offset + qh_bytes
    gguf_bytes = n_blocks * Q6_BLOCK_BYTES
    return {
        "version": LAYOUT_VERSION,
        "rows": rows,
        "columns": columns,
        ...
        "byte_neutral": d_pad == 0 and scale_pad == 0 and total_bytes == gguf_bytes,
        "pad_bytes": d_pad + scale_pad,
    }


def pack_q6_aligned(gguf: bytes, rows: int, columns: int) -> bytes:
    desc = aligned_layout_desc(rows, columns)
    if desc["gguf_bytes"] != len(gguf):
        raise AdmissionError("gguf payload size does not match Q6 geometry")
    soa = bytearray(desc["total_bytes"])
    for block in range(desc["n_blocks"]):
        src = block * Q6_BLOCK_BYTES
        soa[block * 2 : block * 2 + 2] = gguf[src + 208 : src + 210]
        so = desc["scale_offset"] + block * 16
        soa[so : so + 16] = gguf[src + 192 : src + 208]
        qo = desc["ql_offset"] + block * 128
        soa[qo : qo + 128] = gguf[src : src + 128]
        ho = desc["qh_offset"] + block * 64
        soa[ho : ho + 64] = gguf[src + 128 : src + 192]
    return bytes(soa)


def unpack_q6_aligned(soa: bytes, rows: int, columns: int) -> bytes:
    desc = aligned_layout_desc(rows, columns)
    if desc["total_bytes"] != len(soa):
        raise AdmissionError("SoA payload size does not match Q6 geometry")
    gguf = bytearray(desc["gguf_bytes"])
    for block in range(desc["n_blocks"]):
        dst = block * Q6_BLOCK_BYTES
        qo = desc["ql_offset"] + block * 128
        gguf[dst : dst + 128] = soa[qo : qo + 128]
        ho = desc["qh_offset"] + block * 64
        gguf[dst + 128 : dst + 192] = soa[ho : ho + 64]
        so = desc["scale_offset"] + block * 16
        gguf[dst + 192 : dst + 208] = soa[so : so + 16]
        gguf[dst + 208 : dst + 210] = soa[block * 2 : block * 2 + 2]
    return bytes(gguf)
...
def parity_catalog() -> list[dict[str, Any]]:
    return [
        {
            "id": "Q6_aligned_M1_N1_K256_inverse_mmv",
            "family": "Q6_K",
        ...
        {
            "id": "Q6_aligned_M3_N1_K256_pad_inverse_mmv",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 3,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_M17_N1_K256_inverse_mmv",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_M1_N1_K5120_inverse_mmv",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 1,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_M17_N1_K5120_inverse_mmv",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_attn_out_M5120_K6144",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 5120,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_logits_M1024_K5120_sampled",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 1024,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_logits_geom_byte_neutral",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 248320,
            "N": 1,
            "K": 5120,
            "host_geometry_only": True,
            "expect_fallback": False,
        },
        {
            "id": "Q6_aligned_integer_override_M17_K5120",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
        ...
        {
            "id": "Q6_aligned_zero_act_M17_K256",
            "family": "Q6_K",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
...
def family_plan(phase: str, mode: str) -> dict[str, Any]:
    ...
    default_candidates = 2 if phase in {"q6", "d128", "d2048", "prefill-guard"} else 1
    return {
        "phase": phase,
        "mode": mode,
        "family": "decode-q6" if phase == "q6" else phase,
        "control": CONTROL_ID,
...
def _parse_parity_cases(stdout: str) -> list[dict[str, Any]]:
    ...
    for line in (stdout or "").splitlines():
        stripped = line.strip()
        if stripped.startswith("QW38_OPT104_CASE="):
            try:
...
def parse_launch_counts(stdout: str) -> dict[str, int]:
    match = re.search(r"mixer_calls=(\d+)", stdout)
    launches = int(match.group(1)) if match else -1
    attn = re.search(r"attention_input_output_groups=(\d+)", stdout)
    attn_n = int(attn.group(1)) if attn else -1
    return {"q6_launches": launches, "attn_outputs": attn_n}


def dispatch_ok(config: Mapping[str, Any], stdout: str) -> bool:
    counts = parse_launch_counts(stdout)
    layout = re.search(r"q6_device_layout=(\w+)", stdout)
    got_layout = layout.group(1) if layout else ""
    want = int(config["expected_q6_launches"])
    return (
        counts["q6_launches"] == want
        and counts["attn_outputs"] == 16
        and got_layout == config["q6_device_layout"]
    )
...
def run_engine(
    ...
    *,
    ...
    if (
        ...
        raise AdmissionError("graph was not recaptured after selecting layout")
...
def performance_pass_for(
    *,
    ...
    component_ok = (
        ...
        and float(stats.get("mean_diff_ms", 0.0)) >= MIN_Q6_SAVING_MS
    )
    if not component_ok:
        return False
    if require_full_e2e:
...
def decide_independent_verdicts(
    *,
    ...
    if len(kept) > 1:
        raise AdmissionError("at most one Q6 device layout may be kept")
    return {
        ...
        "control_layout": CONTROL_ID,
        "shipping_layout": selected if not shipping_unchanged else CONTROL_ID,
        "opt074_coverage_unadmitted_blocker": False,
        "winners": winners,
...
def write_report(payload: Mapping[str, Any]) -> None:
    ...
    q6 = payload.get("q6") or {}
    component = q6.get("component") or {}
    engine = q6.get("engine") or {}
    ...
    nested_parity = (
        parity.get("parity") if isinstance(parity.get("parity"), dict) else {}
    )
    parity_source = nested_parity.get("source") or parity.get("source")
    parity_host_ok = (
        nested_parity.get("host_ok")
        if "host_ok" in nested_parity
        else parity.get("host_ok")
    )
    parity_catalog = nested_parity.get("catalog_count") or parity.get("catalog_count")
    lines = []
    for config in CONFIGS:
    ...
    text = f"""# OPT-104 — Aligned Q6_K SoA device layout

Status: **{payload.get("status") or "pending"}**. Authority llama.cpp
`{LLAMA_REV}`, GGUF SHA-256 `{GGUF_SHA}`. Exactly two device layouts:
...
shipping_layout={payload.get("shipping_layout", CONTROL_ID)}.
...
source={parity_source}; host_ok={parity_host_ok};
...
Combined Q6 launches stay {Q6_LAUNCHES} (16 attention outputs + logits).
Screen ≥{MIN_Q6_SAVING_MS} ms/token complete Q6 decode.
...
Pin rewrite layout={pins.get("layout")}.
"""
    REPORT.write_text(text, encoding="utf-8")


def write_rejection(payload: Mapping[str, Any]) -> None:
    EVIDENCE.mkdir(parents=True, exist_ok=True)
    q6 = payload.get("q6") or {}
    component = q6.get("component") or {}
    (EVIDENCE / "REJECTION.md").write_text(
        f"""# OPT-104 rejection — retain raw GGUF Q6_K
...
Q6 control {_fmt(component.get("control_mean_ms"))} ms vs candidate
{_fmt(component.get("candidate_mean_ms"))} ms; mean_diff
{_fmt(component.get("mean_diff_ms"), 4)} ms; CI
{_fmt(component.get("ci95_low"), 4)} .. {_fmt(component.get("ci95_high"), 4)}.
...
def merge_fixture(
    ...
    for cid, row in incoming.items():
        ...
        elif phase in {"q6", "d128", "d2048", "prefill-guard"}:
    ...
    q6 = results.get("q6") or {}
    stats = {
        "positive": (q6.get("component") or {}).get("positive"),
        "mean_diff_ms": (q6.get("component") or {}).get("mean_diff_ms"),
    }
    this_sitting = bool((q6.get("component") or {}).get("this_sitting"))
    dispatch_ok_flag = bool((q6.get("component") or {}).get("dispatch_ok"))
    q6_ok = (
        bool(stats.get("positive"))
        and float(stats.get("mean_diff_ms") or 0.0) >= MIN_Q6_SAVING_MS
        and dispatch_ok_flag
        and q6.get("mode") == "acceptance"
        and this_sitting
    )
    late_perf = q6_ok and d128_ok and d2048_ok and prefill_ok
    performance_by_id = {
    ...
    results["control_layout"] = CONTROL_ID
    results["shipping_layout"] = decided["shipping_layout"]
    candidate_kept = bool(
    ...
    if results["shipping_unchanged"] or not candidate_kept:
        results["shipping_layout"] = CONTROL_ID
...
def run_q6_phase(
    *,
    ...
    observed = planned_observation(plan, keep=False)
    admission = admit_counts("q6", mode, observed)
    quality_by_id = {
        cid: {"model_quality_pass": row.get("model_quality_pass")}
    ...
    return {
        ...
        **decided,
        ...
        "this_sitting": this_sitting,
        "measurement_utc": utc_now(),
        "status": "q6_screened" if mode != "acceptance" else "q6_acceptance",
    }
...
def run(
    ...
    *,
    ...
    results.update(
        {
            ...
            "control": CONTROL_ID,
            "control_layout": CONTROL_ID,
            "shipping_layout": CONTROL_ID,
            "q6_decode_path_fixed": "integer_q8_1",
            "q6_launches": Q6_LAUNCHES,
            "min_q6_saving_ms": MIN_Q6_SAVING_MS,
            "opt074_coverage_unadmitted_blocker": False,
            "quality_contract_id": QUALITY_CONTRACT_ID,
            "report_path": "evidence/optimization/opt104-q6-aligned-layout/REPORT.md",
            "replacement_mandatory_on_keep": True,
            "no_extra_persistent_q6": True,
        }
    )
    if phase == "parity":
    ...
    q6_component = (results.get("q6") or {}).get("component") or {}
    q6_keep_ok = bool(
        q6_component.get("positive")
        and float(q6_component.get("mean_diff_ms") or 0.0) >= MIN_Q6_SAVING_MS
        and q6_component.get("dispatch_ok")
        and q6_component.get("this_sitting")
        and (results.get("q6") or {}).get("mode") == "acceptance"
    )
    if phase == "q6" and mode == "acceptance" and not skip_gpu and not q6_keep_ok:
        results["status"] = "measured_reject"
    ...
    dump_json(run_dir / "opt104_q6_aligned.json", results)
        ...
        + json.dumps(
            {
                ...
                "shipping_layout": results.get("shipping_layout"),
                "opt074_coverage_unadmitted_blocker": False,
                "keep": False,
