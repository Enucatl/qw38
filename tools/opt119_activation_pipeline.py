"""OPT-119 keep/reject prefill activation pipeline fusion vs post113 ffn_only."""
...
CONTRACT = ROOT / "pins/opt119_activation_pipeline_contract.json"
ITERATION = ROOT / "pins/opt119_iteration_contract.json"
FIXTURE = ROOT / "fixtures/opt119_activation_pipeline.json"
REPORT = ROOT / "evidence/optimization/opt119-activation-pipeline/REPORT.md"
...
NATIVE = "build/qw38-cuda-opt119-activation-pipeline-test"
QUALITY_NATIVE = "build/qw38-cuda-opt058-quality-baseline-test"
NLL_BUNDLE = "pins/production_quality_v2_nll.bundle"
MODEL = "models/Qwen3.8-27B-Q4_K_M.gguf"
RESULT_PREFIX = "QW38_OPT119_ACTIVATION_PIPELINE_RESULT="
COUNTS_PREFIX = "QW38_OPT119_NATIVE_COUNTS="
PHASES = (
    "preflight",
    "inventory",
    "greedy-parity",
...
REQUIRED_FIXTURE_KEYS = (
    ...
    "state_memory",
    "performance",
    "shipping_mixer_norm_q8_fusion",
    "shipping_ffn_norm_q8_fusion",
    "production_kept",
    "verdict",
    "report_path",
)
...
class PipelineError(RuntimeError):
    """OPT-119 measurement or policy failure."""


def load_contract() -> dict[str, Any]:
    payload = load_json(CONTRACT)
    if payload.get("task") != "OPT-119":
        raise PipelineError("activation-pipeline contract task mismatch")
    return payload


def load_iteration() -> dict[str, Any]:
    payload = load_json(ITERATION)
    validate_future_keep_policy("OPT-119", payload)
    return payload
...
def family_plan(mode: str, family: str) -> str:
    ...
    return (
        f"task=OPT-119 mode={mode} phase={family} "
...
def parse_prefixed(text: str, prefix: str) -> dict[str, Any]:
    ...
    if not records:
        raise PipelineError(f"missing {prefix} record")
    return records[-1]
...
def run_quality_native(run_dir: Path, config_id: str) -> dict[str, Any]:
    ...
    if not restored:
        raise PipelineError("--quality restored packed or r2 defaults")
    return {
        "id": config_id,
        "cases": cases,
...
def evaluate_keep(run_dir: Path) -> dict[str, Any]:
    ...
    same = load_sidecar(run_dir, "greedy-parity.json") or {}
    api = load_sidecar(run_dir, "api.json") or {}
    tails = load_sidecar(run_dir, "tails.json") or {}
    ...
    tails_ok = bool(tails.get("ok"))
    quality_ok = bool(quality.get("ok") and quality.get("candidate_nll_measured"))
    state_ok = bool(state.get("ok"))
    correctness_ok = inventory_ok and same_ok and bool(api.get("ok")) and tails_ok
    reasons: list[str] = []
    if not inventory_ok:
    ...
    performance_ok = True
    for name, row, target, prefill in (
        ("p4096", p4096, True, True),
        ("d128", d128, False, False),
        ("d2048", d2048, False, False),
    ):
        ...
        aa = float(noise.get(name, noise["p4096"]))
        if target:
            better = (
                ci_lower is not None and float(ci_lower) > 1.0 and abs(geo - 1.0) > aa
            )
            p95_ok = True if prefill and p95_ratio == 0.0 else (p95_ratio <= 1.05)
    ...
    if keep:
        ...
        verdict = "reject"
    p4096_delta = 0.0
    if performance.get("p4096", {}).get("candidate_tok_s") and performance.get(
        "p4096", {}
    ).get("parent_tok_s"):
        p4096_delta = float(performance["p4096"]["candidate_tok_s"]) - float(
            performance["p4096"]["parent_tok_s"]
        )
    d128_delta = 0.0
    ...
    return {
        ...
        "verdict": verdict,
        "shipping_mixer_norm_q8_fusion": keep,
        "shipping_ffn_norm_q8_fusion": keep,
        "reasons": reasons,
        "performance": performance,
        "p4096_tok_s_delta": p4096_delta,
        "d128_tok_s_delta": d128_delta if keep else 0.0,
        "d128_tok_s_delta_measured": d128_delta,
...
def write_report(result: Mapping[str, Any]) -> None:
    ...
    tails = result.get("tails") or {}
    state = result.get("state_memory") or {}
    lines = [
        "# OPT-119 — Reduce activation traffic across pipeline boundaries",
        "",
        f"Status: **{verdict}**. Mixer Q8 pin "
        f"`{result.get('shipping_mixer_norm_q8_fusion')}`. "
        f"FFN Q8 pin `{result.get('shipping_ffn_norm_q8_fusion')}`. "
        "Hardware executed on RTX 5090.",
        "",
        "Parent is authenticated post113_selected `ffn_only` after OPT-117 "
        "rejected graphs and OPT-118 kept lazy/overlap. Primary target is "
        "**prefill (P4096)**; D128/D2048 are non-target decode guards. Two "
        "slices: mixer RMSNorm→MMQ Q8_0 and FFN residual+norm→MMQ Q4K Q8. "
        "BF16 rounding is retained in registers. Q8_1 and Q8Block are not "
        "reused as MMQ layouts. Keep requires OPT-115 complete-request gates.",
        "",
        "## Inventory",
        "",
        "Standalone `quantize_mmq_q8_1` after mixer/FFN RMSNorm rereads the "
        "BF16 activation. Fusion writes MMQ Q8 in the producer epilogue and "
        "drops that reread. Last-row BF16 is retained for capture.",
        "",
        "| Slice | encoding | parent quantize launches | candidate fused launches | BF16 reread B removed |",
        "|---|---|---:|---:|---:|",
        f"| mixer RMSNorm→Q8 | `{inventory.get('mixer_encoding')}` | "
        f"{inventory.get('parent_mixer_quantize_launches')} | "
        f"{inventory.get('candidate_mixer_norm_q8_launches')} | "
        f"{inventory.get('mixer_bf16_reread_bytes_removed')} |",
        f"| FFN residual+norm→Q8 | `{inventory.get('ffn_encoding')}` | "
        f"{inventory.get('parent_ffn_quantize_launches')} | "
        f"{inventory.get('candidate_ffn_norm_q8_launches')} | "
        f"{inventory.get('ffn_bf16_reread_bytes_removed')} |",
        "",
        f"Net activation bytes removed `{inventory.get('net_activation_bytes_removed')}`. "
        "Weight sweeps are unchanged.",
        "",
        "## Same-math, tails, quality, state",
        "",
        f"greedy-parity exact=`{same.get('exact')}` tokens "
        f"`{same.get('parent_token')}`/`{same.get('candidate_token')}`.",
        f"P2048 / 5120 tail / 8192 long-prompt ok=`{tails.get('ok')}`.",
        ...
        "p95 ratio | gate |",
        "|---|---:|---:|---:|---:|---:|---|",
        _row("P4096 (target)", performance.get("p4096") or {}),
        _row("D128 (guard)", performance.get("d128") or {}),
        _row("D2048 (guard)", performance.get("d2048") or {}),
        "",
        f"P4096 tok/s delta vs parent `{result.get('p4096_tok_s_delta')}`. "
        f"D128 sitting delta `{result.get('d128_tok_s_delta')}`. "
        f"Keep={keep}. Reasons: "
        f"{', '.join(result.get('reasons') or []) or 'none'}.",
        "",
        (
            "Production dispatch ships mixer RMSNorm→MMQ Q8_0 and FFN "
            "residual+norm→MMQ Q4K Q8 fusion. Diagnostic kernel times cannot "
            "admit a keep."
            if keep
            else "Rejected production dispatch retains the unfused RMSNorm then "
            "`quantize_mmq_q8_1` sequence. Diagnostic kernel times cannot admit a keep."
        ),
        "",
    ]
...
def run_report(run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "shipping_mixer_norm_q8_fusion": decision["shipping_mixer_norm_q8_fusion"],
        "shipping_ffn_norm_q8_fusion": decision["shipping_ffn_norm_q8_fusion"],
        ...
        "independent_verdicts": {
            ...
            "tails_pass": decision["tails_pass"],
        ...
        "p4096_tok_s_delta": decision["p4096_tok_s_delta"],
        "d128_tok_s_delta": decision["d128_tok_s_delta"],
        "d128_tok_s_delta_measured": decision["d128_tok_s_delta_measured"],
    ...
    for key in REQUIRED_FIXTURE_KEYS:
        if key not in payload:
            raise PipelineError(f"fixture missing {key}")
    dump_json(FIXTURE, payload)
    write_report(payload)
    header = ROOT / "cuda/full_scheduler.h"
    text = header.read_text(encoding="utf-8")
    pin = "true" if decision["production_kept"] else "false"
    text = (
        text.replace(
            "constexpr bool kSelectedMixerNormQ8Fusion = false;",
            f"constexpr bool kSelectedMixerNormQ8Fusion = {pin};",
        )
        .replace(
        ...
        .replace(
            "constexpr bool kSelectedFfnNormQ8Fusion = false;",
            f"constexpr bool kSelectedFfnNormQ8Fusion = {pin};",
        )
        .replace(
    ...
    contract["selected_mixer_norm_q8_fusion"] = decision["production_kept"]
    contract["selected_ffn_norm_q8_fusion"] = decision["production_kept"]
...
def run_phase(phase: str, run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    if phase == "tails":
        return run_named_native(
            run_dir, mode, "tails", ["--execution-graphs", "ffn_only"]
        )
    if phase == "quality":
