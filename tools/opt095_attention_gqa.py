"""OPT-095 grouped-KV decode attention admission versus warp_query."""
...
CONTRACT = ROOT / "pins/opt095_attention_gqa_contract.json"
ITERATION = ROOT / "pins/opt095_iteration_contract.json"
FIXTURE = ROOT / "fixtures/opt095_attention_gqa.json"
REPORT = ROOT / "evidence/optimization/opt095-attention-gqa/REPORT.md"
EVIDENCE = ROOT / "evidence/optimization/opt095-attention-gqa"
OPT090_FIXTURE = ROOT / "fixtures/opt090_decode_attribution.json"
NATIVE = "build/qw38-cuda-opt095-attention-gqa-test"
...
CONFIGS: tuple[dict[str, Any], ...] = (
    {
        "id": CONTROL_ID,
        "decode_attention_gqa": "warp_query",
        "role": "control",
    ...
    {
        "id": CANDIDATE_ID,
        "decode_attention_gqa": "warp_query_gqa6",
        "role": "candidate",
        "expected_launch": "warp_query_gqa6_decode_attention",
        "expected_prep_launches": 0,
        "expected_grid_x": 4,
        "expected_block_y": 6,
    },
...
def apply_production_pin(path_id: str) -> None:
    ...
    updated = re.sub(
        r'kSelectedDecodeAttentionGqaPath\[\] = "[^"]+"',
        f'kSelectedDecodeAttentionGqaPath[] = "{path_id}"',
        text,
        count=1,
    )
    if updated == text:
        raise AdmissionError("failed to rewrite the decode attention GQA pin")
...
def attention_gqa_eligible(opt090: Mapping[str, Any]) -> dict[str, Any]:
    ...
    repeated_kv = int(contract["gqa_group"]) == 6
    sink_ok = attention_ms is not None and attention_ms >= MIN_SAVING_MS
    eligible = bool(
        sink_ok
        and repeated_kv
        and attention_row is not None
        and attention_ms is not None
    )
    return {
        "eligible": eligible,
        "verdict": "proceed" if eligible else "no_go_no_measured_sink",
        "reason": (
            "repeated_kv_and_attention_sink"
            if eligible
            else "missing_repeated_kv_or_attention_sink"
        ),
        "attention_ms_per_token": attention_ms,
        "attention_core_ranked": attention_row is not None,
        "repeated_kv_group": 6 if repeated_kv else None,
...
def require_eligible(results: Mapping[str, Any]) -> None:
    ...
    raise AdmissionError(
        "OPT-095 blocked: "
        + str(gate.get("verdict") or "no_go_no_measured_sink")
        + f" ({gate.get('reason')})"
    )
...
def family_plan(phase: str, mode: str) -> dict[str, Any]:
    ...
    return {
        "phase": phase,
        "mode": mode,
        "family": str(workload.get("family", "decode-attention-gqa")),
        "control": CONTROL_ID,
...
def replay_command(
    ...
    args = [
        ...
        str(config["decode_attention_gqa"]),
        "--decode-position",
...
def dispatch_matches(observed: Mapping[str, Any], config: Mapping[str, Any]) -> bool:
    if not observed:
        return False
    if str(observed.get("path")) != str(config["decode_attention_gqa"]):
        return False
...
def decide_no_go_verdicts() -> dict[str, Any]:
    ...
    return {
        ...
        "shipping_decode_attention_gqa": CONTROL_ID,
        "production_kept": True,
        "winners": [],
...
def decide_independent_verdicts(
    *,
    ...
    return {
        ...
        "shipping_decode_attention_gqa": selected,
...
def run_native_correctness(
    *, runner: NativeRunner, run_dir: Path, mode: str
    ...
    by_ident = {
        CONTROL_ID: {"kernel_parity_pass": "pass=true" in completed.stdout},
        CANDIDATE_ID: {"kernel_parity_pass": "pass=true" in completed.stdout},
    }
    return {
...
def run_engine(
    ...
    *,
    ...
    command = [
        ...
        "graph",
        "--decode-attention-gqa",
        str(candidate["decode_attention_gqa"]),
    ]
...
def write_report(payload: Mapping[str, Any]) -> None:
    ...
    text = f"""# OPT-095 — Share decode KV across six query heads
...
query heads x eight output dimensions. Nonfinite={numeric.get("nonfinite")}.
...
Shipping decode attention GQA stays `{payload.get("shipping_decode_attention_gqa", "warp_query")}`.
"""
...
def run_eligibility_phase(*, run_dir: Path) -> dict[str, Any]:
    ...
    gate = attention_gqa_eligible(load_json(OPT090_FIXTURE))
    payload = {
        "schema_version": 1,
        "task": "OPT-095",
        "phase": "eligibility",
...
def run_phase(
    ...
    *,
    ...
    if phase == "prefill-guard":
        return run_prefill_guard_phase(run_dir=run_dir)
    if phase == "parity":
    ...
    configs = [config_by_id(CONTROL_ID), config_by_id(CANDIDATE_ID)]
    quality = opt073_quality()
    if skip_gpu:
        raise AdmissionError("GPU sitting required for OPT-095 performance phases")
    native_runner = runner or default_native_runner
    ...
    candidate_cfg = config_by_id(CANDIDATE_ID)
    try:
        engine = (
            run_engine(
                plan, runner=native_runner, run_dir=run_dir, candidate=candidate_cfg
            )
            or {}
        )
...
def run(mode: str, phase: str, run_dir: Path) -> dict[str, Any]:
    ...
    results.update(
        {
            ...
            "shipping_decode_attention_gqa": CONTROL_ID,
            "report_path": "evidence/optimization/opt095-attention-gqa/REPORT.md",
        }
    )
    ...
    if not eligibility.get("eligible"):
        ...
        results["independent_verdicts"] = no_go["independent_verdicts"]
        write_report(results)
        dump_json(FIXTURE, results)
        dump_json(run_dir / "opt095_attention_gqa.json", results)
        print(
            "QW38_OPT095_ATTENTION_GQA_RESULT="
            + json.dumps(
                {
                    "task": "OPT-095",
                    "mode": mode,
    ...
    if results.get("production_kept"):
        apply_production_pin(CANDIDATE_ID)
        results["shipping_decode_attention_gqa"] = CANDIDATE_ID
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir = args.run_dir or (
        ROOT / "build" / "optimization-runs" / "OPT-095" / args.mode
    )
    try:
        if args.skip_gpu and args.phase not in {
            ...
            raise AdmissionError("GPU sitting required for OPT-095 phases")
