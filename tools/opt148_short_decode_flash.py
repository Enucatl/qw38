"""OPT-148 flash-style vector decode attention hypothesis screen."""
...
CONTRACT = ROOT / "pins/opt148_short_decode_flash_contract.json"
ITERATION = ROOT / "pins/opt148_iteration_contract.json"
FIXTURE = ROOT / "fixtures/opt148_short_decode_flash.json"
REPORT = ROOT / "evidence/optimization/opt148-short-decode-flash/REPORT.md"
...
NATIVE = "build/qw38-cuda-opt148-short-decode-flash-test"
PROBE = "build/qw38-cuda-optimization-engine-probe"
NLL_BUNDLE = "pins/production_quality_v2_nll.bundle"
MODEL = "models/Qwen3.8-27B-Q4_K_M.gguf"
RESULT_PREFIX = "QW38_OPT148_SHORT_DECODE_FLASH_RESULT="
COUNTS_PREFIX = "QW38_OPT148_NATIVE_COUNTS="
PHASES = (
    "correctness",
    "screen",
    "quality",
...
CANDIDATE = "decode_attention_flash_vec_v1"
CANDIDATE_LAUNCH = "flash_vec_decode_attention"
CONTROL = "vec128_online"
...
LENGTH_RE = re.compile(
    r"^(length|graph_eager|smoke) tokens=(\d+) start=0 position=(\d+) "
    r"control_path=(\S+) candidate_path=(\S+) launch=(\S*) "
    r"max_abs=([0-9.eE+-]+) fp64_abs=([0-9.eE+-]+) nonfinite=(\d+) "
    r"equal=(true|false) finite=(true|false).*n_parts=(\d+)",
    re.MULTILINE,
)


class ShortDecodeFlashError(RuntimeError):
    """OPT-148 measurement or policy failure."""


def load_contract() -> dict[str, Any]:
    payload = load_json(CONTRACT)
    if payload.get("task") != "OPT-148":
        raise ShortDecodeFlashError("short-decode flash contract task mismatch")
...
def load_iteration() -> dict[str, Any]:
    ...
    validate_future_keep_policy("OPT-148", payload)
    return payload
...
def family_plan(mode: str, family: str) -> str:
    ...
    return (
        f"task=OPT-148 mode={mode} phase={key} "
...
def production_pins_parent() -> bool:
    ...
    return (
        ...
        and "kSelectedDecodeAttentionFlashVec = false" in decode
    )


def production_pin() -> str:
    match = re.search(
        r"kSelectedDecodeAttentionFlashVec = (true|false)",
        PIN_PATH.read_text(encoding="utf-8"),
    )
    if match is None:
        raise ShortDecodeFlashError("missing kSelectedDecodeAttentionFlashVec")
    return CANDIDATE if match.group(1) == "true" else SHIPPING_DECODE


def apply_production_pin(path: str) -> None:
    if path not in {SHIPPING_DECODE, CANDIDATE}:
        raise ShortDecodeFlashError(f"illegal decode pin {path}")
    enabled = "true" if path == CANDIDATE else "false"
    text = PIN_PATH.read_text(encoding="utf-8")
    updated = re.sub(
        r"kSelectedDecodeAttentionFlashVec = (true|false)",
        f"kSelectedDecodeAttentionFlashVec = {enabled}",
        text,
        count=1,
    )
...
def parse_correctness(stdout: str) -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "control_path": match.group(4),
                "candidate_path": match.group(5),
                "launch": match.group(6),
                "max_abs": float(match.group(7)),
                "fp64_abs": float(match.group(8)),
                "nonfinite": int(match.group(9)),
                "equal": match.group(10) == "true",
                "finite": match.group(11) == "true",
                "n_parts": int(match.group(12)),
            }
        )
    length_rows = [row for row in rows if row["label"] == "length"]
    positions = [row["position"] for row in length_rows]
    d128_ok = all(
        row["candidate_path"] == D128_KERNEL
        for row in length_rows
        if row["position"] < CROSSOVER
    )
    mma_ok = all(
    ...
    flash_ok = all(
        ...
        for row in length_rows
        if CROSSOVER <= row["position"] <= 4096
    )
    ...
    return {
        ...
        "equal": equal,
        "finite": finite,
        "d128_warp_query": d128_ok,
        "opt137_mma": mma_ok,
        "flash_window": flash_ok,
...
def run_correctness(run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    ok = (
        ...
        and parsed["equal"]
        and parsed["finite"]
        and parsed["d128_warp_query"]
        and parsed["opt137_mma"]
        and parsed["flash_window"]
...
def run_engine_ab(
    ...
    *,
    ...
    if decode_attention:
        command.extend(["--decode-attention", CANDIDATE])
    else:
        command.extend(
            [
    ...
    if completed.returncode != 0:
        raise ShortDecodeFlashError(f"{name} engine probe failed: {stdout[-2000:]}")
    if "override_before_capture_applied=true" not in stdout:
        raise ShortDecodeFlashError(f"{name} graph capture override was not applied")
    parsed_pairs = parse_engine_pairs(stdout)
    if len(parsed_pairs) != pairs:
        raise ShortDecodeFlashError(
            f"{name} engine pairs {len(parsed_pairs)} != {pairs}"
        )
    return {
        "name": name,
        "pairs": parsed_pairs,
...
def run_screen(run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    if native.get("pass"):
        ...
        except ShortDecodeFlashError as exc:
...
def run_quality_native(run_dir: Path, config_id: str) -> dict[str, Any]:
    ...
    if not restored:
        raise ShortDecodeFlashError("--quality restored packed or r2 defaults")
    if not cases:
        raise ShortDecodeFlashError(f"OPT-058 produced no NLL cases for {config_id}")
    return {
        "id": config_id,
        "cases": cases,
        "restored_packed_or_r2": False,
...
def run_state_memory(run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    checkpoint_path = str(run_dir / "opt148-checkpoint.bin")
...
def pair_identity(metric: str, *, prefix: int, eval_count: int) -> dict[str, Any]:
    return {
        ...
        "input_trajectory": "opt148_short_decode_flash",
    }
...
def run_performance(run_dir: Path, mode: str) -> dict[str, Any]:
    ...
    probes: dict[str, Any] = {}
    p4096 = run_engine_ab(run_dir, name="p4096", prompt=4096, decode_attention=False)
    probes["p4096"] = p4096
    records.extend(
...
def evaluate_keep(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "shipping_prompt_attention": SHIPPING_PROMPT,
        "shipping_decode_attention": CANDIDATE if keep else SHIPPING_DECODE,
        "reasons": reasons,
...
def maybe_flip_production_pins(keep: bool) -> None:
    if keep:
        ...
        return
    if production_pin() != SHIPPING_DECODE:
        apply_production_pin(SHIPPING_DECODE)
...
def write_report(result: Mapping[str, Any], run_dir: Path) -> None:
    ...
    pin_line = (
        ...
        if kept
        else "Parent retained; shipping decode pin stays `hybrid_crossover`."
    )
    lines = [
        "# OPT-148 — Short-decode flash-style vector attention screen",
        "",
        f"Status: **{verdict}**. Shipping decode attention `{shipping}`. {pin_line}",
        "",
        f"Parent `{PARENT_STACK}` with kept OPT-147 prompt `{SHIPPING_PROMPT}`. "
        f"Parent D2048 path is `{CONTROL}` (`hybrid_crossover` window "
        f"[1024,4096], n_parts=16). Candidate `{CANDIDATE}` is a Quartz-owned "
        "flash-style vector path using llama `flash_attn_ext_vec` only as the "
        "implementation boundary. OPT-137 MMA remains at positions "
        f"`>= {MMA_THRESHOLD}`. D128 stays `{D128_KERNEL}`. OPT-130 is not revived.",
        "",
        "## Candidate",
        "",
        ...
        "## Correctness",
        "",
        "Compared flash-vec versus `vec128_online` at positions 0, 1023, 1024, "
        "2047, 2048, 4096, 8192 with rotating layers and sampled FP64 refs. "
        "D128 must stay `warp_query`; positions `>=8192` must stay MMA.",
        "",
        ...
        "16 rotating attention layers, prep+combine+conversion included, plus "
        ...
        (
            ...
            "this is an arithmetic change. Screened-out retains parent and "
            "does not stub `candidate_nll_not_measured`."
        ),
        "",
        ...
        f"OPT-137 MMA + kept OPT-147 `{SHIPPING_PROMPT}`; parent decode "
        f"`hybrid_crossover`/`{CONTROL}`; candidate launch `{CANDIDATE_LAUNCH}`; "
        f"shipping after this task `{shipping}`; GGUF `{GGUF_SHA}`; llama revision "
        "`cc83d7b4824f73cfdda4dfbb47ee39804f71b328`.",
        "2. **Coverage** — native complete-family screen at D2048 with rotating "
        "layers plus one short engine pair; correctness over required positions.",
        ...
        "4. **Contradiction register** — flash-style tile organization vs "
        "`vec128_online` is the hypothesis under test, not a freeze blocker. "
        "Mechanism stays unknown without source/SASS evidence.",
        ...
        "[`d2048-pair-reconstruction.json`](d2048-pair-reconstruction.json). "
        "Fixture: `fixtures/opt148_short_decode_flash.json`.",
        "",
        "## Status",
        "",
        f"verdict=`{verdict}` production_kept=`{result.get('production_kept')}` "
        f"blocked=`{verdict == 'blocked'}`.",
        f"Production pin `{production_pin()}`. Prompt pin `{SHIPPING_PROMPT}`.",
        "",
    ]
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def validate_fixture(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "ok": not missing and payload.get("task") == "OPT-148",
        "task": "OPT-148",
        "missing": missing,
    }
...
def run_phase(run_dir: Path, mode: str, phase: str) -> dict[str, Any]:
    ...
    raise ShortDecodeFlashError(f"unknown phase {phase}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {"task": "OPT-148", "phase": args.phase, "ok": False, "error": str(exc)}
            )
        )
        return 1
    print(
