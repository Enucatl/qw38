"""OPT-100 lossless 64-byte-aligned Q8 SoA admission. Production stays raw until keep."""
...
CONTRACT = ROOT / "pins/opt100_q8_aligned_contract.json"
ITERATION = ROOT / "pins/opt100_iteration_contract.json"
FIXTURE = ROOT / "fixtures/opt100_q8_aligned.json"
REPORT = ROOT / "evidence/optimization/opt100-q8-aligned-layout/REPORT.md"
...
NATIVE_PARITY = "build/qw38-cuda-opt100-q8-aligned-test"
...
INPUT_PROJECTION_LAUNCHES = 64
Q8_GROUPING_FIXED = "grouped_r1_w4"
...
CONFIGS: tuple[dict[str, Any], ...] = (
    {
        ...
        "q8_grouping": Q8_GROUPING_FIXED,
        "q8_layout": Q8_LAYOUT_FIXED,
        "q8_decode": "dp4a_q8_1",
        "warps_per_row": 4,
        "role": "control",
        "expected_input_projection_launches": INPUT_PROJECTION_LAUNCHES,
    },
    {
        "id": CANDIDATE_ID,
...
def current_layout_pin() -> str:
    ...
    if match is None:
        raise AdmissionError("missing kSelectedQ8DeviceLayout")
...
def apply_layout_pin(layout_id: str) -> None:
    ...
    updated = re.sub(
        r'kSelectedQ8DeviceLayout\[\] = "[^"]+"',
        f'kSelectedQ8DeviceLayout[] = "{layout_id}"',
        text,
        count=1,
    )
    if updated == text:
        raise AdmissionError("failed to rewrite the Q8 device layout pin")
...
def candidate_selectors(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    selectors["q8_device_layout"] = config["q8_device_layout"]
    return selectors


def aligned_layout_desc(rows: int, columns: int) -> dict[str, int]:
    if rows <= 0 or columns <= 0 or columns % Q8_BLOCK_VALUES != 0:
        return {
    ...
    n_blocks_per_row = columns // Q8_BLOCK_VALUES
    n_blocks = rows * n_blocks_per_row
    scale_bytes = n_blocks * 2
    pad_bytes = (ALIGNMENT_BYTES - (scale_bytes % ALIGNMENT_BYTES)) % ALIGNMENT_BYTES
    code_offset = scale_bytes + pad_bytes
    code_bytes = rows * columns
    total_bytes = code_offset + code_bytes
    gguf_bytes = n_blocks * Q8_BLOCK_BYTES
    return {
        "version": LAYOUT_VERSION,
        "rows": rows,
        "columns": columns,
        ...
        "byte_neutral": pad_bytes == 0 and total_bytes == gguf_bytes,
    }


def pack_q8_aligned(gguf: bytes, rows: int, columns: int) -> bytes:
    desc = aligned_layout_desc(rows, columns)
    if desc["gguf_bytes"] != len(gguf):
        raise AdmissionError("gguf payload size does not match Q8 geometry")
    soa = bytearray(desc["total_bytes"])
    for row in range(rows):
        for kb in range(desc["n_blocks_per_row"]):
            block = row * desc["n_blocks_per_row"] + kb
            src = block * Q8_BLOCK_BYTES
            soa[block * 2 : block * 2 + 2] = gguf[src : src + 2]
            dest = desc["code_offset"] + row * columns + kb * Q8_BLOCK_VALUES
            soa[dest : dest + Q8_BLOCK_VALUES] = gguf[
                src + 2 : src + 2 + Q8_BLOCK_VALUES
            ]
    return bytes(soa)


def unpack_q8_aligned(soa: bytes, rows: int, columns: int) -> bytes:
    desc = aligned_layout_desc(rows, columns)
    if desc["total_bytes"] != len(soa):
        raise AdmissionError("SoA payload size does not match Q8 geometry")
    gguf = bytearray(desc["gguf_bytes"])
    for row in range(rows):
        for kb in range(desc["n_blocks_per_row"]):
            block = row * desc["n_blocks_per_row"] + kb
            dst = block * Q8_BLOCK_BYTES
            gguf[dst : dst + 2] = soa[block * 2 : block * 2 + 2]
            src = desc["code_offset"] + row * columns + kb * Q8_BLOCK_VALUES
            gguf[dst + 2 : dst + 2 + Q8_BLOCK_VALUES] = soa[src : src + Q8_BLOCK_VALUES]
    return bytes(gguf)
...
def parity_catalog() -> list[dict[str, Any]]:
    return [
        {
            "id": "Q8_aligned_M1_N1_K256_inverse_mmv",
            "family": "Q8_0",
        ...
        {
            "id": "Q8_aligned_M3_N1_K256_pad_inverse_mmv",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 3,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M17_N1_K256_inverse_mmv",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M1_N1_K5120_inverse_mmv",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 1,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M3_N1_K5120_inverse_mmv",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 3,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M17_N1_K5120_inverse_mmv",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 17,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M48_N1_K5120_skinny_alpha",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 48,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_M1024_N1_K5120_sampled",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 1024,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_gdn_output_M5120_K6144",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 5120,
            "N": 1,
        ...
        {
            "id": "Q8_aligned_grouped_gdn_input",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "group": "gdn_input",
            "expect_fallback": False,
        ...
        {
            "id": "Q8_aligned_grouped_attn_input",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "group": "attn_input",
            "expect_fallback": False,
        ...
        {
            "id": "Q8_aligned_grouped_empty_ok",
            "family": "Q8_0",
            "candidate": CANDIDATE_ID,
            "M": 0,
            "N": 1,
...
def _parse_parity_cases(stdout: str) -> list[dict[str, Any]]:
    ...
    for line in (stdout or "").splitlines():
        stripped = line.strip()
        if stripped.startswith("QW38_OPT100_CASE="):
            try:
...
def dispatch_ok(config: Mapping[str, Any], stdout: str) -> bool:
    ...
    layout = re.search(r"q8_device_layout=(\w+)", stdout)
    got_layout = layout.group(1) if layout else ""
    return (
        launches == want
        and got_grouping == config["q8_grouping"]
        and got_layout == config["q8_device_layout"]
    )
...
def decide_independent_verdicts(
    *,
    ...
    if len(kept) > 1:
        raise AdmissionError("at most one Q8 device layout may be kept")
    return {
...
def write_report(payload: Mapping[str, Any]) -> None:
    ...
    text = f"""# OPT-100 — Aligned Q8 SoA device layout

Status: **{payload.get("status") or "pending"}**. Authority llama.cpp
`{LLAMA_REV}`, GGUF SHA-256 `{GGUF_SHA}`. Exactly two device layouts:
`raw_gguf` control and `aligned_soa` candidate. Q8 `dp4a_q8_1` / `r1_w4`
and grouping `grouped_r1_w4` stay fixed. No Q8 D2R revival.
...
raw GGUF Q8_0. Replacement is in-place; production mixer shapes are
byte-neutral.
...
Input projection launches stay {INPUT_PROJECTION_LAUNCHES} (grouped_r1_w4).
Screen ≥{MIN_MIXER_SAVING_MS} ms/token complete mixer.
...
D128 throughput_ratio={_fmt(d128_guards.get("throughput_ratio"), 3)}.
D2048 throughput_ratio={_fmt(d2048_guards.get("throughput_ratio"), 3)}.
P4096 throughput_ratio={_fmt(prefill.get("throughput_ratio"), 4)}.
...
def run(
    ...
    *,
    ...
    results.update(
        {
            ...
            "q8_layout_fixed": "r1_w4",
            "q8_grouping_fixed": Q8_GROUPING_FIXED,
            "input_projection_launches": INPUT_PROJECTION_LAUNCHES,
            "min_mixer_saving_ms": MIN_MIXER_SAVING_MS,
            "opt074_coverage_unadmitted_blocker": False,
            "quality_contract_id": QUALITY_CONTRACT_ID,
            "report_path": "evidence/optimization/opt100-q8-aligned-layout/REPORT.md",
            "replacement_mandatory_on_keep": True,
            "no_q8_d2r_revival": True,
        }
    )
    if phase == "parity":
    ...
    dump_json(run_dir / "opt100_q8_aligned.json", results)
