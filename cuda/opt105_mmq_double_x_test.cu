...
constexpr char kPrefix[] = "QW38_OPT105_MMQ_DOUBLE_X_RESULT=";
...
constexpr unsigned int kCandidateI = 64;
constexpr unsigned int kCandidateJ = 128;
constexpr int kRotatingLayers = 64;
constexpr float kMinSavingMs = 5.0F;
...
constexpr Variant kVariants[] = {
    {"i128_j128", kControlI, kControlJ, false, "q4_i128_j128_fma_async_x"},
    {"i64_j128_x2", kCandidateI, kCandidateJ, true, "q4_i64_j128_fma_async_x2"},
};
...
int launch_q4(const std::uint8_t* weights, std::size_t output_rows,
  ...
  qw38::cuda::MmqAsyncXOverrideScope async_scope(true);
  qw38::cuda::MmqSplitXYWaitOverrideScope split_scope(false);
  qw38::cuda::MmqDoubleXOverrideScope double_scope(variant.double_x);
  const cudaError_t error = qw38::cuda::launch_quant_mmq_mma_y_ij(
      ...
      output, variant.quality_i, variant.prompt_tile, nullptr);
...
int check_dispatch(const Variant& variant, bool want_pipeline,
  ...
  const bool ok_ij =
      rec.quality_i == variant.quality_i && rec.prompt_tile == variant.prompt_tile;
  const bool ok_pipe = rec.pipeline == want_pipeline;
  const bool ok_fma = !want_pipeline || (rec.fma && rec.async_y);
  const bool ok_x = rec.async_x == (want_pipeline && true);
  const bool ok_x2 = rec.async_x2 == (want_pipeline && variant.double_x);
  const bool ok_split = rec.split_xy_wait == false;
  const bool ok_path = std::strcmp(rec.path, "fma_async") == 0;
  std::printf(
      "dispatch %s variant=%s kernel=%s aligned=%s pipeline=%s fallback=%s "
      "fma=%s async_y=%s async_x=%s async_x2=%s split_xy_wait=%s path=%s\n",
      label, variant.id, rec.kernel, json_bool(rec.aligned),
      json_bool(rec.pipeline), json_bool(rec.fallback), json_bool(rec.fma),
      json_bool(rec.async_y), json_bool(rec.async_x), json_bool(rec.async_x2),
      json_bool(rec.split_xy_wait), rec.path);
  if (!ok_ij || !ok_pipe || !ok_fma || !ok_x || !ok_x2 || !ok_split ||
      !ok_path) {
    std::fprintf(stderr, "dispatch mismatch for %s %s\n", label, variant.id);
    return 1;
  }
  if (want_pipeline && std::strcmp(rec.kernel, variant.kernel) != 0) {
    std::fprintf(stderr, "kernel id mismatch: got %s want %s\n", rec.kernel,
                 variant.kernel);
    return 1;
  }
  if (!want_pipeline) {
    const char* fallback =
        variant.double_x ? "q4_i64_j128_sync_fallback" : "q4_i128_j128_sync_fallback";
    if (std::strcmp(rec.kernel, fallback) != 0) {
      std::fprintf(stderr, "fallback kernel id mismatch: %s\n", rec.kernel);
      return 1;
    }
  }
  return 0;
...
int run_resource_table() {
  ...
  cudaError_t error = qw38::cuda::mmq_x_pipeline_kernel_attributes(
      &occupancy, &registers, &local_bytes, &shared_bytes);
  const std::size_t extra_x =
      qw38::cuda::mmq_x_pipeline_extra_shared_bytes(kControlI);
  int device = 0;
  cudaDeviceProp prop{};
  ...
  const std::size_t optin =
      prop_error == cudaSuccess
          ...
          : 0;
  std::printf(
      "resource variant=i128_j128 occupancy=%d regs=%d local=%zu shared=%zu "
      "extra_x=%zu device_optin=%zu query=%s\n",
      occupancy, registers, local_bytes, shared_bytes, extra_x, optin,
  ...
  if (shared_bytes > optin) {
    std::printf("resource_nogo control shared=%zu optin=%zu\n", shared_bytes,
                optin);
    return 1;
  }
  ...
  error = qw38::cuda::mmq_x2_pipeline_kernel_attributes(
      &cand_occ, &cand_regs, &cand_local, &cand_shared);
  const std::size_t extra_x2 =
      qw38::cuda::mmq_x2_pipeline_extra_shared_bytes(kCandidateI);
  std::printf(
      "resource variant=i64_j128_x2 occupancy=%d regs=%d local=%zu shared=%zu "
      "extra_x2=%zu extra_x2_exact=%zu device_optin=%zu query=%s\n",
      cand_occ, cand_regs, cand_local, cand_shared, extra_x2,
      2U * static_cast<std::size_t>(kCandidateI) * kQ4KBytes + 16U, optin,
      error == cudaSuccess ? "ok" : cudaGetErrorString(error));
  std::printf(
      "stage_lifetime warmup: cp.async raw X[k0] into slot0 and X[k1] into "
      "slot1 (I*9 16B chunks each) plus Y[k0,h0]; one commit group; wait_all; "
      "unpack slot0 into decoded X; CTA barrier. Slot0 is then free.\n");
  std::printf(
      "stage_lifetime iter_k_load: joined waits issue next Y and raw X[k+2] "
      "into the free slot in one commit group; never reuse a live slot.\n");
  std::printf(
      "stage_lifetime iter_k_compute: wait_all before unpack of the live slot "
      "(X[k+1]); decoded-X/MMA fragments, FMA association and epilogue are "
      "unchanged. OPT-097 split wait is not used.\n");
  std::printf(
      "stage_lifetime drain: last kb0 issues no X[k+2]; no final unpack; Y "
      "drain matches the existing two-stage Y pipeline. Tails/misalignment "
      "keep the synchronous reference.\n");
  ...
  if (registers != 0) {
    std::printf("resource_regs control=%d candidate=%d\n", registers, cand_regs);
  }
  return 0;
}
...
int run_variant(CaseBuffers* buffers, const Variant& variant, const char* label,
  ...
  const bool aligned = buffers->output_rows % variant.quality_i == 0 &&
                       buffers->prompt_rows % variant.prompt_tile == 0;
...
int run_graph_eager() {
  ...
  constexpr std::size_t kN = 128;
  CaseBuffers buffers;
  int rc = prepare_case("graph_eager", kM, kK, kN, true, 0xC0D07u, &buffers);
  if (rc != 0) return rc;
  rc = run_variant(&buffers, kVariants[kCandidate], "graph_eager_eager",
                   &buffers.candidate);
  if (rc != 0) {
    free_case(&buffers);
    return rc;
  ...
  if (error != cudaSuccess) {
    free_case(&buffers);
    return fail_cuda("graph stream", error);
  }
  qw38::cuda::MmqAsyncXOverrideScope async_scope(true);
  qw38::cuda::MmqSplitXYWaitOverrideScope split_scope(false);
  qw38::cuda::MmqDoubleXOverrideScope double_scope(true);
  error = cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal);
  if (error == cudaSuccess) {
    error = qw38::cuda::launch_quant_mmq_mma_y_ij(
        qw38::cuda::QuantKind::kQ4K, buffers.d_w, kM, kK, buffers.d_y, kN,
        buffers.d_o, kCandidateI, kCandidateJ, stream);
  }
  ...
  if (error != cudaSuccess) {
    ...
    free_case(&buffers);
    return fail_cuda("graph launch", error);
  }
  std::vector<float> graphed(kN * kM, 0.0F);
  error = cudaMemcpy(graphed.data(), buffers.d_o, graphed.size() * sizeof(float),
                     cudaMemcpyDeviceToHost);
  std::size_t mismatch = 0;
  std::size_t nonfinite = 0;
  const bool same =
      error == cudaSuccess &&
      exact_equal(graphed, buffers.candidate, &mismatch, &nonfinite);
  std::printf("graph_eager mismatch=%zu nonfinite=%zu\n", mismatch, nonfinite);
  cudaGraphExecDestroy(exec);
  cudaGraphDestroy(graph);
  cudaStreamDestroy(stream);
  free_case(&buffers);
  if (error != cudaSuccess) return fail_cuda("graph d2h", error);
  return same ? 0 : 1;
}
...
int run_screen(const char* model_path) {
      ...
      for (int sample = 0; sample < 4; ++sample) {
        ...
        if (sample == 0) {
          std::printf(
              ...
              "async_x=%s async_x2=%s kernel=%s\n",
              group.id, kVariants[v].id, ms, json_bool(rec.pipeline),
              json_bool(rec.async_x), json_bool(rec.async_x2), rec.kernel);
        } else {
          acc += ms;
          ++n;
          std::printf(
              "screen_sample group=%s variant=%s sample=%d ms=%.9g "
              "pipeline=%s async_x=%s async_x2=%s kernel=%s\n",
        ...
        if (rec.async_x != true || rec.async_x2 != kVariants[v].double_x ||
            rec.split_xy_wait ||
            rec.quality_i != kVariants[v].quality_i ||
            rec.prompt_tile != kVariants[v].prompt_tile) {
          std::fprintf(stderr, "screen tile/schedule mismatch for %s kernel=%s\n",
                       kVariants[v].id, rec.kernel);
          cudaFree(rotating);
          cudaEventDestroy(start);
          cudaEventDestroy(stop);
          return 1;
  ...
  const qw38::cuda::ResidentModel* rotating = &model;
  float control_ffn = 0.0F;
  float survivor_ffn = 0.0F;
  auto time_ffn64 = [&](const Variant& variant, float* mean) -> int {
    float acc = 0.0F;
    int n = 0;
    const int layers = static_cast<int>(rotating->layer_count());
    for (int sample = 0; sample < 4; ++sample) {
      float ms = 0.0F;
      error = cudaEventRecord(start, nullptr);
      for (int layer_i = 0; layer_i < kRotatingLayers && error == cudaSuccess;
           ++layer_i) {
        const qw38::cuda::DeviceCommonLayer& one =
            rotating->layer(static_cast<std::size_t>(layer_i % layers)).common;
        error = launch_complete_ffn(one, &workspace, kPrompt, variant);
      }
      if (error == cudaSuccess) error = cudaEventRecord(stop, nullptr);
      if (error == cudaSuccess) error = cudaEventSynchronize(stop);
      if (error == cudaSuccess) error = cudaEventElapsedTime(&ms, start, stop);
      if (error != cudaSuccess) return fail_cuda("complete ffn64", error);
      const qw38::cuda::MmqTileDispatch rec =
          qw38::cuda::last_mmq_tile_dispatch();
      if (sample == 0) {
        std::printf(
            "complete64_warmup variant=%s ms=%.9g last_kernel=%s pipeline=%s "
            "async_x2=%s layers=%d\n",
            variant.id, ms, rec.kernel, json_bool(rec.pipeline),
            json_bool(rec.async_x2), kRotatingLayers);
      } else {
        acc += ms;
        ++n;
        std::printf(
            "complete64_sample variant=%s sample=%d ms=%.9g last_kernel=%s "
            "pipeline=%s async_x2=%s\n",
            variant.id, sample, ms, rec.kernel, json_bool(rec.pipeline),
            json_bool(rec.async_x2));
      }
    }
    *mean = n == 0 ? 0.0F : acc / static_cast<float>(n);
    return 0;
  };
  int rc = time_ffn64(kVariants[kControl], &control_ffn);
  if (rc == 0) rc = time_ffn64(kVariants[kCandidate], &survivor_ffn);
  cudaEventDestroy(start);
  cudaEventDestroy(stop);
  if (rc != 0) return rc;

  const float saving = control_ffn - survivor_ffn;
  const bool keep = gemm_faster && saving >= kMinSavingMs;
  std::printf(
      "screen_winner control_gate_up_ms=%.9g candidate_gate_up_ms=%.9g "
      "control_down_ms=%.9g candidate_down_ms=%.9g control_ffn64_ms=%.9g "
      "survivor_ffn64_ms=%.9g saving_ms=%.9g keep=%s "
      "installed_double_x=%s selected_double_x=%s\n",
      means[0][kControl], means[0][kCandidate], means[1][kControl],
      means[1][kCandidate], control_ffn, survivor_ffn, saving, json_bool(keep),
      json_bool(keep), json_bool(qw38::cuda::selected_mmq_double_x()));
  return 0;
}

void emit_payload(const char* workload, const char* status) {
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-105\",\"workload\":\"%s\","
      "\"tier\":\"%s\",\"status\":\"%s\",\"selected_pipeline\":\"%s\","
      "\"selected_async_x\":%s,\"selected_double_x\":%s,"
      "\"control_tile\":\"i128_j128\",\"candidate_tile\":\"i64_j128_x2\","
      ...
      json_bool(qw38::cuda::selected_mmq_double_x()), kLlamaRev, kGgufSha);
}
