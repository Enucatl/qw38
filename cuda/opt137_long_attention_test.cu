...
constexpr char kPrefix[] = "QW38_OPT137_LONG_ATTENTION_RESULT=";
constexpr char kCounts[] = "QW38_OPT137_NATIVE_COUNTS=";
...
void print_counts(std::size_t warmups, std::size_t samples,
  ...
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"observed_warmups\":%zu,"
...
cudaError_t launch_arm(DeviceCase* device, bool candidate) {
  qw38::cuda::Opt137DenseMmaScope scope(
      candidate ? qw38::cuda::kLegalDecodeAttentionDenseMma
...
int identity_pair(std::size_t position, std::uint32_t seed, bool zero_inputs,
  ...
  if (rc == 0) {
    ...
    mma_launches = qw38::cuda::last_opt137_mma_launches();
  }
  if (rc == 0) {
      ...
      below ? (mma_launches == 0)
            : (mma_launches > 0 && cand_path != nullptr &&
               std::strcmp(cand_path,
                           qw38::cuda::kLegalDecodeAttentionDenseMma) == 0);
...
int run_inspect() {
  ...
  int launch_rc = 0;
  {
    qw38::cuda::Opt137DenseMmaScope scope(
        qw38::cuda::kLegalDecodeAttentionDenseMma);
    occupancy = qw38::cuda::opt137_mma_occupancy(&nsm);
    ...
    grid_x = qw38::cuda::g_opt137_mma_grid_x;
    grid_y = qw38::cuda::g_opt137_mma_grid_y;
    block = qw38::cuda::g_opt137_mma_block;
    device.free_all();
  }
  ...
  const bool parent_8191 = !qw38::cuda::opt137_uses_mma_at(8191);
  bool mma_8192 = false;
  {
    qw38::cuda::Opt137DenseMmaScope scope(
        qw38::cuda::kLegalDecodeAttentionDenseMma);
    mma_8192 = qw38::cuda::opt137_uses_mma_at(8192);
  }
  ...
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"inspect\","
      ...
      "\"ncols1\":1,\"ncols2\":8,\"gqa_ratio\":6,\"dense_bf16\":true,"
      "\"shadow_f16_cache\":false,\"keep\":false}\n",
      kPrefix, json_bool(ok), kLlamaRev, kGgufSha,
      "dense_bf16_tile_f16_mma_decode_v1", parent_crossover, parent_verified,
...
int run_identity() {
  ...
  bool first = true;
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"identity\","
      "\"cases\":[",
      kPrefix);
...
int run_screen(const Options& options) {
  ...
  bool advance = true;
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"screen\","
...
qw38::Status run_request_arm(const qw38::cuda::ResidentModel& model,
  ...
  qw38::cuda::Opt137DenseMmaScope attn(
      candidate ? qw38::cuda::kLegalDecodeAttentionDenseMma
                : qw38::cuda::kLegalDecodeAttentionHybridCrossover);
...
qw38::Status prepare_persistent_arm(const qw38::cuda::ResidentModel& model,
  ...
  if (status.is_ok()) {
    qw38::cuda::Opt137DenseMmaScope parent(
        qw38::cuda::kLegalDecodeAttentionHybridCrossover);
    status = create_graphs(model, &arm->session, &arm->workspace,
                           &arm->parent_graphs);
  }
  if (status.is_ok()) {
    qw38::cuda::Opt137DenseMmaScope candidate(
        qw38::cuda::kLegalDecodeAttentionDenseMma);
    status = create_graphs(model, &arm->session, &arm->workspace,
...
int run_graph_ab(const qw38::cuda::ResidentModel& model, const Options& options) {
  ...
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"graph-ab\","
...
int run_same_math(const qw38::cuda::ResidentModel& model,
  ...
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"same-math\","
...
int run_cancellation(const qw38::cuda::ResidentModel& model,
  ...
  bool all_ok = true;
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\","
...
int run_handoff(const qw38::cuda::ResidentModel& model, const Options& options) {
  ...
  {
    qw38::cuda::Opt137DenseMmaScope cand(
        qw38::cuda::kLegalDecodeAttentionDenseMma);
  ...
  json_escape(status.message(), &message);
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"handoff\","
...
int run_quality_long(const qw38::cuda::ResidentModel& model,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    unsigned int parent_launches = qw38::cuda::last_opt137_mma_launches();
    qw38::cuda::reset_opt137_mma_launches();
    if (status.is_ok()) {
      status = run_request_arm(model, tokens, prefix, outputs, true, &cand);
    }
    const unsigned int cand_launches = qw38::cuda::last_opt137_mma_launches();
    std::string message;
    json_escape(status.message(), &message);
...
int run_mechanism(const qw38::cuda::ResidentModel& model,
  ...
  const unsigned int launches = qw38::cuda::last_opt137_mma_launches();
  const bool ok = status.is_ok() && launches > 0 && cand.topology_recapture == 0;
  std::string message;
  json_escape(status.message(), &message);
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-137\",\"workload\":\"mechanism\","
