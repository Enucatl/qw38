...
constexpr char kPrefix[] = "QW38_OPT066_MMQ_X_PIPELINE_RESULT=";
...
int launch_q4(const std::uint8_t* weights, std::size_t output_rows,
              ...
              std::size_t prompt_rows, float* output, bool async_x) {
  qw38::cuda::clear_mmq_tile_dispatch();
  qw38::cuda::MmqAsyncXOverrideScope scope(async_x);
...
int check_dispatch(const Variant& variant, bool want_pipeline,
  ...
  std::printf(
      ...
      "fma=%s async_y=%s async_x=%s path=%s\n",
      label, variant.id, rec.kernel, json_bool(rec.aligned),
      json_bool(rec.pipeline), json_bool(rec.fallback), json_bool(rec.fma),
      json_bool(rec.async_y), json_bool(rec.async_x), rec.path);
  if (!ok_ij || !ok_pipe || !ok_fma || !ok_x || !ok_path) {
  ...
  if (want_pipeline && variant.async_x &&
    ...
    std::fprintf(stderr, "candidate kernel id mismatch: %s\n", rec.kernel);
    return 1;
  }
  if (want_pipeline && !variant.async_x &&
...
int run_resource_table() {
  ...
  std::printf(
      "stage_lifetime iter_k_load: after unpack the raw slot is free; half0 "
      "issues next Y and raw X[k+1] into that same slot in one commit group "
      "with the Y copies. MMA still consumes decoded X, not the raw slot.\n");
  std::printf(
      "stage_lifetime iter_k_unpack: after both MMA halves, unpack the "
      "prefetched slot into decoded X; CTA barrier before the next kb0\n");
  std::printf(
      "stage_lifetime iter_k_compute: MMA both K128 halves on decoded X and "
      "the active Y stage; decoded X stays live across both halves\n");
  std::printf(
      "stage_lifetime next_ready: next raw X is waited with Y at the end of "
      "half0 (wait_all on the combined group) and unpacked after half1 MMA\n");
  std::printf(
...
int run_variant(CaseBuffers* buffers, const Variant& variant, const char* label,
  ...
  int rc = launch_q4(buffers->d_w, buffers->output_rows, buffers->columns,
                     buffers->d_y, buffers->prompt_rows, buffers->d_o,
                     variant.async_x);
...
int run_correctness(const char* model_path) {
  ...
  rc |= run_q4_pair("corr_k768", 128, 768, 128, true);
  rc |= run_q4_pair("tail_m129", 129, 768, 129, false);
  rc |= run_stale_shared();
...
cudaError_t launch_complete_ffn(const qw38::cuda::DeviceCommonLayer& layer,
                                ...
                                std::size_t prompt_rows, bool async_x) {
  qw38::cuda::MmqAsyncXOverrideScope scope(async_x);
...
int run_screen(const char* model_path) {
      ...
      for (int sample = 0; sample < 4; ++sample) {
        ...
        if (error == cudaSuccess) {
          error = time_mmq(rotating, group.rows, group.columns,
                           ...
                           kVariants[v].async_x, start, stop, &ms);
        }
        if (error != cudaSuccess) {
          cudaFree(rotating);
          cudaEventDestroy(start);
        ...
        if (sample == 0) {
          std::printf(
              ...
              json_bool(rec.async_x), rec.kernel);
        } else {
          acc += ms;
          ++n;
          std::printf(
              "screen_sample group=%s variant=%s sample=%d ms=%.9g "
              "pipeline=%s async_x=%s kernel=%s\n",
        ...
        if (rec.async_x != kVariants[v].async_x) {
          std::fprintf(stderr, "screen async_x mismatch for %s\n",
                       kVariants[v].id);
          cudaFree(rotating);
          cudaEventDestroy(start);
          cudaEventDestroy(stop);
  ...
  const bool survivor_async_x = gemm_faster;
  float control_ffn = 0.0F;
  float survivor_ffn = 0.0F;
  auto time_ffn = [&](bool async_x, float* mean) -> int {
    float acc = 0.0F;
    int n = 0;
    for (int sample = 0; sample < 4; ++sample) {
      float ms = 0.0F;
      ...
      if (error == cudaSuccess) {
        error = launch_complete_ffn(layer, &workspace, kPrompt, async_x);
      }
      if (error == cudaSuccess) error = cudaEventRecord(stop, nullptr);
      if (error == cudaSuccess) error = cudaEventSynchronize(stop);
      if (error == cudaSuccess) error = cudaEventElapsedTime(&ms, start, stop);
      ...
      if (sample == 0) {
        std::printf(
            "complete_warmup async_x=%s ms=%.9g last_kernel=%s pipeline=%s\n",
            json_bool(async_x), ms, rec.kernel, json_bool(rec.pipeline));
      } else {
        acc += ms;
        ++n;
        std::printf(
            "complete_sample async_x=%s sample=%d ms=%.9g last_kernel=%s "
            "pipeline=%s\n",
            json_bool(async_x), sample, ms, rec.kernel, json_bool(rec.pipeline));
      }
    }
    *mean = n == 0 ? 0.0F : acc / static_cast<float>(n);
    return 0;
  };
  int rc = time_ffn(false, &control_ffn);
  if (rc == 0) rc = time_ffn(survivor_async_x, &survivor_ffn);
  cudaEventDestroy(start);
  cudaEventDestroy(stop);
  if (rc != 0) return rc;

  const bool keep = survivor_async_x && survivor_ffn < control_ffn;
  std::printf(
      "screen_winner control_gate_up_ms=%.9g candidate_gate_up_ms=%.9g "
      "control_down_ms=%.9g candidate_down_ms=%.9g control_ffn_ms=%.9g "
      "survivor_ffn_ms=%.9g keep=%s installed_async_x=%s selected_async_x=%s\n",
      means[0][kControl], means[0][kCandidate], means[1][kControl],
      means[1][kCandidate], control_ffn, survivor_ffn, json_bool(keep),
      json_bool(keep), json_bool(qw38::cuda::selected_mmq_async_x()));
  return 0;
}

void emit_payload(const char* workload, const char* status) {
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-066\",\"workload\":\"%s\","
      "\"tier\":\"%s\",\"status\":\"%s\",\"selected_pipeline\":\"%s\","
      "\"selected_async_x\":%s,\"tile\":\"i128_j128\","
      ...
      json_bool(qw38::cuda::selected_mmq_async_x()), kLlamaRev, kGgufSha);
}
