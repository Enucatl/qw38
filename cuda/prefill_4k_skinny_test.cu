...
int main(int argc, char** argv) {
  ...
  for (int replicate = 0; replicate < kReplicates; ++replicate) {
    ...
    if (graphs.prompt_graph_rows() != qw38::cuda::kPromptChunkRows ||
      ...
      std::fprintf(stderr, "4K skinny prompt graphs are incomplete\n");
      return 1;
    }
    ...
    if (session.frontier() != kPromptTokens) {
      std::fprintf(stderr, "4K skinny session is incomplete\n");
      return 1;
    }
  }
  ...
  std::printf(
      "QW38_PREFILL_4K_SKINNY_RESULT={\"schema_version\":1,\"task\":\"OPT-023\","
