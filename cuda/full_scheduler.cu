...
class GpuPhaseRecorder final {
 ...
 private:
  ...
  // embedding + 64 * (mixer_mmq_in + core + mixer_mmq_out + ffn) + logits +
  // commit, plus one optional mixer RMSNorm core phase on the fused path.
  std::array<Phase, 264> phases_{};
  std::size_t count_ = 0;
  bool active_ = false;
};
...
Status execute_token(const ResidentModel& model, std::size_t token,
  ...
  if (decode_attribution != nullptr) *decode_attribution = {};
