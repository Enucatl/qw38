...
constexpr char kPrefix[] = "QW38_OPT095_ATTENTION_GQA_RESULT=";
...
int launch_complete(const char* path, const qw38::cuda::AttentionConfig& config,
  ...
  qw38::cuda::DecodeAttentionGqaPathScope scope(path);
...
int run_case(const char* name, std::size_t position, std::uint32_t capacity,
  ...
  const char* paths[] = {"warp_query", "warp_query_gqa6"};
...
int run_zero_case(std::size_t position) {
  ...
  int rc = launch_complete("warp_query", config, position, query, key, value,
                           gate, qscale, kscale, committed_key, committed_value,
                           &control, false);
  rc |= launch_complete("warp_query_gqa6", config, position, query, key, value,
                        gate, qscale, kscale, committed_key, committed_value,
                        &candidate, false);
  if (rc != 0) return rc;
  float max_abs = 0.0F;
...
int run_candidate_boundary() {
  ...
  fill_case(config, 4, 3, &query, &key, &value, &gate, &qscale, &kscale,
            &committed_key, &committed_value);
  LaunchResult base;
  int rc = launch_complete("warp_query_gqa6", config, 4, query, key, value,
                           gate, qscale, kscale, committed_key, committed_value,
                           &base, false);
  if (rc != 0) return rc;
  for (std::uint32_t dim = 0; dim < config.head_width; ++dim) {
  ...
  rc = launch_complete("warp_query_gqa6", config, 4, query, key, value, gate,
...
int run_alignment_fallback() {
  ...
  int   rc = launch_complete("warp_query_gqa6", config, 3, query, key, value, gate,
                       qscale, kscale, committed_key, committed_value, &aligned,
                       false);
  const unsigned int aligned_flag = qw38::cuda::last_decode_kv_unaligned_fallback();
  if (rc != 0) return rc;
  rc = launch_complete("warp_query_gqa6", config, 3, query, key, value, gate,
                       qscale, kscale, committed_key, committed_value,
                       &unaligned, true);
...
int run_attrs() {
  ...
  const int gqa_occ = qw38::cuda::decode_kv_warp_query_gqa6_occupancy();
  const std::size_t shared_bytes =
      2 * kWidth * sizeof(__nv_bfloat16) + 6 * kWidth * sizeof(float) +
      6 * sizeof(float);
  std::printf(
      "warp_occupancy=%d gqa6_occupancy=%d shared_bytes=%zu gqa_grid_x=4 "
      "gqa_block=(32,6) sequential_pin=%s n_parts_low=%d n_parts_high=%d\n",
      warp_occ, gqa_occ, shared_bytes,
  ...
  FILE* pipe = popen(
      "cuobjdump -sass build/qw38-cuda-opt095-attention-gqa-test "
...
int main(int argc, char** argv) {
  ...
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-095\",\"workload\":\"%s\","
      "\"llama_revision\":\"%s\",\"gguf_sha256\":\"%s\","
      "\"selected_decode_attention_gqa\":\"%s\",\"n_parts\":16,\"nonfinite\":%d,"
      "\"pass\":%s}\n",
      kPrefix, workload, kLlamaRev, kGgufSha,
      qw38::cuda::selected_decode_attention_gqa_path(), pass ? 0 : 1,
      pass ? "true" : "false");
  std::printf("status=%s\n", pass ? "passed" : "failed");
  return pass ? 0 : 1;
}
