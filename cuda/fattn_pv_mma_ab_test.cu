...
constexpr char kPrefix[] = "QW38_FATTN_PV_MMA_AB_RESULT=";
constexpr const char* kCandidates[] = {"scalar", "mma"};
...
int occupancy_for(const char* pv_path) {
  if (std::strcmp(pv_path, "mma") == 0)
...
#undef QW38_COPY
  ...
  for (int c = 0; c < kCandidateCount; ++c) {
    ...
    error = qw38::cuda::launch_attention_prepare_chunk_stream_k_pv(
        kConfig, 0, kTokenCount, device_query, device_key, device_value,
    ...
    results[c].candidate_exact =
        ...
        std::memcmp(actual_candidate_value.data(), tiled_candidate_value.data(),
                    row_values * sizeof(__nv_bfloat16)) == 0;
    if (c == 0) {
      scalar_output = results[c].output;
      results[c].vs_scalar = Envelope{};
    } else {
      results[c].vs_scalar = compare(results[c].output, scalar_output);
    }
    results[c].eligible =
    ...
    std::fprintf(raw,
                 "pv_mma_ab id=%s occupancy=%d launch_ok=%s "
                 "max_abs=%.9g rms=%.9g nonfinite=%zu vs_scalar_max_abs=%.9g "
                 "vs_scalar_rms=%.9g scratch=%s committed=%s candidate=%s "
                 "eligible=%s\n",
                 ...
                 static_cast<double>(results[c].vs_scalar.max_abs),
                 static_cast<double>(results[c].vs_scalar.rms),
  ...
  if (!results[0].eligible) {
    std::fclose(raw);
    std::fprintf(stderr, "scalar stream-K candidate is not eligible\n");
    return 1;
  }
  ...
  for (int round = 0; round < kWarmups + kMeasured; ++round) {
    for (int c = 0; c < kCandidateCount; ++c) {
      ...
      if (round < kWarmups) {
        ...
        std::fprintf(raw, "pv_mma_ab id=%s warmup=%d ms=%.9g occupancy=%d\n",
                     kCandidates[c], round, static_cast<double>(ms),
                     results[c].occupancy);
      } else {
        results[c].samples.push_back(ms);
        std::fprintf(raw, "pv_mma_ab id=%s sample=%d ms=%.9g occupancy=%d\n",
  ...
  for (int c = 0; c < kCandidateCount; ++c) {
    ...
    std::fprintf(raw,
                 "pv_mma_ab_mean id=%s mean_ms=%.9g occupancy=%d eligible=%s\n",
                 results[c].id, static_cast<double>(results[c].mean_ms),
                 results[c].occupancy, json_bool(results[c].eligible));
  }
  const float scalar_mean = results[0].mean_ms;
  const bool mma_win =
      results[1].eligible && results[1].mean_ms < scalar_mean;
  const char* winner = mma_win ? "mma" : "scalar";
  std::fprintf(raw,
               "pv_mma_ab_winner id=%s win=%s scalar_ms=%.9g mma_ms=%.9g "
               "include_combine=true\n",
               winner, json_bool(mma_win),
               static_cast<double>(scalar_mean),
  ...
  std::printf("%s{\"schema_version\":1,\"task\":\"OPT-035\",\"status\":\"passed\",",
              kPrefix);
  ...
  std::printf("\"include_combine\":true,\"winner\":\"%s\",\"win\":%s,", winner,
              json_bool(mma_win));
  std::printf("\"selected_vkq_accum\":\"registers\",");
  ...
  for (int c = 0; c < kCandidateCount; ++c) {
    ...
    std::printf(
        ...
        "\"vs_scalar\":{\"max_abs\":%.9g,\"rms\":%.9g,\"nonfinite\":%zu}}",
        ...
        static_cast<double>(result.vs_scalar.max_abs),
        static_cast<double>(result.vs_scalar.rms), result.vs_scalar.nonfinite);
  }
