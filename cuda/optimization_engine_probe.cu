...
int parse_args(int argc, char** argv, Options* options) {
  for (int index = 1; index < argc; ++index) {
    ...
    } else if (std::strcmp(arg, "--decode-norm-q8") == 0 && index + 1 < argc) {
      options->decode_norm_q8 = argv[++index];
...
int apply_defaults(const qw38::cuda::TestTier tier, Options* options) {
  ...
  if (std::strcmp(options->workload, "attn-ab") == 0) {
    if (options->decode_query_prep != nullptr ||
        ...
        options->decode_norm_q8 != nullptr ||
...
int reject_over_bounds(qw38::cuda::TestTier tier, const Options& options) {
  ...
  if (tier == qw38::cuda::TestTier::kScreen) {
    ...
    const bool grouping_ok = grouping_ab && options.prefix == kScreenPrefix &&
                             options.output_tokens == kScreenOutputTokens &&
                             options.prompt == 0 && options.pairs <= 1;
    const bool device_ok = device_ab && options.prefix == kScreenPrefix &&
                           options.output_tokens == kScreenOutputTokens &&
                           options.prompt == 0 && options.pairs <= 1;
    const bool attn_ok =
  ...
  if (tier == qw38::cuda::TestTier::kAcceptance &&
    ...
    const bool grouping_ok = grouping_ab &&
                             ...
                             options.output_tokens == kScreenOutputTokens &&
                             options.prompt == 0 && options.pairs <= 5;
    const bool device_ok = device_ab &&
                           (options.prefix == kScreenPrefix ||
                            options.prefix == 128) &&
                           options.output_tokens == kScreenOutputTokens &&
                           options.prompt == 0 && options.pairs <= 5;
...
int run_keep_ab(const Options& options) {
  ...
  const bool attn = std::strcmp(options.workload, "attn-ab") == 0;
  qw38::cuda::ResidentModel model;
  const int loaded = load_model(options.model, &model);
  if (loaded != 0) return loaded;
  const bool decode_ab =
      q8 || q4 || gdn || grouping || device_layout || q4_device_layout;
  const std::size_t prompt =
      attn ? options.prompt : ((!decode_ab) ? options.prompt : 0);
  const std::size_t prefix =
      attn ? options.prefix : (decode_ab ? options.prefix : 0);
  ...
  const char* control_device = "raw_gguf";
  const char* candidate_device = "aligned_soa";
  const char* control_q4 = q4_device_layout ? "integer_q8_late" : "packed";
  const char* control_ffn =
      q4_device_layout ? "paired_integer" : "paired_staged";
  const char* control_q4_layout = "raw_gguf";
  const char* candidate_q4 =
      options.q4_decode != nullptr
          ? options.q4_decode
          : (q4_device_layout ? "integer_q8_aligned" : "integer_q8");
  const char* candidate_ffn =
      options.ffn_decode != nullptr ? options.ffn_decode : "paired_integer";
  const char* candidate_q4_layout = options.q4_device_layout != nullptr
                                        ? options.q4_device_layout
                                        : "aligned_meta";
  const char* candidate_gdn =
      options.gdn_decode != nullptr ? options.gdn_decode : "tile16";
  const char* candidate_attn = options.attention_pipeline != nullptr
  ...
  for (int pair = 0; pair < pairs; ++pair) {
    ...
    const bool numeric_side = q4 || q4_device_layout || gdn || attn;
    const char* first = "0";
    const char* second = "1";
    if (numeric_side) {
      first = ba ? "1" : "0";
      ...
      first = ba ? candidate_grouping : control_grouping;
      second = ba ? control_grouping : candidate_grouping;
    } else if (device_layout) {
      first = ba ? candidate_device : control_device;
      second = ba ? control_device : candidate_device;
    } else if (q8) {
      first = ba ? candidate_layout : control_layout;
      second = ba ? control_layout : candidate_layout;
    } else {
      first = ba ? "1" : "0";
      second = ba ? "0" : "1";
    }
    ...
    for (int side = 0; side < 2; ++side) {
      const bool candidate_side =
          ...
          (device_layout && std::strcmp(labels[side], candidate_device) == 0);
      if (q4 || q4_device_layout) {
        ...
        if (q4_device_layout) {
          const char* layout =
              candidate_side ? candidate_q4_layout : control_q4_layout;
          if (!qw38::cuda::apply_q4_device_layout_ident(layout)) {
            std::fprintf(stderr, "invalid q4 device layout %s\n", layout);
            return 2;
          }
          const qw38::Status converted = model.set_q4_device_layout(
              layout, pair == 0 && side == 0);
          if (!converted.is_ok()) return fail_status(converted);
        }
      } else if (gdn) {
        ...
        if (options.decode_norm_q8 != nullptr) {
          const char* path = candidate_side
                                 ? options.decode_norm_q8
                                 : qw38::cuda::opt149::kControlId;
          if (!qw38::cuda::opt149::apply_decode_norm_q81_ident(path)) {
      ...
      qw38::cuda::opt149::clear_decode_norm_q81_fusion_override();
    }
    ...
    if (q4_device_layout) {
      const qw38::Status restored = model.set_q4_device_layout(
          qw38::cuda::selected_q4_device_layout(), false);
      if (!restored.is_ok()) return fail_status(restored);
      qw38::cuda::clear_q4_device_layout_override();
    }
    std::printf(
  ...
  std::printf(
      ...
      q4 ? "q4"
         : (gdn ? "gdn"
                : (grouping ? "grouping" : (device_layout ? "q8_device" : (q8 ? "q8" : "mmq")))),
  ...
  std::printf(
      ...
      "\"family\":\"attn\",\"tier\":\"%s\",\"warmups\":0,\"samples\":%d,"
      "\"observed_warmups\":0,\"observed_samples\":%d,\"observed_candidates\":2,"
      "\"observed_shapes\":1,\"observed_tier\":\"%s\",\"pairs\":%d,"
      "\"acceptance_executed\":%s,\"keep\":false,"
      "\"override_before_capture_applied\":true,"
