...
constexpr char kPrefix[] = "QW38_OPT103_VECTOR_ATTENTION_RESULT=";
...
int launch_complete(const char* path, int n_parts,
  ...
  qw38::cuda::DecodeAttentionVec128PathScope scope(path);
...
int run_case(const char* name, std::size_t position, std::uint32_t capacity,
             int cand_parts, bool sample_fp64) {
  ...
  int rc = launch_complete("warp_query", kParts, config, position, query, key,
                           value, gate, qscale, kscale, committed_key,
                           committed_value, &control, false);
  rc |= launch_complete("vec128_online", cand_parts, config, position, query,
                        key, value, gate, qscale, kscale, committed_key,
                        committed_value, &candidate, false);
  if (rc != 0) return rc;
  float max_abs = 0.0F;
  ...
  if (sample_fp64) {
    ...
    for (int hi = 0; hi < 4; ++hi) {
      for (int di = 0; di < 8; ++di, ++si) {
        ...
        fp64_abs = std::max(
            fp64_abs, std::fabs(candidate.output[index] -
                                static_cast<float>(sampled[si])));
      }
    }
  }
  const bool pass = nonfinite == 0 && max_abs <= kVsControlAbs &&
                    fp64_abs <= kFp64Abs;
  std::printf(
      "attn_case=%s position=%zu n_parts=%d seq_abs=%.9g seq_nonfinite=%zu "
      "fp64_abs=%.9g prep_launches_last=%u pass=%s\n",
      name, position, cand_parts, max_abs, nonfinite, fp64_abs,
...
int run_zero_case(std::size_t position) {
  ...
  int rc = launch_complete("warp_query", kParts, config, position, query, key,
                           value, gate, qscale, kscale, committed_key,
                           committed_value, &control, false);
  rc |= launch_complete("vec128_online", kParts, config, position, query, key,
                        value, gate, qscale, kscale, committed_key,
                        committed_value, &candidate, false);
  if (rc != 0) return rc;
  float max_abs = 0.0F;
...
int run_candidate_boundary() {
  ...
  fill_case(config, 4, 3, &query, &key, &value, &gate, &qscale, &kscale,
            &committed_key, &committed_value);
  LaunchResult base;
  int rc = launch_complete("vec128_online", kParts, config, 4, query, key, value,
  ...
  rc = launch_complete("vec128_online", kParts, config, 4, query, key, value,
                       gate, qscale, kscale, committed_key, committed_value,
                       &committed_mut, false);
  if (rc != 0) return rc;
  value[0] += 1.5F;
...
int run_alignment_fallback() {
  ...
  int rc = launch_complete("vec128_online", kParts, config, 3, query, key, value,
                           gate, qscale, kscale, committed_key, committed_value,
                           &aligned, false);
  const unsigned int aligned_flag = qw38::cuda::last_decode_kv_unaligned_fallback();
  if (rc != 0) return rc;
  rc = launch_complete("vec128_online", kParts, config, 3, query, key, value,
                       gate, qscale, kscale, committed_key, committed_value,
                       &unaligned, true);
  ...
  std::printf(
      ...
      aligned_flag, unaligned_flag, max_abs, "true",
      pass ? "true" : "false");
  return pass ? 0 : 1;
}
...
int run_repeat_eager() {
  ...
  LaunchResult first;
  LaunchResult second;
  int rc = launch_complete("vec128_online", kParts, config, 32, query, key,
                           value, gate, qscale, kscale, committed_key,
                           committed_value, &first, false);
  rc |= launch_complete("vec128_online", kParts, config, 32, query, key, value,
                        gate, qscale, kscale, committed_key, committed_value,
                        &second, false);
  if (rc != 0) return rc;
...
int run_attrs() {
  ...
  const int vec_occ = qw38::cuda::decode_kv_vec128_online_occupancy();
  int regs = 0;
  std::size_t local_bytes = 0;
  int occ = 0;
  {
    qw38::cuda::DecodeAttentionVec128PathScope scope("vec128_online");
    qw38::cuda::decode_attention_kernel_attributes(
        qw38::cuda::effective_decode_query_prep_path(), &regs, &local_bytes,
        &occ);
  }
  std::printf(
      "warp_occupancy=%d vec128_occupancy=%d registers=%d local_bytes=%zu "
      "vec_grid_x=24 vec_block=(32,4) sequential_pin=%s gqa_pin=%s "
      "n_parts_low=%d n_parts_high=%d selected_vec128_parts=%d\n",
      warp_occ, vec_occ, regs, local_bytes,
      qw38::cuda::selected_decode_attention_vec128_path(),
      qw38::cuda::selected_decode_attention_gqa_path(),
      qw38::cuda::selected_decode_kv_parts_below_2048(),
      qw38::cuda::selected_decode_kv_parts_at_or_above_2048(),
      qw38::cuda::selected_vec128_n_parts());
  return (warp_occ > 0 && vec_occ > 0 &&
          qw38::cuda::legal_decode_attention_vec128_path(
              qw38::cuda::selected_decode_attention_vec128_path()) &&
          std::strcmp(qw38::cuda::selected_decode_attention_gqa_path(),
                      "warp_query") == 0 &&
          qw38::cuda::legal_vec128_n_parts(
              qw38::cuda::selected_vec128_n_parts()))
             ? 0
             : 1;
}
...
int main(int argc, char** argv) {
  ...
  if (!qw38::cuda::legal_decode_attention_vec128_path(
          qw38::cuda::selected_decode_attention_vec128_path())) {
    std::fprintf(stderr, "production pin is not a legal vec128 path\n");
    return 1;
  }
  ...
  int rc = 0;
  rc |= run_case("tiny_p0", 0, 32, 16, false);
  rc |= run_case("tiny_p1", 1, 32, 16, true);
  if (std::strcmp(workload, "smoke") != 0) {
    rc |= run_case("tiny_p31", 31, 64, 16, false);
    rc |= run_case("tiny_p32", 32, 64, 16, true);
    rc |= run_case("parts4_p32", 32, 64, 4, true);
    rc |= run_case("parts8_p32", 32, 64, 8, true);
    rc |= run_zero_case(1);
  ...
  if (std::strcmp(workload, "smoke") != 0 &&
    ...
    rc |= run_case("prod_p127", 127, 256, 16, false);
    rc |= run_case("prod_p128", 128, 256, 16, true);
  } else if (std::strcmp(workload, "correctness") == 0) {
    rc |= run_case("prod_p127", 127, 256, 16, true);
  }
  if (std::strcmp(workload, "acceptance") == 0) {
    rc |= run_case("prod_p2047", 2047, 2064, 16, false);
    rc |= run_case("prod_p2048", 2048, 2064, 16, true);
  }
  rc |= run_attrs();
  const bool pass = rc == 0;
  std::printf(
      "%s{\"schema_version\":1,\"task\":\"OPT-103\",\"workload\":\"%s\","
      "\"llama_revision\":\"%s\",\"gguf_sha256\":\"%s\","
      "\"selected_decode_attention_vec128\":\"%s\",\"n_parts\":16,"
      "\"nonfinite\":%d,\"pass\":%s}\n",
      kPrefix, workload, kLlamaRev, kGgufSha,
      qw38::cuda::selected_decode_attention_vec128_path(), pass ? 0 : 1,
      pass ? "true" : "false");
  std::printf("status=%s\n", pass ? "passed" : "failed");
  return pass ? 0 : 1;
}
