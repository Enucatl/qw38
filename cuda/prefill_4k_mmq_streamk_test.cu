...
int main(int argc, char** argv) {
  ...
  for (int replicate = 0; replicate < kReplicates; ++replicate) {
    ...
    if (graphs.prompt_graph_rows() != qw38::cuda::kPromptChunkRows ||
      ...
      std::fprintf(stderr, "4K MMQ stream-K prompt graphs are incomplete\n");
      return 1;
    }
    ...
    if (session.frontier() != kPromptTokens) {
      std::fprintf(stderr, "4K MMQ stream-K session is incomplete\n");
      return 1;
    }
  }
  ...
  std::printf(
      "QW38_PREFILL_4K_MMQ_STREAMK_RESULT={\"schema_version\":1,\"task\":\"OPT-028\","
      ...
      "\"selected_mmq_stream_k_path\":\"%s\",\"nsight_systems\":\"not_used\","
      ...
      mean_tok_s, kPromptTokens, qw38::cuda::selected_mmq_stream_k_path());
