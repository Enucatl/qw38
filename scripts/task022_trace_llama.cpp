...
static bool trace_input(ggml_tensor *tensor, bool ask, void *user_data) {
    ...
    const std::string node(tensor->name);
    constexpr std::array<const char *, 12> attention_nodes = {
        "l_out-2", "attn_norm-3", "Qcur_full-3", "Kcur-3", "Vcur-3",
        "Qcur_normed-3", "Kcur_normed-3", "Qcur-3", "attn_pregate-3",
        "gate_sigmoid-3", "attn_gated-3", "attn_output-3"};
    if (std::find_if(attention_nodes.begin(), attention_nodes.end(),
                     [&](const char *name) { return node == name; }) != attention_nodes.end()) {
        const auto path = std::filesystem::path(trace.directory) /
                          ("attention." + node + "." + trace.phase + ".bin");
        if (std::filesystem::exists(path) || !ggml_is_contiguous(tensor)) return false;
        if (ask) return true;
        if (!tensor->buffer) fail("attention node has no buffer: " + node);
        std::vector<char> data(ggml_nbytes(tensor));
        ggml_backend_tensor_get(tensor, data.data(), 0, data.size());
        std::ofstream output(path, std::ios::binary);
        output.write(data.data(), static_cast<std::streamsize>(data.size()));
        if (!output) fail("failed to write attention node: " + path.string());
        std::ofstream metadata(path.string() + ".txt");
        metadata << node << '\t' << trace.phase << '\t' << ggml_type_name(tensor->type);
        for (int i = 0; i < 4; ++i) metadata << '\t' << tensor->ne[i];
        metadata << '\t' << data.size() << '\n';
        std::cerr << "traced " << path << " bytes=" << data.size() << '\n';
        return true;
    }
    if (node.rfind("l_out-", 0) == 0 || node == "attn_residual-3" ||
        node == "attn_residual-7" || node == "attn_residual-31" ||
        node == "attn_residual-63") {
        const auto path = std::filesystem::path(trace.directory) /
                          ("residual." + node + "." + trace.phase + ".bin");
        if (std::filesystem::exists(path) || !ggml_is_contiguous(tensor)) return false;
        if (ask) return true;
        if (!tensor->buffer || tensor->type != GGML_TYPE_F32 || tensor->ne[0] != 5120) {
            fail("invalid residual node: " + node);
        }
        ...
        if (trace.phase == "prefill") {
            if (tensor->ne[1] != 384) fail("unexpected prefill residual shape");
            for (int p : {0, 63, 255, 383}) {
                output.write(reinterpret_cast<const char *>(data.data() + p * 5120),
                             5120 * sizeof(float));
            }
        } else {
            if (tensor->ne[1] != 1) fail("unexpected decode residual shape");
            output.write(reinterpret_cast<const char *>(data.data()), 5120 * sizeof(float));
        }
        if (!output) fail("failed to write residual: " + path.string());
        std::cerr << "traced " << path << '\n';
        return true;
    }
    constexpr std::array<const char *, 11> nodes = {
        "linear_attn_qkv_mixed-0", "z-0", "conv_output_raw-0",
        "conv_output_silu-0", "q_conv_predelta-0", "k_conv_predelta-0",
        "v_conv_predelta-0", "a_softplus-0", "gate-0",
        "beta_sigmoid-0", "final_output-0"};
    if (std::find_if(nodes.begin(), nodes.end(), [&](const char *name) {
            return node == name;
        }) != nodes.end()) {
        const auto path = std::filesystem::path(trace.directory) /
                          ("node." + node + "." + trace.phase + ".bin");
        if (std::filesystem::exists(path) || !ggml_is_contiguous(tensor)) return false;
        if (ask) return true;
        if (!tensor->buffer) fail("trace node has no buffer: " + node);
        std::vector<char> data(ggml_nbytes(tensor));
        ggml_backend_tensor_get(tensor, data.data(), 0, data.size());
        std::ofstream output(path, std::ios::binary);
        output.write(data.data(), static_cast<std::streamsize>(data.size()));
