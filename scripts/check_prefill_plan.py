#!/usr/bin/env python3
"""Check Qwen3.8-27B prefill-plan contracts against sitting text_config.

Reads ``text_config`` from a Transformers ``config.json`` (no safetensor
payloads) and either prints the prefill-plan summary object or checks that
``docs/architecture/prefill-plan.md`` matches it.
"""
...
CANONICAL_SENTENCE_SCHEDULE = (
    "The prefill schedule in this document is a hardware-independent order "
    "of TASK-11 nodes for many-token inference from zero state, not a CUDA "
    "graph, kernel launch sequence, or selected fusion."
)

CANONICAL_SENTENCE_TRAFFIC = (
    "Unavoidable traffic is the TASK-06 unique-weight, triangular-KV/state, "
    "and T-scaled activation minimum; proposed-boundary traffic is this "
    "schedule's named stage-cut channel."
)

CANONICAL_SENTENCE_OPEN_QUESTION = (
    "Decode/prefill representation tradeoffs and the potential need for "
    "multiple views remain unresolved without evidence; view_binding stays "
    "a capability, not a selected split."
)
...
FUNDAMENTAL_DIFFERENCE_IDS: tuple[str, ...] = (
    "diff_matrix_matrix",
    "diff_reuse",
    "diff_tiling",
    "diff_state",
...
NODE_VS_DECODE_WORK_CLASS: dict[str, str] = {
    "embed": "gather_T_vs_1",
...
NODE_VS_DECODE_STATE_CLASS: dict[str, str] = {
    "embed": "none",
...
NODE_VS_DECODE_TILING_AXIS: dict[str, str] = {
    "embed": "gather_rows",
...
SCHEMA_KEYS: tuple[str, ...] = (
    ...
    "activation_sequence_length_is_T",
    "stage_kinds_match_decode",
    ...
    "mode_selected",
    "n_mode_hypotheses",
    "n_mode_hypotheses_selected",
    "mode_usefulness_label",
    "representation_hypothesis_ids",
    "representation_selected",
    "n_representation_hypotheses",
    "n_representation_hypotheses_selected",
    "representation_usefulness_label",
    "fundamental_difference_ids",
    "n_fundamental_differences",
    "fundamental_difference_usefulness_label",
    "node_compare_ids",
    "n_nodes_compared_with_decode",
    "n_stage_kinds_compared_with_decode",
    "node_vs_decode_work_class",
    "node_vs_decode_state_class",
    "node_vs_decode_tiling_axis",
    "n_token_presentation_streams",
    "example_T",
    "prefill_T_new_equals_T",
    "T_is_stored_length_after_prefill",
    "incoming_state_zeros",
    "incoming_state_populated",
    "primary_includes_mtp",
    "mtp_runs_all_T_positions",
    "mac_C_language",
    "mac_C_complete",
    "mac_A_language",
    "mac_A_complete",
    ...
    "schedule_layer_serial_selected",
    ...
    "every_semantic_node_compared",
    "representation_tradeoffs_unresolved",
    "ledger_open_question_representation_tradeoffs_closed",
    "mac_prefill_equals_decode_at_T1",
    "state_read_prefill_equals_decode_at_T1",
    "kv_incoming_empty_at_T1_both",
    "diagram_ids",
    "n_diagrams",
    "canonical_sentence_logical",
    "canonical_sentence_schedule",
...
class PrefillPlanMismatch(Exception):
    """Raised when the prefill-plan markdown fails a content check."""
...
def dumps_summary(summary: dict) -> str:
    """Pretty-print the prefill-plan summary object.

    Args:
        summary: Object produced by :func:`instantiate_prefill_plan`.
...
def _ordered_node_mapping(values: dict[str, str]) -> dict[str, str]:
    """Return a JSON object keyed in ``NODE_TYPE_IDS`` order.

    Args:
        values: Mapping from node-type id to a comparison class string.

    Returns:
        Ordered dict with string values.
    """

    return {node_id: values[node_id] for node_id in NODE_TYPE_IDS}


def _prefill_mac(mac_c: int, mac_a: int, t_value: int) -> int:
    r"""Return TASK-06 prefill MAC \(W=TC+AT(T+1)/2\).

    Args:
        mac_c: Per-token non-attention MAC \(C\).
        mac_a: Attention coefficient \(A\).
        t_value: Prompt length \(T\).

    Returns:
        Integer MAC count for one complete or language-only prefill.
    """

    return mac_c * t_value + mac_a * t_value * (t_value + 1) // 2


def _scale_by_t(base: int, example_t: list[int]) -> list[int]:
    """Return ``base * T`` at each example prompt length.

    Args:
        base: Per-token or decode-rank byte identity.
        example_t: Prompt lengths in ``example_T`` order.

    Returns:
        T-scaled integer list.
    """

    return [base * t_value for t_value in example_t]


def instantiate_prefill_plan(config: dict) -> dict:
    """Compute the TASK-14 prefill-plan summary from ``config``.
    ...
    mac_prefill_last_logits_language = [
        mac_prefill_language[index] - (t_value - 1) * mac_lm_head
        for index, t_value in enumerate(example_t)
    ]
    mac_prefill_last_logits_complete = [
        mac_prefill_complete[index] - 2 * (t_value - 1) * mac_lm_head
        for index, t_value in enumerate(example_t)
    ]
    ...
    weight_gather_language = _scale_by_t(weight_gather_bytes_per_row, example_t)
    weight_gather_complete = _scale_by_t(2 * weight_gather_bytes_per_row, example_t)
    weight_unique_plus_gather_complete = [
        weight_bytes_unique_non_embed + gather for gather in weight_gather_complete
    ]
    ...
    decode_write_bytes = kv_bytes_all_per_token + c_write_bytes_all + s_bytes_all
    ...
    prefill_kv_read = [
        kv_bytes_all_per_token * t_value * (t_value - 1) // 2 for t_value in example_t
    ]
    prefill_kv_write = [kv_bytes_all_per_token * t_value for t_value in example_t]
    prefill_c_write = [c_write_bytes_all * t_value for t_value in example_t]
    prefill_s_write = [s_bytes_all * t_value for t_value in example_t]
    prefill_write = [
        prefill_kv_write[index] + prefill_c_write[index] + prefill_s_write[index]
        for index in range(len(example_t))
    ]
    prefill_c_read_math = [c_read_bytes_all * t_value for t_value in example_t]
    prefill_s_read_math = [s_bytes_all * t_value for t_value in example_t]
    prefill_c_read_physical = [
        c_read_bytes_all * (t_value - 1) for t_value in example_t
    ]
    prefill_s_read_physical = [s_bytes_all * (t_value - 1) for t_value in example_t]
    prefill_read_physical = [
        prefill_kv_read[index]
        + prefill_c_read_physical[index]
        + prefill_s_read_physical[index]
        for index in range(len(example_t))
    ]
    ...
    act_residual_stage_cut_decode = n_residual_stage_crossings * residual_bytes
    act_logits_stage_cut_decode = n_logit_outputs * logits_bytes
    act_stage_cut_decode_complete = (
        act_residual_stage_cut_decode + act_logits_stage_cut_decode
    )
    act_forced_decode_language = (
        n_decoder_layers * residual_bytes * 2
        + n_full_layers * g_bytes
        + n_linear_layers * z_bytes
        + logits_bytes
    )
    act_forced_decode_complete = (
        act_forced_decode_language + 2 * residual_bytes + g_bytes + logits_bytes
    )
    act_region_cut_decode_language = (
        act_forced_decode_language
        + residual_bytes
    ...
    act_region_cut_decode_complete = (
        act_region_cut_decode_language
        + 2 * residual_bytes
        + g_bytes
        + logits_bytes
    ...
    act_h_mid_stage_cut_decode = n_h_mid_crossings * residual_bytes
    act_residual_stage_cut = _scale_by_t(act_residual_stage_cut_decode, example_t)
    act_logits_stage_cut = _scale_by_t(act_logits_stage_cut_decode, example_t)
    act_stage_cut_complete = _scale_by_t(act_stage_cut_decode_complete, example_t)
    act_forced_complete = _scale_by_t(act_forced_decode_complete, example_t)
    act_forced_language = _scale_by_t(act_forced_decode_language, example_t)
    act_region_cut_complete = _scale_by_t(act_region_cut_decode_complete, example_t)
    act_region_cut_language = _scale_by_t(act_region_cut_decode_language, example_t)
    act_boundary_added_vs_forced = [
        act_stage_cut_complete[index] - act_forced_complete[index]
        for index in range(len(example_t))
    ]
    act_boundary_added_vs_region_cut = [
        act_stage_cut_complete[index] - act_region_cut_complete[index]
        for index in range(len(example_t))
    ]
    act_gz_local = _scale_by_t(act_gz_local_decode, example_t)
    act_h_mid_stage_cut = _scale_by_t(act_h_mid_stage_cut_decode, example_t)
    act_h64_second_consumer = _scale_by_t(residual_bytes, example_t)
    act_logits_last_logits = [act_logits_stage_cut_decode for _ in example_t]
    ...
    mode_selected = {hyp_id: False for hyp_id in MODE_HYPOTHESIS_IDS}
    representation_selected = {
        hyp_id: False for hyp_id in REPRESENTATION_HYPOTHESIS_IDS
    }
    n_stage_instances = sum(STAGE_MULTIPLICITIES)
    mac_prefill_equals_decode_at_t1 = mac_prefill_complete[0] == (
        mac_c_complete + mac_a_complete
    )
    ...
    summary = {
        ...
        "fundamental_difference_ids": list(FUNDAMENTAL_DIFFERENCE_IDS),
        "n_fundamental_differences": len(FUNDAMENTAL_DIFFERENCE_IDS),
        "fundamental_difference_usefulness_label": "HYPOTHESIS",
        "node_compare_ids": list(NODE_TYPE_IDS),
        "n_nodes_compared_with_decode": len(NODE_TYPE_IDS),
        "n_stage_kinds_compared_with_decode": len(STAGE_KIND_IDS),
        "node_vs_decode_work_class": _ordered_node_mapping(NODE_VS_DECODE_WORK_CLASS),
        "node_vs_decode_state_class": _ordered_node_mapping(NODE_VS_DECODE_STATE_CLASS),
        "node_vs_decode_tiling_axis": _ordered_node_mapping(NODE_VS_DECODE_TILING_AXIS),
        ...
        "weight_gather_bytes_prefill_language_at_example_T": weight_gather_language,
        "weight_gather_bytes_prefill_complete_at_example_T": weight_gather_complete,
        "weight_unique_plus_gather_prefill_complete_at_example_T": (
            weight_unique_plus_gather_complete
        ),
        ...
        "prefill_kv_read_bytes_at_example_T": prefill_kv_read,
        "prefill_kv_write_bytes_at_example_T": prefill_kv_write,
        "prefill_c_write_bytes_at_example_T": prefill_c_write,
        "prefill_s_write_bytes_at_example_T": prefill_s_write,
        "prefill_write_bytes_at_example_T": prefill_write,
        "prefill_c_read_math_at_example_T": prefill_c_read_math,
        "prefill_s_read_math_at_example_T": prefill_s_read_math,
        "prefill_c_read_physical_at_example_T": prefill_c_read_physical,
        "prefill_s_read_physical_at_example_T": prefill_s_read_physical,
        "prefill_read_physical_at_example_T": prefill_read_physical,
        ...
        "act_residual_stage_cut_prefill_at_example_T": act_residual_stage_cut,
        "act_logits_stage_cut_prefill_at_example_T": act_logits_stage_cut,
        "act_stage_cut_prefill_complete_at_example_T": act_stage_cut_complete,
        "act_forced_prefill_complete_at_example_T": act_forced_complete,
        "act_forced_prefill_language_at_example_T": act_forced_language,
        "act_region_cut_prefill_complete_at_example_T": act_region_cut_complete,
        "act_region_cut_prefill_language_at_example_T": act_region_cut_language,
        "act_boundary_added_vs_forced_at_example_T": act_boundary_added_vs_forced,
        "act_boundary_added_vs_region_cut_at_example_T": (
            act_boundary_added_vs_region_cut
        ),
        "act_gz_local_prefill_at_example_T": act_gz_local,
        "act_h_mid_stage_cut_prefill_at_example_T": act_h_mid_stage_cut,
        "act_h64_second_consumer_bytes_at_example_T": act_h64_second_consumer,
        "act_h64_second_consumer_is_hypothesis": True,
        "act_logits_stage_cut_last_logits_at_example_T": act_logits_last_logits,
        ...
        "mac_prefill_equals_decode_at_T1": mac_prefill_equals_decode_at_t1,
...
def _assert_identities(summary: dict, text: dict) -> None:
    """Assert dossier identities on a live prefill-plan object.
    ...
    assert summary["n_mode_hypotheses"] == 2
    assert summary["n_mode_hypotheses_selected"] == 0
    assert summary["n_representation_hypotheses"] == 4
    assert summary["n_representation_hypotheses_selected"] == 0
    assert summary["n_fundamental_differences"] == 5
    assert summary["n_nodes_compared_with_decode"] == 6
    ...
    assert summary["n_token_presentation_streams"] == 2
    ...
    assert summary["act_residual_stage_cut_prefill_at_example_T"] == [
        1361920,
        5578424320,
    ]
    assert summary["logits_bytes"] == 496640
    assert summary["act_logits_stage_cut_prefill_at_example_T"] == [
        993280,
        4068474880,
    ]
    assert summary["act_stage_cut_prefill_complete_at_example_T"] == [
        2355200,
        9646899200,
    ]
    assert summary["act_forced_prefill_complete_at_example_T"] == [
        3123200,
        12792627200,
    ]
    assert summary["act_region_cut_prefill_complete_at_example_T"] == [
        5847040,
        23949475840,
    ]
    assert summary["act_boundary_added_vs_forced_at_example_T"] == [
        -768000,
        -3145728000,
    ]
    assert summary["act_boundary_added_vs_region_cut_at_example_T"] == [
        -3491840,
        -14302576640,
    ]
    assert summary["act_gz_local_prefill_at_example_T"] == [798720, 3271557120]
    assert summary["act_h_mid_stage_cut_prefill_at_example_T"] == [
        665600,
        2726297600,
    ]
    assert summary["weight_bytes_unique_non_embed"] == 52098598912
    assert summary["weight_gather_bytes_prefill_complete_at_example_T"] == [
        20480,
        83886080,
    ]
    assert summary["weight_unique_plus_gather_prefill_complete_at_example_T"] == [
        52098619392,
        52182484992,
    ]
    ...
    assert summary["prefill_kv_write_bytes_at_example_T"] == [69632, 285212672]
    assert summary["prefill_write_bytes_at_example_T"][0] == 152047616
    assert summary["prefill_read_physical_at_example_T"][0] == 0
    ...
    for index, t_value in enumerate(summary["example_T"]):
        assert summary["act_stage_cut_prefill_complete_at_example_T"][index] == (
            2355200 * t_value
        )
        assert summary["act_residual_stage_cut_prefill_at_example_T"][index] == (
            summary["n_residual_stage_crossings"]
            * summary["residual_bytes"]
            * t_value
        )
        assert summary["act_region_cut_prefill_complete_at_example_T"][index] == (
            5847040 * t_value
        )
        assert summary["act_forced_prefill_complete_at_example_T"][index] == (
            3123200 * t_value
        )
        assert summary["act_forced_prefill_language_at_example_T"][index] == (
            2593792 * t_value
        )
        assert summary["prefill_s_write_bytes_at_example_T"][index] == (
            summary["s_bytes_all"] * t_value
        )
        assert summary["prefill_write_bytes_at_example_T"][index] == (
            summary["decode_write_bytes"] * t_value
        )
        assert summary["prefill_write_bytes_at_example_T"][index] == (
            summary["prefill_kv_write_bytes_at_example_T"][index]
            + summary["prefill_c_write_bytes_at_example_T"][index]
            + summary["prefill_s_write_bytes_at_example_T"][index]
        )
        assert summary["prefill_s_read_physical_at_example_T"][index] == (
            summary["s_bytes_all"] * (t_value - 1)
        )
        assert summary["prefill_read_physical_at_example_T"][index] == (
            summary["prefill_kv_read_bytes_at_example_T"][index]
            + summary["prefill_c_read_physical_at_example_T"][index]
            + summary["prefill_s_read_physical_at_example_T"][index]
        )
        assert summary["mac_prefill_complete_at_example_T"][index] == (
            summary["mac_C_complete"] * t_value
            + summary["mac_A_complete"] * t_value * (t_value + 1) // 2
        )
    assert summary["mac_prefill_complete_at_example_T"][0] == (
        summary["mac_C_complete"] + summary["mac_A_complete"]
    )
    assert summary["mac_prefill_equals_decode_at_T1"] is True
    assert summary["state_read_prefill_equals_decode_at_T1"] is False
    ...
    assert summary["schedule_layer_serial_selected"] is True
    ...
    assert summary["ledger_open_question_representation_tradeoffs_closed"] is False
    assert summary["representation_tradeoffs_unresolved"] is True
    assert summary["every_semantic_node_compared"] is True
    assert summary["thread_geometry_absent"] is True
    assert summary["hardware_independent"] is True
    assert summary["incoming_state_zeros"] is True
    assert summary["incoming_state_populated"] is False
    ...
    assert all(value is False for value in summary["mode_selected"].values())
    assert all(value is False for value in summary["representation_selected"].values())
    ...
    assert summary["fundamental_difference_ids"] == list(FUNDAMENTAL_DIFFERENCE_IDS)
    assert summary["node_compare_ids"] == summary["node_type_ids"]
...
def diff_summary(live: dict, documented: dict) -> list[str]:
    """Compare documented JSON against a live prefill-plan object.
...
def check_prefill_plan(live: dict, prefill_plan_path: Path) -> None:
    """Check a prefill-plan markdown file against a live summary.

    Args:
        live: Prefill-plan summary object from sitting config.
        prefill_plan_path: Path to ``prefill-plan.md``.

    Raises:
        PrefillPlanMismatch: On heading, JSON, diagram, or token mismatches.
        OSError: If the file cannot be read.
    """

    text = prefill_plan_path.read_text(encoding="utf-8")
    ...
    if differences:
        raise PrefillPlanMismatch(differences)


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    """Parse CLI arguments for the prefill-plan checker."""

    parser = argparse.ArgumentParser(
        description=(
            "Instantiate Qwen3.8-27B language+MTP prefill-plan summary "
            "from text_config and check docs/architecture/prefill-plan.md."
        )
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Write prefill-plan summary JSON to stdout.",
    )
    parser.add_argument(
        "--prefill-plan",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    """Run the prefill-plan checker CLI.
    ...
    try:
        ...
        summary = instantiate_prefill_plan(config)
        if args.prefill_plan is not None:
            if not args.prefill_plan.is_file():
                print(
                    f"prefill-plan file not found: {args.prefill_plan}",
                    file=sys.stderr,
                )
                return 1
            check_prefill_plan(summary, args.prefill_plan)
    except MissingConfig as exc:
        print(str(exc.path), file=sys.stderr)
        return 2
    except PrefillPlanMismatch as exc:
    ...
    if args.json or args.prefill_plan is None:
