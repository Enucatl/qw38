#!/usr/bin/env python3
"""Check Qwen3.8-27B model-compiler plan against sitting text_config.

Reads ``text_config`` from a Transformers ``config.json`` (no safetensor
payloads) and either prints the model-compiler-plan summary object or
checks that ``docs/architecture/model-compiler-plan.md`` matches it.
"""
...
CANONICAL_SENTENCE_STAGES = (
    "Compiler stages in this document are a conceptual pipeline, not a "
    "selected runtime implementation."
)

CANONICAL_SENTENCE_PROFILES = (
    "Quality, balanced, and compression profiles in this document are "
    "conceptual intent axes, not selected winners."
)

CANONICAL_SENTENCE_OWNERSHIP = (
    "Profile-decision ownership is classified; recipe maps, packing winners, "
    "and calibration corpora remain unselected."
)

OWNERSHIP_QUESTION_SENTENCE = (
    "Profile-decision ownership is architecture for legal sets and "
    "intent-axis names, model for source-weight statistics, calibration for "
    "recipe maps and corpora, and backend for specialized views and artifact "
    "boundary."
)
...
QUANTIZATION_OUTPUT_IDS: tuple[str, ...] = (
    "element_codes",
    "group_scales",
...
DECISION_OWNERSHIP_VALUES: tuple[str, ...] = (
    "architecture",
    "architecture",
    "architecture",
    "architecture",
...
LOCKED_STRUCTURE_IDS: tuple[str, ...] = DECISION_IDS[:8]
UNSELECTED_WINNER_IDS: tuple[str, ...] = DECISION_IDS[8:]

COMPILER_PROFILE_IDS: tuple[str, ...] = (
    "quality",
    "balanced",
    "compression",
)
...
COMPILER_RISK_SEVERITIES: tuple[str, ...] = (
    "high",
    "high",
    "medium",
    "medium",
...
COMPILER_HIGH_IDS: tuple[str, ...] = (
    "c_profile_as_winner",
    "c_skip_validation",
)

COMPILER_MEDIUM_IDS: tuple[str, ...] = (
    "c_calibration_as_scale_id",
    "c_bake_specialized_only",
    "c_restream_in_study",
)

COMPILER_LOW_IDS: tuple[str, ...] = ("c_integrity_as_quality",)
...
SCHEMA_KEYS: tuple[str, ...] = (
    ...
    "n_compiler_output_kinds",
    "per_tensor_streaming_allowed",
    "validation_check_ids",
    "n_validation_checks",
    "validation_reads_payloads",
    "analysis_output_ids",
    "n_analysis_outputs",
    "analysis_cites_task05",
    "analysis_group_local_at_compile",
    "calibration_hook_without_corpus",
    "quantization_output_ids",
    "n_quantization_outputs",
    ...
    "n_decisions",
    "n_locked_structure",
    "n_unselected_winners",
    "decision_selected",
    "n_unselected_winners_selected",
    "n_architecture_decisions",
    "n_model_decisions",
    "n_calibration_decisions",
    "n_backend_decisions",
    "ownership_classified",
    "ledger_open_question_ownership_closed",
    "ownership_question_sentence",
...
SUBSTRING_ID_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("compiler stage", COMPILER_STAGE_IDS),
    ("compiler profile", COMPILER_PROFILE_IDS),
    ("ownership class", OWNERSHIP_CLASS_IDS),
    ("decision", DECISION_IDS),
    ("validation check", VALIDATION_CHECK_IDS),
    ("analysis output", ANALYSIS_OUTPUT_IDS),
    ("quantization output", QUANTIZATION_OUTPUT_IDS),
    ("compiler risk", COMPILER_RISK_IDS),
...
class ModelCompilerPlanMismatch(Exception):
    """Raised when the model-compiler-plan markdown fails a content check."""
...
def dumps_summary(summary: dict) -> str:
    """Pretty-print the model-compiler-plan summary object.

    Args:
        summary: Object produced by :func:`instantiate_model_compiler_plan`.
...
def instantiate_model_compiler_plan(config: dict) -> dict:
    """Compute the TASK-10 model-compiler-plan summary from ``config``.
    ...
    decision_ownership = {
        decision_id: owner
        for decision_id, owner in zip(DECISION_IDS, DECISION_OWNERSHIP_VALUES)
    }
    decision_selected = {decision_id: False for decision_id in UNSELECTED_WINNER_IDS}
    ...
    summary = {
        ...
        "compiler_input_ids": list(COMPILER_INPUT_IDS),
        "n_compiler_inputs": len(COMPILER_INPUT_IDS),
        "compiler_output_kinds": list(ARTIFACT_KINDS),
        "n_compiler_output_kinds": len(ARTIFACT_KINDS),
        "per_tensor_streaming_allowed": True,
        "validation_check_ids": list(VALIDATION_CHECK_IDS),
        "n_validation_checks": len(VALIDATION_CHECK_IDS),
        "validation_reads_payloads": False,
        "analysis_output_ids": list(ANALYSIS_OUTPUT_IDS),
        "n_analysis_outputs": len(ANALYSIS_OUTPUT_IDS),
        "analysis_cites_task05": True,
        "analysis_group_local_at_compile": True,
        "calibration_hook_without_corpus": True,
        "quantization_output_ids": list(QUANTIZATION_OUTPUT_IDS),
        "n_quantization_outputs": len(QUANTIZATION_OUTPUT_IDS),
        ...
        "ownership_class_ids": list(OWNERSHIP_CLASS_IDS),
        "n_ownership_classes": len(OWNERSHIP_CLASS_IDS),
        "decision_ids": list(DECISION_IDS),
        "decision_ownership": decision_ownership,
        "decision_ownership_values": list(DECISION_OWNERSHIP_VALUES),
        "locked_structure_ids": list(LOCKED_STRUCTURE_IDS),
        "unselected_winner_ids": list(UNSELECTED_WINNER_IDS),
        "n_decisions": len(DECISION_IDS),
        "n_locked_structure": len(LOCKED_STRUCTURE_IDS),
        "n_unselected_winners": len(UNSELECTED_WINNER_IDS),
        "decision_selected": decision_selected,
        "n_unselected_winners_selected": 0,
        "n_architecture_decisions": DECISION_OWNERSHIP_VALUES.count("architecture"),
        "n_model_decisions": DECISION_OWNERSHIP_VALUES.count("model"),
        "n_calibration_decisions": DECISION_OWNERSHIP_VALUES.count("calibration"),
        "n_backend_decisions": DECISION_OWNERSHIP_VALUES.count("backend"),
        "ownership_classified": True,
        "ledger_open_question_ownership_closed": True,
        "ownership_question_sentence": OWNERSHIP_QUESTION_SENTENCE,
        ...
        "compiler_risk_ids": list(COMPILER_RISK_IDS),
        "compiler_risk_severities": list(COMPILER_RISK_SEVERITIES),
        "compiler_high_ids": list(COMPILER_HIGH_IDS),
        "compiler_medium_ids": list(COMPILER_MEDIUM_IDS),
        "compiler_low_ids": list(COMPILER_LOW_IDS),
        "n_compiler_risks": len(COMPILER_RISK_IDS),
        "n_compiler_high": len(COMPILER_HIGH_IDS),
        "n_compiler_medium": len(COMPILER_MEDIUM_IDS),
        "n_compiler_low": len(COMPILER_LOW_IDS),
        ...
        "canonical_sentence_stages": CANONICAL_SENTENCE_STAGES,
        "canonical_sentence_profiles": CANONICAL_SENTENCE_PROFILES,
        "canonical_sentence_ownership": CANONICAL_SENTENCE_OWNERSHIP,
    }
...
def _assert_identities(summary: dict, text: dict) -> None:
    """Assert dossier identities on a live model-compiler-plan object.
    ...
    assert summary["cit_embed_n_zero"] == 7528
    assert summary["cit_conv1d_frac_out_6x"] == CIT_CONV1D_FRAC_OUT_6X
    assert summary["n_compiler_stages"] == 6
    assert summary["n_compiler_profiles"] == 3
    assert summary["n_ownership_classes"] == 4
    assert summary["n_decisions"] == 16
    assert summary["n_locked_structure"] == 8
    assert summary["n_unselected_winners"] == 8
    assert summary["n_unselected_winners_selected"] == 0
    assert summary["n_architecture_decisions"] == 9
    assert summary["n_model_decisions"] == 1
    assert summary["n_calibration_decisions"] == 3
    assert summary["n_backend_decisions"] == 3
    assert summary["n_validation_checks"] == 7
    assert summary["n_analysis_outputs"] == 5
    assert summary["n_quantization_outputs"] == 5
    assert summary["n_compiler_risks"] == 6
    assert summary["n_compiler_high"] == 2
    assert summary["n_compiler_medium"] == 3
    assert summary["n_compiler_low"] == 1
    assert summary["n_candidate_recipes"] == 22
    assert summary["n_policy_families"] == 16
    assert summary["n_packing_capabilities"] == 8
    assert summary["n_compiler_output_kinds"] == 6
    ...
    assert summary["decision_ownership"]["source_weight_statistics"] == "model"
    assert summary["decision_ownership"]["profile_recipe_map"] == "calibration"
    assert summary["decision_ownership"]["specialized_tile_view"] == "backend"
    assert summary["decision_ownership"]["stage_pipeline"] == "architecture"
    assert summary["decision_ownership"]["integrity_algorithm"] == "architecture"
    assert all(value is False for value in summary["decision_selected"].values())
    assert summary["compiler_profile_selected"] is False
    assert summary["integrity_algorithm_selected"] is False
    ...
    assert summary["ownership_classified"] is True
    assert summary["ledger_open_question_ownership_closed"] is True
    ...
    assert summary["validation_reads_payloads"] is False
    ...
    assert summary["decision_ownership_values"] == list(DECISION_OWNERSHIP_VALUES)
    assert summary["compiler_stage_ids"] == list(COMPILER_STAGE_IDS)
    assert summary["stage_order_strict"] is True
    assert summary["compiler_input_ids"] == list(COMPILER_INPUT_IDS)
    assert summary["compiler_output_kinds"] == list(ARTIFACT_KINDS)
    assert summary["per_tensor_streaming_allowed"] is True
    assert summary["validation_check_ids"] == list(VALIDATION_CHECK_IDS)
    assert summary["analysis_output_ids"] == list(ANALYSIS_OUTPUT_IDS)
    assert summary["analysis_cites_task05"] is True
    assert summary["analysis_group_local_at_compile"] is True
    assert summary["calibration_hook_without_corpus"] is True
    assert summary["quantization_output_ids"] == list(QUANTIZATION_OUTPUT_IDS)
    assert summary["packing_capability_ids"] == list(PACKING_CAPABILITY_IDS)
    assert summary["integrity_algorithm_candidates"] == list(
        INTEGRITY_ALGORITHM_CANDIDATES
    )
    assert summary["integrity_stage_emits_record"] is True
    assert summary["ownership_class_ids"] == list(OWNERSHIP_CLASS_IDS)
    assert summary["decision_ids"] == list(DECISION_IDS)
    assert summary["locked_structure_ids"] == list(LOCKED_STRUCTURE_IDS)
    assert summary["unselected_winner_ids"] == list(UNSELECTED_WINNER_IDS)
    assert summary["decision_selected"] == {
        decision_id: False for decision_id in UNSELECTED_WINNER_IDS
    }
    assert summary["compiler_profile_ids"] == list(COMPILER_PROFILE_IDS)
    assert summary["control_profile_id"] == "control"
    assert summary["quality_high_ids"] == list(QUALITY_HIGH_IDS)
    assert summary["compiler_risk_ids"] == list(COMPILER_RISK_IDS)
    assert summary["compiler_risk_severities"] == list(COMPILER_RISK_SEVERITIES)
    assert summary["compiler_high_ids"] == list(COMPILER_HIGH_IDS)
    assert summary["compiler_medium_ids"] == list(COMPILER_MEDIUM_IDS)
    assert summary["compiler_low_ids"] == list(COMPILER_LOW_IDS)
    high_from_ops = [
        risk_id
        for risk_id, severity in zip(
            summary["compiler_risk_ids"], summary["compiler_risk_severities"]
        )
        if severity == "high"
    ]
    medium_from_ops = [
    ...
    assert high_from_ops == summary["compiler_high_ids"]
    assert medium_from_ops == summary["compiler_medium_ids"]
    assert low_from_ops == summary["compiler_low_ids"]
    ...
    assert summary["vision_compile_deferred"] is True
    assert summary["diagram_ids"] == list(DIAGRAM_REQUIRED_IDS)
    assert summary["canonical_sentence_logical"] == CANONICAL_SENTENCE_LOGICAL
    assert summary["canonical_sentence_stages"] == CANONICAL_SENTENCE_STAGES
    assert summary["canonical_sentence_profiles"] == CANONICAL_SENTENCE_PROFILES
    assert summary["canonical_sentence_ownership"] == CANONICAL_SENTENCE_OWNERSHIP
    assert summary["ownership_question_sentence"] == OWNERSHIP_QUESTION_SENTENCE
    ...
    profile_key_index = list(summary).index("compiler_profile_selected")
    control_key_index = list(summary).index("control_profile_is_keep_source")
    assert profile_key_index == control_key_index + 1
    assert list(summary).count("compiler_profile_selected") == 1
...
def diff_summary(live: dict, documented: dict) -> list[str]:
    """Compare documented JSON against a live model-compiler-plan object.
...
def _winner_phrase_allowed(text: str, start: int, phrase: str) -> bool:
    ...
    if phrase == "selected profile":
        allowed = text[max(0, start - 6) : end]
        return allowed == "not a selected profile"
    return False


def check_model_compiler_plan(live: dict, design_path: Path) -> None:
    """Check a model-compiler-plan markdown file against a live summary.

    Args:
        live: Model-compiler-plan summary object from sitting config.
        design_path: Path to ``model-compiler-plan.md``.

    Raises:
        ModelCompilerPlanMismatch: On heading, JSON, diagram, or token
            mismatches.
    ...
    if CANONICAL_SENTENCE_STAGES not in text:
        differences.append("canonical_sentence_stages not present verbatim")
    if CANONICAL_SENTENCE_PROFILES not in text:
        differences.append("canonical_sentence_profiles not present verbatim")
    if CANONICAL_SENTENCE_OWNERSHIP not in text:
        differences.append("canonical_sentence_ownership not present verbatim")
    if OWNERSHIP_QUESTION_SENTENCE not in text:
        differences.append("ownership_question_sentence not present verbatim")
    ...
    if differences:
        raise ModelCompilerPlanMismatch(differences)


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    """Parse CLI arguments for the model-compiler-plan checker.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Instantiate Qwen3.8-27B language+MTP model-compiler-plan "
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Write model-compiler-plan summary JSON to stdout.",
    )
    parser.add_argument(
        "--model-compiler-plan",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    """Run the model-compiler-plan checker CLI.
    ...
    try:
        ...
        summary = instantiate_model_compiler_plan(config)
        if args.model_compiler_plan is not None:
            if not args.model_compiler_plan.is_file():
                print(
                    f"model-compiler-plan file not found: "
                    f"{args.model_compiler_plan}",
                    file=sys.stderr,
                )
                return 1
            check_model_compiler_plan(summary, args.model_compiler_plan)
    except MissingConfig as exc:
        print(str(exc.path), file=sys.stderr)
        return 2
    except ModelCompilerPlanMismatch as exc:
    ...
    if args.json or args.model_compiler_plan is None:
