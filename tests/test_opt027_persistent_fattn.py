...
PREFIX = "QW38_PREFILL_4K_PERSISTENT_FATTN_RESULT="
CONTRACT = ROOT / "pins/opt027_persistent_fattn_contract.json"
FIXTURE = ROOT / "fixtures/opt027_persistent_fattn.json"
EVIDENCE = ROOT / "evidence/optimization/opt027-persistent-fattn"
...
PROOF = (
    "persistent Ada+ fattn stream-K; OPT-005 envelopes unloosened; "
...
def selected_persistent_from_source() -> str:
    text = (ROOT / "cuda/fattn_mma_f16.cuh").read_text()
    match = re.search(r'kSelectedPersistentFattnPath\[\] = "([^"]+)"', text)
    assert match is not None
    return match.group(1)
...
def production_persistent_installed() -> bool:
    return (
        selected_persistent_from_source() == "persistent" and scheduler_uses_stream_k()
    )


def production_persistent_off() -> bool:
    return selected_persistent_from_source() == "off"
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-027"
    ...
    assert (
        result["selected_persistent_fattn_path"]
        == contract["selected_persistent_fattn_path"]
    )
    assert result["ab_winner"] in ("stream_k", "persistent")
    ...
    keep = (
        ...
        and result["ab_winner"] == "persistent"
    )
    if result["status"] == "measured":
        assert keep
        assert result["reverted"] is False
        assert result["successor_oracle"] is True
        assert result["production_persistent_installed"] is True
        assert result["selected_persistent_fattn_path"] == "persistent"
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_persistent_installed"] is False
        assert result["selected_persistent_fattn_path"] == "off"
...
def test_opt027_contract_and_fixture_are_connected() -> None:
    ...
    assert (
        selected_persistent_from_source() == contract["selected_persistent_fattn_path"]
    )
    assert skinny_path_unchanged()
    assert ffn_shared_y_unchanged()
    assert scheduler_uses_stream_k()
    if contract["selected_persistent_fattn_path"] == "persistent":
        assert production_persistent_installed()
    else:
        assert production_persistent_off()


def test_opt027_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("opt026_quartz_mean_tok_s", 1.0),
        lambda x: x.__setitem__("report_path", "/tmp/opt027/REPORT.md"),
        ...
        lambda x: x.__setitem__("selected_fattn_path", "occ2"),
    ):
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-persistent-fattn-test",
    ]
    commands = [
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-027 — Persistent Ada+ fattn stream-K
...
`selected_persistent_fattn_path`={fixture["selected_persistent_fattn_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_persistent_installed`={json.dumps(fixture["production_persistent_installed"])};
...
| Nsight | not_used |
| Keep/reject | strictly greater Quartz mean tok/s than {OPT026_MEAN} and A/B persistent win |
...
def test_opt027_native_4k_keep_reject() -> None:
    ...
    ab_winner, ab_win = (
        _parse_ab(AB_RAW.read_text()) if AB_RAW.is_file() else ("stream_k", False)
    )
    if ab_win:
        assert production_persistent_installed(), (
            "4K sitting must run on the winning persistent path"
        )
        assert selected_persistent_from_source() == "persistent"
        assert ab_winner == "persistent"
    else:
        assert production_persistent_off()
    assert selected_fattn_path_from_source() == "stream_k"
    assert scheduler_uses_stream_k()
    ...
    keep = mean_tok_s > OPT026_MEAN and ab_win and ab_winner == "persistent"
    path = "persistent" if keep else "off"
    fixture = {
        "schema_version": 1,
        "task": "OPT-027",
        ...
        "selected_persistent_fattn_path": path,
        "ab_winner": ab_winner,
        "ab_win": ab_win,
        "production_persistent_installed": True if keep else False,
        ...
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt027-persistent-fattn/REPORT.md",
    }
    assert quartz["graphs_created"] is True
    assert quartz["prompt_graph_rows"] == 4096
    assert quartz["prompt_tokens"] == 4096
