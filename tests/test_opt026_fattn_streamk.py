...
PREFIX = "QW38_PREFILL_4K_FATTN_RESULT="
CONTRACT = ROOT / "pins/opt026_fattn_streamk_contract.json"
FIXTURE = ROOT / "fixtures/opt026_fattn_streamk.json"
EVIDENCE = ROOT / "evidence/optimization/opt026-fattn-streamk"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
LLAMA_JSON = EVIDENCE / "llama-bench-4k.json"
QUARTZ_JSON = EVIDENCE / "quartz-4k.json"
AB_RAW = EVIDENCE / "fattn-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
LLAMA_BENCH = ROOT / ".cache/authorities/llama-build/bin/llama-bench"
OPT025_MEAN = 1709.21912
OPTIMIZED_IDS = ("occ2", "stream_k")
PROOF = (
    "fattn occupancy / stream-K; OPT-005 envelopes unloosened; "
    "4K keep/reject; envelopes unloosened; "
    "does not substitute for the 2K llama.cpp parity gate; "
    "tiled attention remains the reference"
)
...
def scheduler_uses_stream_k() -> bool:
    text = (ROOT / "cuda/full_scheduler.cu").read_text()
    return "launch_attention_prepare_chunk_stream_k" in text


def production_is_optimized() -> bool:
    path = selected_path_from_source()
    if path == "stream_k":
        return scheduler_uses_stream_k()
    if path == "occ2":
        return True
    return False
...
def production_is_baseline() -> bool:
    return selected_path_from_source() == "baseline"
...
def decode_attention_unchanged() -> bool:
    text = (ROOT / "cuda/attention_decode.cu").read_text()
    return "return launch_attention_prepare_chunk(" in text
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-026"
    ...
    assert contract["opt025_quartz_mean_tok_s"] == OPT025_MEAN
    ...
    assert result["opt025_quartz_mean_tok_s"] == OPT025_MEAN
    assert result["selected_fattn_path"] == contract["selected_fattn_path"]
    assert result["ab_winner"] in ("baseline", "occ2", "stream_k")
    assert result["ladder_exhausted"] is True
    ...
    assert result["decode_attention_unchanged"] is True
    keep = (
        float(quartz["mean_tok_s"]) > OPT025_MEAN
    ...
    if result["status"] == "measured":
        ...
        assert result["production_fattn_optimized"] is True
        assert result["selected_fattn_path"] in OPTIMIZED_IDS
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_fattn_optimized"] is False
        assert result["selected_fattn_path"] == "baseline"
        assert REJECTION.is_file()
        rejection = REJECTION.read_text()
        assert str(quartz["mean_tok_s"]) in rejection
        assert str(OPT025_MEAN) in rejection
...
def test_opt026_contract_and_fixture_are_connected() -> None:
    ...
    opt025 = json.loads((ROOT / "fixtures/opt025_ffn_shared_y.json").read_text())
    assert opt025["quartz"]["mean_tok_s"] == OPT025_MEAN
    assert contract["opt025_quartz_mean_tok_s"] == OPT025_MEAN
    pin = json.loads((ROOT / "pins/opt019_core_recovery_contract.json").read_text())
    assert pin["attention_mma_query_rows_2048"] == 16
    ...
    assert selected_path_from_source() == contract["selected_fattn_path"]
    assert skinny_path_unchanged()
    assert ffn_shared_y_unchanged()
    if contract["selected_fattn_path"] == "stream_k":
        assert scheduler_uses_stream_k()


def test_opt026_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("opt025_quartz_mean_tok_s", 1.0),
        lambda x: x.__setitem__("report_path", "/tmp/opt026/REPORT.md"),
        lambda x: x.__setitem__("skinny_mixer_unchanged", False),
        lambda x: x.__setitem__("shared_residual_y", False),
        lambda x: x.__setitem__("decode_attention_unchanged", False),
        lambda x: x.__setitem__("ladder_exhausted", False),
    ):
    ...
    if fixture["status"] == "measured":
        ...
        keep_break["quartz"]["mean_tok_s"] = OPT025_MEAN
        keep_break["quartz"]["tok_s"] = [OPT025_MEAN] * 3
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-fattn-test",
    ]
    commands = [
...
def _parse_ab(raw_text: str) -> tuple[str, bool]:
    ...
    for line in raw_text.splitlines():
        if not line.startswith("fattn_ab_winner "):
            continue
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-026 — Attention fattn stream-K occupancy
...
{OPT025_MEAN} tok/s. Numeric and exact-state **envelopes unloosened**. This
...
**{decision}** — Quartz mean tok/s {quartz["mean_tok_s"]} versus OPT-025 baseline {OPT025_MEAN}.
A/B winner `{fixture["ab_winner"]}` win={json.dumps(fixture["ab_win"])};
`selected_fattn_path`={fixture["selected_fattn_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_fattn_optimized`={json.dumps(fixture["production_fattn_optimized"])};
...
| Nsight | not_used |
| Keep/reject | strictly greater Quartz mean tok/s than {OPT025_MEAN} and A/B optimized win |
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    quartz = fixture["quartz"]
    text = f"""# OPT-026 rejection

Attention fattn stream-K / occupancy fixup was measured on exclusive RTX 5090
and did not strictly beat occupancy-1 whole-tile, or the 4K oracle did not
strictly beat the OPT-025 Quartz baseline.

- quality sitting mean tok/s: {quartz["mean_tok_s"]}
- OPT-025 baseline: {OPT025_MEAN}
...
- production prompt fattn remains occupancy-1 whole-tile
- successor_oracle: false (denominator remains {OPT025_MEAN})
- ladder_exhausted: true
- measurement_utc: {fixture["measurement_utc"]}
"""
...
def test_opt026_native_4k_keep_reject() -> None:
    ...
    if FIXTURE.is_file():
        ...
        if existing.get("status") == "rejected" and existing.get("reverted") is True:
            ...
            assert skinny_path_unchanged()
            assert ffn_shared_y_unchanged()
            assert decode_attention_unchanged()
            validate_result(existing)
            return

    ab_winner, ab_win = (
        _parse_ab(AB_RAW.read_text()) if AB_RAW.is_file() else ("baseline", False)
    )
    if ab_win:
        assert production_is_optimized(), (
            "4K sitting must run on the winning fattn path"
        )
    ...
    keep = mean_tok_s > OPT025_MEAN and ab_win and ab_winner in OPTIMIZED_IDS
    path = ab_winner if keep else "baseline"
    fixture = {
        "schema_version": 1,
        "task": "OPT-026",
        ...
        "opt025_quartz_mean_tok_s": OPT025_MEAN,
        ...
        "ladder_exhausted": True,
        "selected_fattn_path": path,
        "ab_winner": ab_winner,
        "ab_win": ab_win,
        "production_fattn_optimized": True if keep else False,
        "shared_residual_y": True,
        "skinny_mixer_unchanged": True,
        "ffn_shared_y_unchanged": True,
        "decode_attention_unchanged": True,
        "nsight_systems": "not_used",
        "nsight_compute": "not_used",
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt026-fattn-streamk/REPORT.md",
    }
    assert quartz["graphs_created"] is True
    assert quartz["prompt_graph_rows"] == 4096
    assert quartz["prompt_tokens"] == 4096
