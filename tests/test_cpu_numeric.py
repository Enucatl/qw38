...
REAL_STEPS = (
    ...
    ("--check-real-attention-step", ROOT / "fixtures" / "cpu_real_attention_step_2b.json"),
...
def test_avx2_q8_activation_matvec_stays_inside_frozen_scalar_envelope() -> None:
    ...
    q4 = bytes.fromhex(
        json.loads((ROOT / "fixtures" / "quant_authority.json").read_text())["cases"][0][
            "block_hex"
        ]
    )
    q5 = bytes.fromhex(json.loads(Q5_FIXTURES.read_text())["cases"][0]["block_hex"])
    q6 = bytes.fromhex(
        json.loads((ROOT / "fixtures" / "quant_authority.json").read_text())["cases"][2][
            "block_hex"
        ]
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for case in document["cases"]:
            ...
            left = struct.unpack(
                f"<{rows}f", bytes.fromhex(parse_output(avx.stdout)["output_f32_le_hex"])
            )
            right = struct.unpack(
                f"<{rows}f",
...
@pytest.mark.skipif(not LLAMA.exists(), reason="cpu_llama_scalar_authority.json is generated by the Darwin CPU llama.cpp oracle")
