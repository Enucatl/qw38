...
PREFIX = "QW38_PREFILL_4K_FFN_RESULT="
CONTRACT = ROOT / "pins/opt025_ffn_shared_y_contract.json"
FIXTURE = ROOT / "fixtures/opt025_ffn_shared_y.json"
EVIDENCE = ROOT / "evidence/optimization/opt025-ffn-shared-y"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
LLAMA_JSON = EVIDENCE / "llama-bench-4k.json"
QUARTZ_JSON = EVIDENCE / "quartz-4k.json"
AB_RAW = EVIDENCE / "ffn-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
LLAMA_BENCH = ROOT / ".cache/authorities/llama-build/bin/llama-bench"
OPT023_MEAN = 1687.86169
OPTIMIZED_IDS = ("shared_y", "swiglu_q8", "shared_y_swiglu_q8")
PROOF = (
    "dense FFN shared-Y and SwiGLU-into-down Q8; Q4_K association; "
...
def scheduler_uses_ffn_helper() -> bool:
    text = (ROOT / "cuda/full_scheduler.cu").read_text()
    return "execute_prompt_ffn_projections" in text


def production_is_optimized() -> bool:
    return selected_path_from_source() in OPTIMIZED_IDS and scheduler_uses_ffn_helper()


def production_is_baseline() -> bool:
    return selected_path_from_source() == "baseline" and scheduler_uses_ffn_helper()
...
def large_mixer_quality_mma() -> bool:
    text = (ROOT / "cuda/quant_mmq_mma.cuh").read_text()
...
def decode_ffn_unchanged() -> bool:
    ...
    return (
        ...
        in scheduler
        and "error = launch_swiglu_bf16(" in scheduler
        and "error = matrix_vector(layer.ffn_down, workspace->ffn_activated_, workspace,"
        in scheduler
    )
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-025"
    ...
    assert result["selected_ffn_path"] == contract["selected_ffn_path"]
    ...
    if result["status"] == "measured":
        ...
        assert result["production_ffn_optimized"] is True
        assert result["selected_ffn_path"] in OPTIMIZED_IDS
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_ffn_optimized"] is False
        assert result["selected_ffn_path"] == "baseline"
...
def test_opt025_contract_and_fixture_are_connected() -> None:
    ...
    assert selected_path_from_source() == contract["selected_ffn_path"]
    assert skinny_path_unchanged()
    assert large_mixer_quality_mma()
    assert scheduler_uses_ffn_helper()


def test_opt025_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("report_path", "/tmp/opt025/REPORT.md"),
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-ffn-test",
    ]
    commands = [
...
def _parse_ab(raw_text: str) -> tuple[str, bool]:
    ...
    for line in raw_text.splitlines():
        if not line.startswith("ffn_ab_winner "):
            continue
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-025 — Dense FFN shared-Y and SwiGLU-into-down Q8
...
`selected_ffn_path`={fixture["selected_ffn_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_ffn_optimized`={json.dumps(fixture["production_ffn_optimized"])}.
...
| Nsight | not_used |
| Keep/reject | strictly greater Quartz mean tok/s than {OPT023_MEAN} and A/B optimized win |
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    quartz = fixture["quartz"]
    text = f"""# OPT-025 rejection

Dense FFN shared-Y and SwiGLU-into-down Q8 was measured on exclusive RTX 5090
and did not strictly beat per-GEMM quantize, or the 4K oracle did not strictly
beat the OPT-023 Quartz baseline.
...
- production prompt FFN remains per-GEMM matrix_prompt plus BF16 SwiGLU
- successor_oracle: false (OPT-026+ denominator remains {OPT023_MEAN})
- measurement_utc: {fixture["measurement_utc"]}
"""
    REJECTION.write_text(text)


def test_opt025_native_4k_keep_reject() -> None:
    ...
    if ab_win:
        assert production_is_optimized(), "4K sitting must run on the winning FFN path"
    ...
    keep = mean_tok_s > OPT023_MEAN and ab_win and ab_winner in OPTIMIZED_IDS
    ...
    fixture = {
        ...
        "selected_ffn_path": path,
        "ab_winner": ab_winner,
        "ab_win": ab_win,
        "production_ffn_optimized": True if keep else False,
        ...
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt025-ffn-shared-y/REPORT.md",
    }
    assert quartz["graphs_created"] is True
    assert quartz["prompt_graph_rows"] == 4096
    assert quartz["prompt_tokens"] == 4096
