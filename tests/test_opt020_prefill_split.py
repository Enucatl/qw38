...
CONTRACT = ROOT / "pins/opt020_prefill_split_contract.json"
FIXTURE = ROOT / "fixtures/opt020_prefill_split.json"
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-020"
    ...
    assert "mixer versus core split" in proof
    categories = result["categories_ms"]
    assert isinstance(categories, dict) and set(categories) == set(CATEGORIES)
    assert "gdn" not in categories and "attention" not in categories
    for name in CATEGORIES:
        value = categories[name]
        assert isinstance(value, (int, float))
        assert value >= 0.0
    assert categories["graph"] >= 0.0
    assert categories["mixer_mmq"] > 0.0
    assert categories["gdn_core"] > 0.0
    assert categories["attention_core"] > 0.0
...
def _write_fixture(result: dict[str, Any]) -> None:
...
def test_opt020_contract_and_fixture_are_connected() -> None:
    contract = _contract()
    assert contract["task"] == "OPT-020"
    assert contract["prompt_tokens"] == 2048
    assert contract["capacity"] == 131072
    assert contract["chunk_rows"] == 4096
    assert contract["abs_tol_ms"] == 0.05
    assert contract["rel_tol"] == 1e-4
    assert contract["graph_at_2048"] == "measured_zero"
    assert contract["nsight"] == "not_required"
    assert contract["detailed_mode"] == "opt-in"
    assert contract["owns_opt016_parity_gate"] is False
    assert contract["historical_opt014_fixture"] == (
        "fixtures/cuda_prefill_attribution.json"
    )
    assert set(contract["categories"]) == set(CATEGORIES)
    validate_result(json.loads(FIXTURE.read_text()))
    chapter = (ROOT / "docs" / "51-runtime-timing-and-nvtx.md").read_text().casefold()
    for term in [
        "mixer-projection mmq",
        "gdn_core",
        "attention_core",
...
def test_opt020_validator_rejects_inadmissible_evidence() -> None:
    fixture = json.loads(FIXTURE.read_text())

    def restore_composite(payload: dict[str, Any]) -> None:
        payload["categories_ms"].pop("mixer_mmq")
        payload["categories_ms"].pop("gdn_core")
        payload["categories_ms"].pop("attention_core")
        payload["categories_ms"]["gdn"] = 1.0
        payload["categories_ms"]["attention"] = 1.0

    mutations = []
    for mutate in (
        lambda x: x["categories_ms"].pop("mixer_mmq"),
        restore_composite,
        ...
        lambda x: x.__setitem__("task", "OPT-014"),
        lambda x: x.__setitem__("driver", "placeholder"),
        lambda x: x.__setitem__(
            "proof_limit",
            "live CUDA-event attribution; 2048-token cold prefill; "
...
def test_opt020_prefill_split_native() -> None:
...
if __name__ == "__main__":
    if sys.argv[1:] != ["--regenerate-fixture"]:
        raise SystemExit("usage: test_opt020_prefill_split.py --regenerate-fixture")
    _regenerate()
