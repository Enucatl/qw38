"""Host tests for OPT-137 long-context dense MMA decode attention."""
...
from tools.opt137_long_attention import (
    CANDIDATE,
    CONTRACT,
    DECODE_TOKENS,
    FIXTURE,
...
NATIVE_SRC = ROOT / "cuda/opt137_long_attention_test.cu"
KERNEL = ROOT / "cuda/opt137_dense_mma_decode.cuh"
...
RUNNER = ROOT / "tools/opt137_long_attention.py"
TASK_RUNNER = ROOT / "tools/run_optimization_task.py"
PROVENANCE = ROOT / "pins/opt137_long_attention_provenance.json"
...
def test_contracts_makefile_and_native_hooks() -> None:
    ...
    iteration = load_iteration("OPT-137")
    ...
    assert iteration["diagnostics_make_target"] == "cuda-opt137-diagnostics"
    assert iteration["aggregate_deadline_s"] == 7200
    assert "cuda-opt137-diagnostics" in makefile
    assert "qw38-cuda-opt137-long-attention-test" in makefile
    ...
    assert "QW38_OPT137_LONG_ATTENTION_RESULT=" in native
    assert "QW38_OPT137_NATIVE_COUNTS=" in native
    assert "QW38_OPT137_NATIVE_COUNTS=" in task_runner
    assert "QW38_OPT137_LONG_ATTENTION_RESULT=" in task_runner
    ...
    assert "apply_opt137_dense_mma_ident" in quality
    assert "effective_decode_graph_topology_count" in scheduler
    assert "opt137_dense_mma_decode.cuh" in attn
    assert "dense_bf16_tile_f16_mma_decode" in kernel
    assert "__float2half_rn(__bfloat162float" in kernel
    assert "kSelectedOpt137DenseMma" in pin
    assert "kLegalDecodeAttentionDenseMma" in pin
    ...
    validate_future_keep_policy("OPT-137", iteration)
...
def test_keep_policy_roles_and_quality_wiring() -> None:
    ...
    assert REPORT.parent.as_posix().endswith("opt137-long-attention")
    assert FIXTURE.name == "opt137_long_attention.json"
...
def test_iteration_loop_products() -> None:
    ...
    described = describe_plan("OPT-137", "feedback", iteration, "preflight")
...
def test_pins_match_keep_state() -> None:
    ...
    if kept:
        ...
        assert "kSelectedOpt137DenseMma = false" in pin
    contract = load_contract()
