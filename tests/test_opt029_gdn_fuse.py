...
PREFIX = "QW38_PREFILL_4K_GDN_FUSE_RESULT="
CONTRACT = ROOT / "pins/opt029_gdn_fuse_contract.json"
FIXTURE = ROOT / "fixtures/opt029_gdn_fuse.json"
EVIDENCE = ROOT / "evidence/optimization/opt029-gdn-fuse"
...
OPTIMIZED_IDS = ("fuse_conv", "fuse_gate", "fuse_both")
PROOF = (
    "fused GDN conv/gated-output; GDN-002 envelopes; "
    "4K keep/reject; envelopes unloosened; "
    "does not substitute for the 2K llama.cpp parity gate; "
    "sequential GDN-002 reference retained"
)
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-029"
    ...
    assert result["selected_gdn_fuse_path"] == contract["selected_gdn_fuse_path"]
    ...
    assert result["ab_winner"] in ("off", "fuse_conv", "fuse_gate", "fuse_both")
    ...
    assert result["decode_gdn_unchanged"] is True
    keep = (
    ...
    if result["status"] == "measured":
        ...
        assert result["production_gdn_fuse_installed"] is True
        assert result["selected_gdn_fuse_path"] in OPTIMIZED_IDS
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_gdn_fuse_installed"] is False
        assert result["selected_gdn_fuse_path"] == "off"
...
def test_opt029_contract_and_fixture_are_connected() -> None:
    ...
    assert selected_path_from_source() == contract["selected_gdn_fuse_path"]
...
def test_opt029_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x["quartz"].__setitem__("attribution", {"gdn_core": 1.0}),
        ...
        lambda x: x.__setitem__("report_path", "/tmp/opt029/REPORT.md"),
        lambda x: x.__setitem__("skinny_mixer_unchanged", False),
        lambda x: x.__setitem__("shared_residual_y", False),
        lambda x: x.__setitem__("decode_gdn_unchanged", False),
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-gdn-fuse-test",
    ]
    commands = [
...
def _parse_ab(raw_text: str) -> tuple[str, bool]:
    ...
    for line in raw_text.splitlines():
        if not line.startswith("gdn_ab_winner "):
            continue
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-029 — Fuse GDN conv and gated output
...
`selected_gdn_fuse_path`={fixture["selected_gdn_fuse_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_gdn_fuse_installed`={json.dumps(fixture["production_gdn_fuse_installed"])};
...
def test_opt029_native_4k_keep_reject() -> None:
    ...
    if ab_win:
        assert production_is_optimized(), (
            "4K sitting must run on the winning GDN fuse path"
        )
