...
PREFIX = "QW38_PREFILL_4K_PDL_RESULT="
CONTRACT = ROOT / "pins/opt030_pdl_launches_contract.json"
FIXTURE = ROOT / "fixtures/opt030_pdl_launches.json"
EVIDENCE = ROOT / "evidence/optimization/opt030-pdl-launches"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
LLAMA_JSON = EVIDENCE / "llama-bench-4k.json"
QUARTZ_JSON = EVIDENCE / "quartz-4k.json"
AB_RAW = EVIDENCE / "pdl-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
LLAMA_BENCH = ROOT / ".cache/authorities/llama-build/bin/llama-bench"
OPT026_MEAN = 1746.71973
OPTIMIZED_IDS = ("pdl_host", "pdl")
PROOF = (
    "Hopper/Blackwell PDL serialization of ungraphed mixer/GDN/attention prompt launches; "
    "arithmetic and graph-vs-fused byte equality retained; "
...
def selected_path_from_source() -> str:
    text = (ROOT / "cuda/pdl_launch.cuh").read_text()
    match = re.search(r'kSelectedPdlPath\[\] = "([^"]+)"', text)
...
def selected_mmq_stream_k_from_source() -> str:
    text = (ROOT / "cuda/quant_mmq_mma.cuh").read_text()
...
def selected_gdn_fuse_from_source() -> str:
    text = (ROOT / "cuda/gdn_fused_quality.cuh").read_text()
    match = re.search(r'kSelectedGdnFusePath\[\] = "([^"]+)"', text)
    assert match is not None
    return match.group(1)
...
def mixer_quality_unchanged() -> bool:
    text = (ROOT / "cuda/quant_mmq_mma.cuh").read_text()
    return 'kSelectedLargeMixerQ8Path[] = "quality_mma"' in text
...
def decode_launches_unchanged() -> bool:
    text = (ROOT / "cuda/full_scheduler.cu").read_text()
    return (
        "rms_norm_fp32_to_bf16<<<1, kThreads, 0, stream>>>(" in text
        and "launch_gdn_prepare_tiled(" in text
        and "launch_attention_prepare(" in text
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-030"
    ...
    assert result["selected_pdl_path"] == contract["selected_pdl_path"]
    ...
    assert result["ab_winner"] in ("off", "pdl_host", "pdl")
    ...
    assert result["decode_launches_unchanged"] is True
    keep = (
    ...
    if result["status"] == "measured":
        ...
        assert result["production_pdl_installed"] is True
        assert result["selected_pdl_path"] in OPTIMIZED_IDS
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_pdl_installed"] is False
        assert result["selected_pdl_path"] == "off"
...
def test_opt030_contract_and_fixture_are_connected() -> None:
    ...
    opt028 = json.loads((ROOT / "fixtures/opt028_mmq_streamk.json").read_text())
    assert opt028["successor_oracle"] is False
    opt029 = json.loads((ROOT / "fixtures/opt029_gdn_fuse.json").read_text())
    assert opt029["successor_oracle"] is False
    assert selected_path_from_source() == contract["selected_pdl_path"]
    assert skinny_path_unchanged()
    assert mixer_quality_unchanged()
    assert ffn_shared_y_unchanged()
    assert selected_fattn_path_from_source() == "stream_k"
    assert selected_mmq_stream_k_from_source() == "off"
    assert selected_gdn_fuse_from_source() == "off"
    assert decode_launches_unchanged()


def test_opt030_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x["quartz"].__setitem__("attribution", {"attention_core": 1.0}),
        ...
        lambda x: x.__setitem__("opt026_quartz_mean_tok_s", 1729.0459),
        lambda x: x.__setitem__("opt026_quartz_mean_tok_s", 1725.36658),
        lambda x: x.__setitem__("report_path", "/tmp/opt030/REPORT.md"),
        lambda x: x.__setitem__("skinny_mixer_unchanged", False),
        lambda x: x.__setitem__("shared_residual_y", False),
        lambda x: x.__setitem__("decode_launches_unchanged", False),
        ...
        lambda x: x.__setitem__("selected_mmq_stream_k_path", "stream_k"),
        lambda x: x.__setitem__("selected_pdl_path", "cooperative"),
    ):
...
def _nvcc_objects(source: str, output: str) -> list[str]:
    return [
        *_common(IMAGE),
        "nvcc",
        "-std=c++17",
...
def _pin_selected_path(path: str) -> None:
    source = ROOT / "cuda/pdl_launch.cuh"
    text = source.read_text()
    updated, count = re.subn(
        r'constexpr char kSelectedPdlPath\[\] = "[^"]+";',
        f'constexpr char kSelectedPdlPath[] = "{path}";',
        text,
        count=1,
    )
    assert count == 1
    ...
    contract = _contract()
    contract["selected_pdl_path"] = path
...
def _parse_ab(raw_text: str) -> tuple[str, bool]:
    ...
    for line in raw_text.splitlines():
        if not line.startswith("pdl_ab_winner "):
            continue
...
def _run_ab() -> tuple[str, bool]:
    EVIDENCE.mkdir(parents=True, exist_ok=True)
    commands = [
        [*_common(IMAGE), "make", "build/qw38-cuda-timing-test"],
        _nvcc_objects("cuda/pdl_launch_test.cu", "build/qw38-cuda-pdl-launch-test"),
        [
            *_common(IMAGE),
            "./build/qw38-cuda-pdl-launch-test",
    ...
    assert AB_RAW.is_file()
    return _parse_ab(AB_RAW.read_text())
...
def _run_quartz() -> dict[str, Any]:
    commands = [
        ...
        _nvcc_objects(
            ...
            "build/qw38-cuda-prefill-4k-pdl-test",
        ),
        [
            *_common(IMAGE),
            "./build/qw38-cuda-prefill-4k-pdl-test",
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-030 — Hopper/Blackwell PDL prompt launches
...
**4K keep/reject** versus the frozen OPT-026 Quartz mean {OPT026_MEAN} tok/s.
...
`selected_pdl_path`={fixture["selected_pdl_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_pdl_installed`={json.dumps(fixture["production_pdl_installed"])};
...
def test_opt030_native_4k_keep_reject() -> None:
    ...
    ab_winner, ab_win = _run_ab()
    if ab_win:
        assert ab_winner in OPTIMIZED_IDS
        _pin_selected_path(ab_winner)
        assert production_is_optimized()
        assert selected_path_from_source() == ab_winner
    else:
        _pin_selected_path("off")
        assert production_is_baseline()
    assert skinny_path_unchanged()
    assert mixer_quality_unchanged()
    assert ffn_shared_y_unchanged()
    ...
    if not keep and selected_path_from_source() != "off":
        _pin_selected_path("off")
    fixture = {
        "schema_version": 1,
        "task": "OPT-030",
