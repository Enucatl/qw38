...
CONTRACT = ROOT / "pins/opt047_q8_decode_contract.json"
FIXTURE = ROOT / "fixtures/opt047_q8_decode.json"
...
EVIDENCE = ROOT / "evidence/optimization/opt047-q8-decode"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
AB_RAW = EVIDENCE / "q8-decode-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
AB_PREFIX = "QW38_OPT047_Q8_DECODE_AB_RESULT="
...
LEGAL_PATHS = {"direct_bf16", "dp4a_q8_1"}
LEGAL_CANDIDATES = (
    "direct_bf16",
    "dp4a_w1",
    "dp4a_w2",
...
PROOF = (
    "OPT-044 production-numerics budgets vs original BF16; "
    "direct-BF16 q8_mmv_bf16 retained as strict reference; "
    "Q8_0 x Q8_1 DP4A with OPT-046 Q8_1 staging; "
    "shared mixer-input staging, output restaged; "
    "OPT-046 packed Q4_K decode retained; "
    "complete mixer group including staging; "
    "95% throughput floors versus OPT-045; "
    "105% p95 ceilings versus OPT-045; "
...
def pin_from_source() -> str:
    text = (ROOT / "cuda/q8_decode_path.cuh").read_text()
    match = re.search(r'kSelectedQ8DecodePath\[\] = "([^"]+)"', text)
    assert match is not None
    return match.group(1)


def warps_from_source() -> tuple[int, int, int]:
    text = (ROOT / "cuda/q8_decode_path.cuh").read_text()
    skinny = re.search(r"kSelectedQ8DecodeWarpsSkinny = ([0-9]+)", text)
    medium = re.search(r"kSelectedQ8DecodeWarpsMedium = ([0-9]+)", text)
    wide = re.search(r"kSelectedQ8DecodeWarpsWide = ([0-9]+)", text)
    assert skinny and medium and wide
    return int(skinny.group(1)), int(medium.group(1)), int(wide.group(1))


def direct_bf16_retained() -> bool:
    text = (ROOT / "cuda/full_scheduler.cu").read_text()
    return (
        "__global__ void q8_mmv_bf16" in text
        and "quartz_launch_kernel(" in text
        and "q8_mmv_bf16, dim3(blocks), dim3(kThreads)" in text
        and "__fadd_rn(sum," in text
        and "__bfloat162float(activation[column])" in text
    )


def dp4a_design_present() -> bool:
    header = (ROOT / "cuda/q8_decode_dots.cuh").read_text()
    scheduler = (ROOT / "cuda/full_scheduler.cu").read_text()
    return (
        "__dp4a" in header
        and "q8_coop_mmv" in header
        and "vec_dot_q8_0_q8_1" in header
        and "WarpsPerRow" in header
        and "launch_q8_coop_mmv_prequant" in scheduler
        and "q8_decode_uses_dp4a_for_rows" in scheduler
        and "q8_decode_staged_activation_" in scheduler
    )


def packed_q4_stays() -> bool:
    path = (ROOT / "cuda/q4k_decode_path.cuh").read_text()
    mmv = (ROOT / "cuda/quant_mmv.cu").read_text()
    return (
        'kSelectedQ4DecodePath[] = "packed"' in path
        and 'kSelectedMmvLoadPath[] = "packed"' in mmv
        and "__dp4a" not in mmv
    )
...
def _select_component_winner(block: dict[str, Any]) -> str:
    ...
    for ident in LEGAL_CANDIDATES:
        ...
        if not cand["eligible"]:
            continue
        mean = float(cand["weighted_group_ms"])
        if mean < best_mean:
            best = ident
            best_mean = mean
    return best
...
def _keep_predicates(result: dict[str, Any]) -> bool:
    contract = _contract()
    if result["selected_q8_decode_path"] == "direct_bf16":
        return False
    if result["reverted"] is True or result["keep_sitting_skipped"] is True:
        return False
    if _select_install(result) == "direct_bf16":
        return False
    if result["opt046_selected_q4_decode_path"] != "packed":
        return False
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-047"
    assert result["status"] in ("measured", "rejected")
    assert result["status"] not in ("scout", "source_inspected")
    assert contract["yardstick"] == "q8_0_mixer_decode_dp4a_shared_staging"
    ...
    path = result["selected_q8_decode_path"]
    assert path in LEGAL_PATHS
    for warps in (
        result["selected_q8_decode_warps_skinny"],
        result["selected_q8_decode_warps_medium"],
        result["selected_q8_decode_warps_wide"],
    ):
        assert warps in (0, 1, 2, 4, 8)
    assert contract["ab"]["candidates"] == list(LEGAL_CANDIDATES)
    assert contract["ab"]["ties_retain"] == "direct_bf16"
    assert _select_component_winner(result["ab"]) == result["ab"]["winner"]
    for ident in LEGAL_CANDIDATES:
        _ab_candidate(result["ab"], ident)
    packed = result["ab"]["candidates"]["direct_bf16"]
    assert packed["path"] == "direct_bf16"
    assert direct_bf16_retained()
    assert dp4a_design_present()
    assert packed_q4_stays()
    assert pin_from_source() == result["selected_q8_decode_path"]
    assert warps_from_source() == (
        result["selected_q8_decode_warps_skinny"],
        result["selected_q8_decode_warps_medium"],
        result["selected_q8_decode_warps_wide"],
    )
    ...
    if result["status"] == "measured":
        ...
        assert "Q8_1" in REPORT.read_text()
    else:
        assert keep is False
        assert result["selected_q8_decode_path"] == "direct_bf16"
        ...
        assert "direct_bf16" in REJECTION.read_text()
...
def _ab_stub(
    ...
    for ident in LEGAL_CANDIDATES:
        path = "direct_bf16" if ident == "direct_bf16" else "dp4a_q8_1"
        warps = 0 if ident == "direct_bf16" else int(ident.rsplit("_w", 1)[1])
        mean = means.get(ident, 1.0)
        shapes = {name: _shape_slot(mean, eligible=eligible) for name in SHAPES}
        candidates[ident] = {
            "id": ident,
            "path": path,
            "warps_per_row": warps,
            "direct": ident == "direct_bf16",
            "occupancy": 1,
            "registers": 40,
            "local_bytes": 0,
            "cooperative": ident != "direct_bf16",
            "dp4a": ident != "direct_bf16",
            "eligible": eligible,
            "weighted_group_ms": mean,
            "shapes": shapes,
            "gdn_group": _timed(mean),
            "attn_group": _timed(mean),
        }
    return {"winner": winner, "win": winner != "direct_bf16", "candidates": candidates}
...
def _reject_fixture() -> dict[str, Any]:
    return {
        ...
        "reverted": True,
        "selected_q8_decode_path": "direct_bf16",
        "selected_q8_decode_warps_skinny": 4,
        "selected_q8_decode_warps_medium": 4,
        "selected_q8_decode_warps_wide": 4,
        "opt046_selected_q4_decode_path": "packed",
        "ab": _ab_stub("direct_bf16", {ident: 1.0 for ident in LEGAL_CANDIDATES}),
        "production_numerics": {
            "formula": "max(strict_reference_ceiling, 1.05 * measured_llama_error + 1e-6)",
            "budget": {
                "max_abs": 18.6608887,
                "rms": 8.139086731,
                "one_minus_cosine": 8.571427969e-6,
            },
            "activation_approximation": (
                "Q8_1 block quantization of previously direct-BF16 mixer activations"
            ),
        },
        ...
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt047-q8-decode/REPORT.md",
    }


def test_opt047_contract_and_source_pins() -> None:
    contract = _contract()
    ...
    assert contract["opt045_quartz_p_mean_tok_s"] == OPT045_P
    assert contract["opt045_quartz_d2048_mean_tok_s"] == OPT045_D2048
    assert contract["opt046_selected_q4_decode_path"] == "packed"
    assert contract["ab"]["candidates"] == list(LEGAL_CANDIDATES)
    ...
    assert "opt047_q8_decode_ab_test" not in makefile
    assert "cuda/q8_decode_dots.cuh" in makefile
    assert "cuda/q8_decode_path.cuh" in makefile
    ab = (ROOT / "cuda/opt047_q8_decode_ab_test.cu").read_text()
    assert "QW38_CUDA_TEST_TIER must be set" in ab
    assert "test_tier_valid" in ab


def test_opt047_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("task", "OPT-046"),
        lambda x: x["ab"].__setitem__("winner", "dp4a_w4"),
        lambda x: x.__setitem__("selected_q8_decode_path", "dp4a_q8_1"),
    ...
    if rejection_prev is None:
        REJECTION.write_text("rejected placeholder direct_bf16\n")
    try:
...
def test_opt047_fixture_connected() -> None:
    if not FIXTURE.is_file():
        pytest.skip("OPT-047 fixture is written by the exclusive CUDA sitting")
...
def _set_pin(path: str, skinny: int, medium: int, wide: int) -> None:
    source = ROOT / "cuda/q8_decode_path.cuh"
    text = source.read_text()
    text = re.sub(
        r'kSelectedQ8DecodePath\[\] = "[^"]+"',
        f'kSelectedQ8DecodePath[] = "{path}"',
        text,
    )
    text = re.sub(
        r"kSelectedQ8DecodeWarpsSkinny = [0-9]+",
        f"kSelectedQ8DecodeWarpsSkinny = {skinny}",
        text,
    )
    text = re.sub(
        r"kSelectedQ8DecodeWarpsMedium = [0-9]+",
        f"kSelectedQ8DecodeWarpsMedium = {medium}",
        text,
    )
    text = re.sub(
        r"kSelectedQ8DecodeWarpsWide = [0-9]+",
        f"kSelectedQ8DecodeWarpsWide = {wide}",
        text,
    )
    source.write_text(text)
    contract = _contract()
    contract["selected_q8_decode_path"] = path
    contract["selected_q8_decode_warps_skinny"] = skinny
    contract["selected_q8_decode_warps_medium"] = medium
    contract["selected_q8_decode_warps_wide"] = wide
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    bf16_ms = ab["candidates"]["direct_bf16"]["weighted_group_ms"]
    winner = ab["winner"]
    winner_ms = ab["candidates"][winner]["weighted_group_ms"]
    text = f"""# OPT-047 — Q8_0 mixer decode DP4A with shared staging
...
OPT-045 keep denominators are P {OPT045_P}, D128 {OPT045_D128},
...
selected_q8_decode_path={fixture["selected_q8_decode_path"]};
warps skinny/medium/wide={fixture["selected_q8_decode_warps_skinny"]}/
{fixture["selected_q8_decode_warps_medium"]}/
{fixture["selected_q8_decode_warps_wide"]};
A/B winner {winner} (direct_bf16 {bf16_ms} ms, winner {winner_ms} ms);
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    ab = fixture["ab"]
    reason = (
        "component A/B retained direct_bf16"
    ...
    text = f"""# OPT-047 rejection

{reason}. Production Q8_0 mixer decode remains direct BF16
(`selected_q8_decode_path=direct_bf16`). OPT-046 packed Q4_K stays.
...
def _run_ab(tier: str) -> dict[str, Any]:
    ...
    commands = [
        ...
        _nvcc(
            "cuda/opt047_q8_decode_ab_test.cu",
            "build/qw38-cuda-opt047-q8-decode-ab-test",
            extra=AB_OBJECTS,
        ),
        [
            *_common(IMAGE, tier),
...
def _fixture_from_ab(ab: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reverted": True,
        "selected_q8_decode_path": "direct_bf16",
        "selected_q8_decode_warps_skinny": warps,
        "selected_q8_decode_warps_medium": warps,
        "selected_q8_decode_warps_wide": warps,
        "opt046_selected_q4_decode_path": "packed",
        "ab": {
            "winner": block["winner"],
            "win": block["win"],
        ...
        "keep_sitting_skipped": selected == "direct_bf16",
        "p": None,
        "d128": None,
        "d2048": None,
        "owns_opt016_parity_gate": False,
...
def test_opt047_exclusive_cuda_sitting() -> None:
    tier = cuda_test_tier()
    if not MODEL.exists():
    ...
    closed = subprocess.run(
        [*_common(IMAGE), "./build/qw38-cuda-opt047-q8-decode-ab-test"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    if tier != "acceptance":
        assert ab["task"] == "OPT-047"
        assert ab["winner"] in LEGAL_CANDIDATES
        return

    wrapper = _fixture_from_ab(ab)
    selected = wrapper.pop("_install")
    warps = wrapper.pop("_warps")
    if selected == "direct_bf16":
        _set_pin("direct_bf16", 4, 4, 4)
        wrapper["selected_q8_decode_warps_skinny"] = 4
        wrapper["selected_q8_decode_warps_medium"] = 4
        wrapper["selected_q8_decode_warps_wide"] = 4
    ...
    _set_pin("dp4a_q8_1", warps, warps, warps)
    _run(
        [
            *_common(IMAGE),
            "make",
    ...
    fixture["selected_q8_decode_path"] = "dp4a_q8_1"
    fixture["selected_q8_decode_warps_skinny"] = warps
    fixture["selected_q8_decode_warps_medium"] = warps
    fixture["selected_q8_decode_warps_wide"] = warps
    fixture["p"] = {
        "quartz": {
            "prompt_tokens": 4096,
            "replicates": 3,
    ...
    if _keep_predicates(fixture):
        ...
        fixture["selected_q8_decode_path"] = "direct_bf16"
        fixture["selected_q8_decode_warps_skinny"] = 4
        fixture["selected_q8_decode_warps_medium"] = 4
        fixture["selected_q8_decode_warps_wide"] = 4
        _set_pin("direct_bf16", 4, 4, 4)
        _write_report(fixture)
