...
PREFIX = "QW38_PREFILL_4K_SKINNY_RESULT="
CONTRACT = ROOT / "pins/opt023_skinny_mixer_contract.json"
FIXTURE = ROOT / "fixtures/opt023_skinny_mixer.json"
EVIDENCE = ROOT / "evidence/optimization/opt023-skinny-mixer"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
LLAMA_JSON = EVIDENCE / "llama-bench-4k.json"
QUARTZ_JSON = EVIDENCE / "quartz-4k.json"
AB_RAW = EVIDENCE / "skinny-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
LLAMA_BENCH = ROOT / ".cache/authorities/llama-build/bin/llama-bench"
OPT022_MEAN = 1680.80627
SKINNY_IDS = ("mma_i32_j128", "mma_i64_j128", "mmv_tiled_j1")
PROOF = (
    "skinny-M mixer dispatch; Q8 association; "
...
def scheduler_alpha_uses_mmv() -> bool:
    text = (ROOT / "cuda/full_scheduler.cu").read_text()
    return (
        "layer.gdn.alpha" in text
        and "launch_q8_mmq_bf16_variant" in text
        and "layer.gdn.alpha" in text[text.find("launch_q8_mmq_bf16_variant") :]
    )


def production_is_skinny() -> bool:
    path = selected_path_from_source()
    if path in ("mma_i32_j128", "mma_i64_j128"):
        return not scheduler_alpha_uses_mmv()
    if path == "mmv_tiled_j1":
        return scheduler_alpha_uses_mmv()
    return False


def production_is_baseline() -> bool:
    return (
        selected_path_from_source() == "mma_i128_j128"
        and not scheduler_alpha_uses_mmv()
    )
...
def decode_mmv_unchanged() -> bool:
    mmv = (ROOT / "cuda/quant_mmv.cu").read_text()
    scheduler = (ROOT / "cuda/full_scheduler.cu").read_text()
    return (
        "unsigned int selected_mmv_warps(std::size_t rows) noexcept" in mmv
        and "q8_mmv_bf16<<<blocks, kThreads, 0, stream>>>" in scheduler
        and "return launch_quant_mmv(" in scheduler
    )
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-023"
    ...
    assert contract["opt022_quartz_mean_tok_s"] == OPT022_MEAN
    ...
    assert result["opt022_quartz_mean_tok_s"] == OPT022_MEAN
    assert (
        result["selected_skinny_mixer_path"] == contract["selected_skinny_mixer_path"]
    )
    assert result["ab_winner"] in (
        "mma_i128_j128",
        "mma_i32_j128",
        "mma_i64_j128",
        "mmv_tiled_j1",
    ...
    assert result["large_mixer_quality_mma"] is True
    assert result["decode_mmv_unchanged"] is True
    keep = (
        float(quartz["mean_tok_s"]) > OPT022_MEAN
        and result["ab_win"] is True
        and result["ab_winner"] in SKINNY_IDS
    )
    if result["status"] == "measured":
        assert keep
        assert result["reverted"] is False
        assert result["successor_oracle"] is True
        assert result["production_skinny"] is True
        assert result["selected_skinny_mixer_path"] == result["ab_winner"]
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_skinny"] is False
        assert result["selected_skinny_mixer_path"] == "mma_i128_j128"
        assert REJECTION.is_file()
        rejection = REJECTION.read_text()
        assert str(quartz["mean_tok_s"]) in rejection
        assert str(OPT022_MEAN) in rejection
...
def test_opt023_contract_and_fixture_are_connected() -> None:
    ...
    opt022 = json.loads((ROOT / "fixtures/opt022_mixer_q8_quality.json").read_text())
    assert opt022["quartz"]["mean_tok_s"] == OPT022_MEAN
    assert contract["opt022_quartz_mean_tok_s"] == OPT022_MEAN
    ...
    assert selected_path_from_source() == contract["selected_skinny_mixer_path"]


def test_opt023_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("opt022_quartz_mean_tok_s", 1.0),
        lambda x: x.__setitem__("report_path", "/tmp/opt023/REPORT.md"),
        lambda x: x.__setitem__("decode_mmv_unchanged", False),
        lambda x: x.__setitem__("large_mixer_quality_mma", False),
    ):
    ...
    if fixture["status"] == "measured":
        ...
        keep_break["quartz"]["mean_tok_s"] = OPT022_MEAN
        keep_break["quartz"]["tok_s"] = [OPT022_MEAN] * 3
        mutations.append(keep_break)
        flag_break = json.loads(json.dumps(fixture))
        flag_break["reverted"] = True
        mutations.append(flag_break)
        skinny_break = json.loads(json.dumps(fixture))
        skinny_break["ab_win"] = False
        mutations.append(skinny_break)
    else:
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-skinny-test",
    ]
    commands = [
...
def _parse_ab(raw_text: str) -> tuple[str, bool]:
    winner = "mma_i128_j128"
    win = False
    for line in raw_text.splitlines():
        if not line.startswith("skinny_ab_winner "):
            continue
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-023 — Skinny-M mixer dispatch for small output rows
...
{OPT022_MEAN} tok/s. Numeric and exact-state **envelopes unloosened**. This
...
**{decision}** — Quartz mean tok/s {quartz["mean_tok_s"]} versus OPT-022 baseline {OPT022_MEAN}.
A/B winner `{fixture["ab_winner"]}` win={json.dumps(fixture["ab_win"])};
`selected_skinny_mixer_path`={fixture["selected_skinny_mixer_path"]};
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_skinny`={json.dumps(fixture["production_skinny"])}.
...
| Nsight | not_used |
| Keep/reject | strictly greater Quartz mean tok/s than {OPT022_MEAN} and A/B skinny win |
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    quartz = fixture["quartz"]
    text = f"""# OPT-023 rejection

Skinny-M mixer dispatch was measured on exclusive RTX 5090 and did not
strictly beat the OPT-022 Quartz baseline, or the paired CUDA-event A/B did
not install a skinny winner.

- quality sitting mean tok/s: {quartz["mean_tok_s"]}
- OPT-022 baseline: {OPT022_MEAN}
...
- production mixer α/β remain OPT-022 I=128 J=128 quality MMA
- successor_oracle: false (OPT-024+ denominator remains {OPT022_MEAN})
- measurement_utc: {fixture["measurement_utc"]}
"""
    REJECTION.write_text(text)


def test_opt023_native_4k_keep_reject() -> None:
    ...
    if ab_win:
        assert production_is_skinny(), "4K sitting must run on the winning skinny path"
        assert selected_path_from_source() == ab_winner
    else:
        assert production_is_baseline()
    assert scheduler_shares_residual_y()
    assert decode_mmv_unchanged()
    llama = _run_llama_bench()
    quartz = _run_quartz()
    mean_tok_s = float(quartz["mean_tok_s"])
    avg_ts = float(llama["avg_ts"])
    quartz_meets_llama = mean_tok_s >= avg_ts
    keep = mean_tok_s > OPT022_MEAN and ab_win and ab_winner in SKINNY_IDS
    path = ab_winner if keep else "mma_i128_j128"
    fixture = {
        "schema_version": 1,
        "task": "OPT-023",
        ...
        "opt022_quartz_mean_tok_s": OPT022_MEAN,
        "quartz_meets_llama": quartz_meets_llama,
        "owns_opt016_parity_gate": False,
        "substitutes_for_opt016": False,
        "reverted": False if keep else True,
        "successor_oracle": True if keep else False,
        "selected_skinny_mixer_path": path,
        "ab_winner": ab_winner,
        "ab_win": ab_win,
        "production_skinny": True if keep else False,
        "shared_residual_y": True,
        "large_mixer_quality_mma": True,
        "decode_mmv_unchanged": True,
        "nsight_systems": "not_used",
        "nsight_compute": "not_used",
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt023-skinny-mixer/REPORT.md",
    }
    assert quartz["graphs_created"] is True
    assert quartz["prompt_graph_rows"] == 4096
    assert quartz["prompt_tokens"] == 4096
    ...
    if not keep:
        ...
        pytest.fail(
            f"keep failed: quartz mean {mean_tok_s} vs {OPT022_MEAN}; "
            f"ab_winner={ab_winner} ab_win={ab_win}; revert skinny dispatch "
            "and retain this sitting"
        )
