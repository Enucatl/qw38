...
PREFIX = "QW38_PREFILL_4K_KEEP_RESULT="
CONTRACT = ROOT / "pins/opt022_mixer_q8_quality_contract.json"
FIXTURE = ROOT / "fixtures/opt022_mixer_q8_quality.json"
EVIDENCE = ROOT / "evidence/optimization/opt022-mixer-q8-quality"
...
OPT021_MEAN = 967.267761
PROOF = (
    "mixer Q8_0 quality MMQ; shared residual Y; Q8 association; "
...
def _launch_q8_mmq_bf16_body() -> str:
    text = (ROOT / "cuda/quant_mmv.cu").read_text()
    start = text.index("cudaError_t launch_q8_mmq_bf16(")
    end = text.index("cudaError_t launch_q8_mmq_bf16_reference(")
    return text[start:end]


def production_is_quality() -> bool:
    body = _launch_q8_mmq_bf16_body()
    return "launch_q8_mmq_quality" in body and "launch_q8_mmq_mma(" not in body


def production_is_rank1() -> bool:
    body = _launch_q8_mmq_bf16_body()
    return "launch_q8_mmq_mma(" in body and "launch_q8_mmq_quality" not in body
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-022"
    ...
    assert contract["opt021_quartz_mean_tok_s"] == OPT021_MEAN
    ...
    assert result["opt021_quartz_mean_tok_s"] == OPT021_MEAN
    ...
    keep = float(quartz["mean_tok_s"]) > OPT021_MEAN
    if result["status"] == "measured":
        assert keep
        assert result["reverted"] is False
        assert result["successor_oracle"] is True
        assert result["production_q8"] == "quality_mma_shared_y"
        assert result["shared_residual_y"] is True
        assert result["kernel_nodes"] == 2
        assert result["block"] == [32, 8, 1]
        assert not REJECTION.is_file()
    else:
        assert not keep
        assert result["reverted"] is True
        assert result["successor_oracle"] is False
        assert result["production_q8"] == "opt017_rank1_mma"
        assert result["shared_residual_y"] is False
        assert result["kernel_nodes"] == 1
        assert result["block"] == [256, 1, 1]
        assert REJECTION.is_file()
        rejection = REJECTION.read_text()
        assert str(quartz["mean_tok_s"]) in rejection
        assert str(OPT021_MEAN) in rejection
...
def test_opt022_contract_and_fixture_are_connected() -> None:
    ...
    opt021 = json.loads((ROOT / "fixtures/opt021_oracle.json").read_text())
    assert opt021["quartz"]["mean_tok_s"] == OPT021_MEAN
    assert contract["opt021_quartz_mean_tok_s"] == OPT021_MEAN
...
def test_opt022_validator_rejects_inadmissible_evidence() -> None:
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("opt021_quartz_mean_tok_s", 1.0),
        lambda x: x.__setitem__("report_path", "/tmp/opt022/REPORT.md"),
    ):
    ...
    if fixture["status"] == "measured":
        ...
        keep_break["quartz"]["mean_tok_s"] = OPT021_MEAN
        keep_break["quartz"]["tok_s"] = [OPT021_MEAN] * 3
...
def _run_quartz() -> dict[str, Any]:
    nvcc = [
        *_common(IMAGE),
        ...
        "build/utf8proc.o",
        "-o",
        "build/qw38-cuda-prefill-4k-keep-test",
    ]
    commands = [
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-022 — Mixer Q8_0 quality MMQ + shared residual Y
...
versus the frozen OPT-021 Quartz mean {OPT021_MEAN} tok/s. Numeric and exact-state
...
**{decision}** — Quartz mean tok/s {quartz["mean_tok_s"]} versus OPT-021 baseline {OPT021_MEAN}.
`reverted`={json.dumps(fixture["reverted"])}; `successor_oracle`={json.dumps(fixture["successor_oracle"])};
`production_q8`={fixture["production_q8"]}; `shared_residual_y`={json.dumps(fixture["shared_residual_y"])}.
...
| Nsight | not_used |
| Keep/reject | strictly greater Quartz mean tok/s than {OPT021_MEAN} |
...
- kernel_nodes: {fixture["kernel_nodes"]}; block: {fixture["block"]}
"""
    EVIDENCE.mkdir(parents=True, exist_ok=True)
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    quartz = fixture["quartz"]
    text = f"""# OPT-022 rejection

Quality mixer Q8_0 MMQ with shared residual Y was measured on exclusive RTX 5090
and did not strictly beat the OPT-021 Quartz baseline.

- quality sitting mean tok/s: {quartz["mean_tok_s"]}
- OPT-021 baseline: {OPT021_MEAN}
- rule: strictly_greater_mean_tok_s (equality is a reject)
- production mixer Q8_0 reverted to OPT-017 Rank-1 `launch_q8_mmq_mma`
- successor_oracle: false (OPT-023+ denominator remains {OPT021_MEAN})
- measurement_utc: {fixture["measurement_utc"]}
"""
    REJECTION.write_text(text)


def test_opt022_native_4k_keep_reject() -> None:
    ...
    if FIXTURE.is_file():
        ...
        if existing.get("status") == "rejected" and existing.get("reverted") is True:
            assert production_is_rank1()
            validate_result(existing)
            return

    assert production_is_quality(), "4K sitting must run on the quality production path"
    ...
    fixture = {
        ...
        "opt021_quartz_mean_tok_s": OPT021_MEAN,
        "quartz_meets_llama": quartz_meets_llama,
        "owns_opt016_parity_gate": False,
        "substitutes_for_opt016": False,
        "reverted": False if keep else True,
        "successor_oracle": True if keep else False,
        "production_q8": "quality_mma_shared_y" if keep else "opt017_rank1_mma",
        "shared_residual_y": True if keep else False,
        "kernel_nodes": 2 if keep else 1,
        "block": [32, 8, 1] if keep else [256, 1, 1],
        "nsight_systems": "not_used",
        "nsight_compute": "not_used",
        "proof_limit": PROOF,
        "report_path": "evidence/optimization/opt022-mixer-q8-quality/REPORT.md",
    }
    assert quartz["graphs_created"] is True
    assert quartz["prompt_graph_rows"] == 4096
    assert quartz["prompt_tokens"] == 4096
    ...
    if not keep:
        ...
        pytest.fail(
            f"keep failed: quartz mean {mean_tok_s} is not strictly greater than "
            f"{OPT021_MEAN}; revert production mixer Q8 to Rank-1 and retain this sitting"
        )
