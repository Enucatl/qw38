...
CONTRACT = ROOT / "pins/opt036_decode_kv_partition_contract.json"
FIXTURE = ROOT / "fixtures/opt036_decode_kv_partition.json"
EVIDENCE = ROOT / "evidence/optimization/opt036-decode-kv-partition"
REPORT = EVIDENCE / "REPORT.md"
REJECTION = EVIDENCE / "REJECTION.md"
AB_RAW = EVIDENCE / "kv-ab-raw.txt"
MODEL = ROOT / "models" / "Qwen3.8-27B-Q4_K_M.gguf"
GGUF_SHA = "31629f53165ab6a7dad8c9847dcfd1fdf55829dac1e6e748f4a68581b0033d34"
LLAMA_REV = "cc83d7b4824f73cfdda4dfbb47ee39804f71b328"
AB_PREFIX = "QW38_DECODE_KV_AB_RESULT="
...
OPT032_P = 1637.58594
OPT032_D128 = 11.8731956
OPT032_D2048 = 3.70951414
OPT032_D128_P95 = 95.380661
OPT032_D128_RUN_P95 = 84.2481613
OPT032_D2048_P95 = 280.727844
OPT032_D2048_RUN_P95 = 269.61731
HISTORICAL_P = 1746.71973
LEGAL_PARTS = {1, 4, 8, 16}
PROOF = (
    "frozen attention envelopes; lower component time; improved D2048; "
    "cross-workload guard; does not substitute for the 2K llama.cpp parity gate; "
    "decode-oracle P D128 D2048 are the keep denominators"
)
NVCC_OBJECTS = [
...
def pins_from_source() -> tuple[int, int]:
    text = (ROOT / "cuda/attention_decode.cu").read_text()
    low = int(re.search(r"kSelectedDecodeKvPartsLow = (\d+)", text).group(1))
    high = int(re.search(r"kSelectedDecodeKvPartsHigh = (\d+)", text).group(1))
    return low, high
...
def fattn_path_unchanged() -> bool:
    ...
    return 'kSelectedFattnPath[] = "stream_k"' in text
...
def _ab_candidate(block: dict[str, Any], n_parts: int) -> None:
    cand = block["candidates"][str(n_parts)]
    assert cand["n_parts"] == n_parts
...
def _select_winner(block: dict[str, Any]) -> int:
    assert set(block["candidates"]) == {"1", "4", "8", "16"}
    one = block["candidates"]["1"]
    winner = 1
    best = float(one["mean_ms"])
    if not one["eligible"]:
        return 1
    for n_parts in (4, 8, 16):
        cand = block["candidates"][str(n_parts)]
        mean = float(cand["mean_ms"])
        if cand["eligible"] and mean < float(one["mean_ms"]) and mean < best:
            winner = n_parts
            best = mean
    return winner
...
def _keep_predicates(result: dict[str, Any]) -> bool:
    contract = _contract()
    high = int(result["selected_decode_kv_parts_at_or_above_2048"])
    low = int(result["selected_decode_kv_parts_below_2048"])
    if high not in {4, 8, 16}:
        return False
    if low not in LEGAL_PARTS:
        return False
    ...
    if quartz_d2048 <= contract["opt032_quartz_d2048_mean_tok_s"]:
        return False
    quartz_d128 = float(result["d128"]["quartz"]["mean_tok_s"])
    if quartz_d128 < 0.95 * contract["opt032_quartz_d128_mean_tok_s"]:
        return False
    if (
        result["d128"]["quartz"]["token_latency_p95_ms"]
        > 1.05 * contract["opt032_quartz_d128_token_latency_p95_ms"]
    ):
        return False
    if (
        result["d128"]["quartz"]["run_mean_token_latency_p95_ms"]
        > 1.05 * contract["opt032_quartz_d128_run_mean_token_latency_p95_ms"]
    ):
        return False
    if (
        result["d2048"]["quartz"]["token_latency_p95_ms"]
        > 1.05 * contract["opt032_quartz_d2048_token_latency_p95_ms"]
    ):
        return False
    if (
        result["d2048"]["quartz"]["run_mean_token_latency_p95_ms"]
        > 1.05 * contract["opt032_quartz_d2048_run_mean_token_latency_p95_ms"]
    ):
        return False
    if (
        float(result["p"]["quartz"]["mean_tok_s"])
        < 0.95 * contract["opt032_quartz_p_mean_tok_s"]
    ):
        return False
    return True
...
def validate_result(result: Any) -> None:
    ...
    assert result["schema_version"] == 1 and result["task"] == "OPT-036"
    assert result["status"] in ("measured", "rejected")
    assert result["status"] not in ("scout", "source_inspected")
    assert contract["yardstick"] == "d2048_decode_attention_kv_partition"
    assert contract["opt032_quartz_p_mean_tok_s"] == OPT032_P
    assert contract["opt032_quartz_d128_mean_tok_s"] == OPT032_D128
    assert contract["opt032_quartz_d2048_mean_tok_s"] == OPT032_D2048
    ...
    low = int(result["selected_decode_kv_parts_below_2048"])
    high = int(result["selected_decode_kv_parts_at_or_above_2048"])
    assert low in LEGAL_PARTS and high in LEGAL_PARTS
    assert _select_winner(result["ab_d128"]) == result["ab_d128"]["winner"]
    assert _select_winner(result["ab_d2048"]) == result["ab_d2048"]["winner"]
    _ab_candidate(result["ab_d128"], 1)
    _ab_candidate(result["ab_d128"], 4)
    _ab_candidate(result["ab_d128"], 8)
    _ab_candidate(result["ab_d128"], 16)
    _ab_candidate(result["ab_d2048"], 1)
    _ab_candidate(result["ab_d2048"], 4)
    _ab_candidate(result["ab_d2048"], 8)
    _ab_candidate(result["ab_d2048"], 16)
    assert result["ab_d128"]["position"] == 128
    assert result["ab_d2048"]["position"] == 2048
    source_low, source_high = pins_from_source()
    assert source_low == low == contract["selected_decode_kv_parts_below_2048"]
    assert source_high == high == contract["selected_decode_kv_parts_at_or_above_2048"]
...
def _candidate_stub(n_parts: int, mean: float, eligible: bool = True) -> dict[str, Any]:
    return {
        "n_parts": n_parts,
        "mean_ms": mean,
        "samples": [mean] * 30,
        ...
        "vs_reference": {"max_abs": 0.0, "rms": 0.0, "nonfinite": 0},
    }


def _ab_stub(position: int, winner: int, means: dict[int, float]) -> dict[str, Any]:
    return {
        "position": position,
        "winner": winner,
        "win": winner != 1,
        "candidates": {str(n): _candidate_stub(n, means[n]) for n in (1, 4, 8, 16)},
    }


def test_opt036_contract_and_source_pins() -> None:
    contract = _contract()
    opt032 = json.loads((ROOT / "fixtures/opt032_decode_oracle.json").read_text())
    assert opt032["p"]["quartz"]["mean_tok_s"] == OPT032_P
    assert opt032["d128"]["quartz"]["mean_tok_s"] == OPT032_D128
    assert opt032["d2048"]["quartz"]["mean_tok_s"] == OPT032_D2048
    assert contract["opt032_quartz_p_mean_tok_s"] == OPT032_P
    assert contract["ab"]["candidates"] == [1, 4, 8, 16]
    assert contract["ab"]["positions"] == [128, 2048]
    ...
    low, high = pins_from_source()
    assert low in LEGAL_PARTS and high in LEGAL_PARTS
    assert scheduler_dispatches_partitioned()
    assert fattn_path_unchanged()


def test_opt036_validator_rejects_inadmissible_evidence() -> None:
    means = {1: 2.0, 4: 3.0, 8: 4.0, 16: 5.0}
    fixture = {
        "schema_version": 1,
        "task": "OPT-036",
        "status": "rejected",
        ...
        "reverted": True,
        "selected_decode_kv_parts_below_2048": 1,
        "selected_decode_kv_parts_at_or_above_2048": 1,
        "ab_d128": _ab_stub(128, 1, means),
        "ab_d2048": _ab_stub(2048, 1, means),
    ...
    for mutate in (
        ...
        lambda x: x.__setitem__("report_path", "/tmp/opt036/REPORT.md"),
        lambda x: x.__setitem__("status", "source_inspected"),
        lambda x: x.__setitem__("task", "OPT-033"),
        lambda x: x["ab_d2048"].__setitem__("winner", 4),
        lambda x: x.__setitem__("selected_decode_kv_parts_at_or_above_2048", 4),
    ...
    equality["selected_decode_kv_parts_at_or_above_2048"] = 8
    equality["ab_d2048"] = _ab_stub(2048, 8, {1: 2.0, 4: 1.5, 8: 1.0, 16: 3.0})
    equality["p"] = {"quartz": {"mean_tok_s": OPT032_P}}
    equality["d128"] = {
        "quartz": {
            "mean_tok_s": OPT032_D128,
            "token_latency_p95_ms": OPT032_D128_P95,
            "run_mean_token_latency_p95_ms": OPT032_D128_RUN_P95,
        }
    }
    equality["d2048"] = {
        "quartz": {
            "mean_tok_s": OPT032_D2048,
            "token_latency_p95_ms": OPT032_D2048_P95,
            "run_mean_token_latency_p95_ms": OPT032_D2048_RUN_P95,
        }
    }
    mutations.append(equality)
    historical = json.loads(json.dumps(equality))
    historical["p"]["quartz"]["mean_tok_s"] = HISTORICAL_P
    mutations.append(historical)
    loosened = json.loads(json.dumps(fixture))
    loosened["ab_d2048"]["candidates"]["4"]["vs_tiled"]["max_abs"] = 0.1
    loosened["ab_d2048"]["candidates"]["4"]["eligible"] = True
    loosened["ab_d2048"]["winner"] = 4
    mutations.append(loosened)
    skip_d128 = json.loads(json.dumps(fixture))
    skip_d128["ab_d128"]["candidates"].pop("4")
    mutations.append(skip_d128)
    ...
    if rejection_prev is None:
        REJECTION.write_text("rejected placeholder 1\n")
    try:
...
def test_opt036_fixture_connected() -> None:
    if not FIXTURE.is_file():
        pytest.skip("OPT-036 fixture is written by the exclusive CUDA sitting")
...
def _set_pins(low: int, high: int) -> None:
    source = ROOT / "cuda/attention_decode.cu"
    text = source.read_text()
    text = re.sub(
        r"kSelectedDecodeKvPartsLow = \d+",
        f"kSelectedDecodeKvPartsLow = {low}",
        text,
    )
    text = re.sub(
        r"kSelectedDecodeKvPartsHigh = \d+",
        f"kSelectedDecodeKvPartsHigh = {high}",
        text,
    )
    source.write_text(text)
    contract = _contract()
    contract["selected_decode_kv_parts_below_2048"] = low
    contract["selected_decode_kv_parts_at_or_above_2048"] = high
...
def _write_report(fixture: dict[str, Any]) -> None:
    ...
    text = f"""# OPT-036 — Partition KV for vector decode attention
...
(P {OPT032_P}, D128 {OPT032_D128}, D2048 {OPT032_D2048}), not
historical OPT-026 {HISTORICAL_P}.

## Decision

**{decision}** — `reverted`={json.dumps(fixture["reverted"])};
`keep_sitting_skipped`={json.dumps(fixture["keep_sitting_skipped"])};
selected below-2048={fixture["selected_decode_kv_parts_below_2048"]};
selected at-or-above-2048={fixture["selected_decode_kv_parts_at_or_above_2048"]};
D128 A/B winner {d128_ab["winner"]} (1-part mean {d128_ab["candidates"]["1"]["mean_ms"]} ms);
D2048 A/B winner {d2048_ab["winner"]} (1-part mean {d2048_ab["candidates"]["1"]["mean_ms"]} ms);
...
- P Quartz mean tok/s: {quartz_p} versus OPT-032 {OPT032_P}
- D128 Quartz mean tok/s: {quartz_d128} versus OPT-032 {OPT032_D128}
- D2048 Quartz mean tok/s: {quartz_d2048} versus OPT-032 {OPT032_D2048}
- owns_opt016_parity_gate: false
- substitutes_for_opt016: false
"""
    EVIDENCE.mkdir(parents=True, exist_ok=True)
...
def _write_rejection(fixture: dict[str, Any]) -> None:
    ...
    reason = (
        "D2048 component A/B selected one-partition"
    ...
    tok = (
        ...
        f"- OPT-032 D2048 baseline: {OPT032_D2048}\n"
        f"- P Quartz mean tok/s: {fixture['p']['quartz']['mean_tok_s']}\n"
        f"- D128 Quartz mean tok/s: {fixture['d128']['quartz']['mean_tok_s']}"
    )
    text = f"""# OPT-036 rejection

{reason}. Production decode attention is one-partition tiled
(`selected_decode_kv_parts_below_2048=1`,
`selected_decode_kv_parts_at_or_above_2048=1`).

- D128 A/B winner: {fixture["ab_d128"]["winner"]} mean_1={d128_ab_mean(fixture)}
- D2048 A/B winner: {d2048["winner"]} mean_1={d2048["candidates"]["1"]["mean_ms"]}
- D2048 candidate means: 1={d2048["candidates"]["1"]["mean_ms"]} 4={d2048["candidates"]["4"]["mean_ms"]} 8={d2048["candidates"]["8"]["mean_ms"]} 16={d2048["candidates"]["16"]["mean_ms"]}
- keep_sitting_skipped: {json.dumps(fixture["keep_sitting_skipped"])}
...
def d128_ab_mean(fixture: dict[str, Any]) -> float:
    return fixture["ab_d128"]["candidates"]["1"]["mean_ms"]


def _flatten_ab(block: dict[str, Any]) -> dict[str, Any]:
    return block
...
def _run_ab() -> dict[str, Any]:
    ...
    commands = [
        ...
        _nvcc(
            "cuda/decode_kv_partition_ab_test.cu",
            "build/qw38-cuda-decode-kv-ab-test",
            ["build/attention_decode.cuda.o"],
        ),
        [
            *_common(IMAGE),
...
def _engine_block(record: dict[str, Any], sidecar: str | None = None) -> dict[str, Any]:
    ...
    if sidecar is not None:
        ...
        block["token_latency_sidecar"] = (
            f"evidence/optimization/opt036-decode-kv-partition/{sidecar}"
        )
...
def test_opt036_native_keep_reject() -> None:
    ...
    ab = _run_ab()
    low = int(ab["ab_d128"]["winner"])
    high = int(ab["ab_d2048"]["winner"])
    if high == 1:
        low = 1
        high = 1
        _set_pins(1, 1)
        fixture = {
            ...
            "selected_decode_kv_parts_below_2048": 1,
            "selected_decode_kv_parts_at_or_above_2048": 1,
            "ab_d128": _flatten_ab(ab["ab_d128"]),
            "ab_d2048": _flatten_ab(ab["ab_d2048"]),
            "keep_sitting_skipped": True,
            "p": None,
            "d128": None,
            "d2048": None,
    ...
    fixture = {
        ...
        "gguf_sha256": GGUF_SHA,
        "reverted": False,
        "selected_decode_kv_parts_below_2048": low,
        "selected_decode_kv_parts_at_or_above_2048": high,
        "ab_d128": _flatten_ab(ab["ab_d128"]),
        "ab_d2048": _flatten_ab(ab["ab_d2048"]),
    ...
    if not keep:
        ...
        fixture["selected_decode_kv_parts_below_2048"] = 1
        fixture["selected_decode_kv_parts_at_or_above_2048"] = 1
        _write_report(fixture)
        _write_rejection(fixture)
        _run([*_common(IMAGE), "make", "build/qw38-cuda-timing-test"])
    else:
